...
└── assets/
    ...
    └── screenshots/        @2x captures from the real app in Browser Mock Mode
...
Screenshots come from Quill's real React frontend running in Browser Mock Mode
inside a network-disabled Docker container. The container receives no host
display socket, personal home, Quill database, or published port.

```sh
./scripts/capture_screenshots_docker.sh
```

The command starts Vite and headless Chromium inside the container, then uses
Chrome DevTools Protocol to drive the actual app components. Tauri IPC calls go
through the existing dev-only fixtures in `src/mocks/ipcFixtures.ts`; the normal
production UI code remains unchanged. The wrapper replaces canonical PNGs only
after all nine captures and dimensions validate.

See [`specs/001-marketing-site/quickstart.md`](../specs/001-marketing-site/quickstart.md).
