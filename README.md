...
This runs Quill's real React frontend in its documented Browser Mock Mode,
drives it through headless Chromium, validates every expected PNG, and only then
replaces `marketing-site/assets/screenshots/`. The screenshot runtime has no host
display socket, personal home, Quill database, or external network access.

The mock layer is dev-only and routes the app's normal Tauri `invoke()` calls to
`src/mocks/ipcFixtures.ts`; production bundles never include it.
