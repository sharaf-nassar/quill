  ...
  return (
    <div className="codex-live__session" title={session.project ?? session.sessionId}>
      ...
      <div className="codex-live__session-side">
        <span className="codex-live__session-tokens">{formatTokenCount(session.tokens)}</span>
        <span className="codex-live__session-age">{formatRelativeTime(session.lastActive)}</span>
      </div>
    </div>
  );
}
...
function CodexLiveModule({ showHeading = true }: CodexLiveModuleProps) {
  ...
  if (showLoadingBody) {
    return (
      <div className="usage-provider-group codex-live">
        {header}
