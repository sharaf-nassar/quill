...
This contract declares the URL fragment identifiers the marketing site exposes and commits to keeping stable. The seven original anchors (`#hero`, `#live`, `#analytics`, `#context`, `#search`, `#learning`, `#install`) are the hard contract. `#memory`, `#brevity`, `#models`, and `#integrations` are additive. External documents may deep-link to any anchor; the site MUST NOT rename or remove the original seven without a deliberate breaking-change pass.

## Anchored sections (canonical order)

| Anchor        | Section heading (visible)                              | Content                                                             |
|---------------|--------------------------------------------------------|---------------------------------------------------------------------|
| `#hero`         | (no visible heading — top of page)                  | Product-led hero and `hero.png` |
| `#analytics`    | "Usage you can trace back to the work."             | CLI/provider/model usage + `hero.png` |
| `#models`       | "See which models actually did the work."           | Current and session-ranked evidence + `models.png` |
| `#context`      | "Keep bloated context out of the model's window."    | Agent-facing working-context tools + `analytics-context.png` |
| `#search`       | "Every past run, indexed — Claude, Codex, and Pi."   | Description + `sessions.png` |
| `#live`         | "Know the cap before it cuts the run."               | Direct and pooled limits + `live.png` |
| `#learning`     | "Turn repeated behavior into reviewable rules."      | Review-gated rules + `learning.png` |
| `#memory`       | "Keep your agent's memory clean and current."        | Memory tools + `memory.png` |
| `#brevity`      | "Get tighter answers from your agents."              | Context/Brevity settings + `brevity.png` |
| `#integrations` | "One place to manage every connection."              | Provider lifecycle + `settings.png` |
| `#install`      | "Put it beside the agents you already use."          | Provider/platform/privacy and release links |
...
3. **Order stability**: the canonical order above is the visual reading order. Reordering is permitted but MUST keep all seven original anchors (`#hero`, `#live`, `#analytics`, `#context`, `#search`, `#learning`, `#install`) present. Additive anchors may be reordered or removed without a breaking-change pass.
