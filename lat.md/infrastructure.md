...
The seeded dataset is photographed as-seeded; the grooming each shot used to need is now the seeder's job — see [[infrastructure#Scripts#Dummy Data Seeder]]. `populate_dummy_data.py` builds every analytics table in the app's current shape and stamps `schema_version` to match, so `token_snapshots` carries migration 20's `is_sidechain` and `get_session_breakdown` answers instead of reading "Breakdown unavailable", and migration 30 never archives the seeded rows out from under the capture. Each `code_change` row stores its own `lines_added`/`lines_removed`, which is what TOK/LOC, LOC/HR, NET LINES and the Charts CODE timeline read. Tokens, tool actions and turns are attributed across `claude` and `codex`, so the usage chart draws two series, the breakdown rows carry real turn counts and two of them roll up a sub-agent. Claude reports three limit buckets — the plan pair plus one model window — which is the composition the 360px LIMITS row and the mockup share, with exactly one cell in the amber band. MiniMax is seeded enabled but not installed, the state that renders the SETUP row the `#live` copy describes. Confirmed rules are written into `<rules>/{claude,codex,shared}` where demo mode scans for them, so ACTIVE RULES is populated and scope badges vary, while candidates get no file and an empty `file_path` — exactly how the app stores a discovered rule.
...
Schema fidelity is the seeder's load-bearing contract. [[scripts/populate_dummy_data.py#ensure_schema]] creates every table in its post-migration-35 shape — source-owned identity columns (`source_key`, `action_key`, `event_key`, `chain_id`), the sub-agent trio on `token_snapshots`, stored `lines_added`/`lines_removed`, and the reconciliation, rename and retention-aggregate tables — and [[scripts/populate_dummy_data.py#populate_schema_version]] records 1..35 so the app runs none of them. Stamping past a migration whose shape is missing is the failure mode: it silently disables whatever reads the absent column. Seeded analytics rows are LIVE rows (`source_key IS NULL`), the shape reconciliation never deletes.

Reruns are safe against drift. [[scripts/populate_dummy_data.py#drop_stale_tables]] checks each seeder-owned table for the columns the current shape requires and drops any table that predates it, because `CREATE TABLE IF NOT EXISTS` would otherwise keep an older shape and the first insert would fail on a missing `action_key`. Dropping costs nothing — every one of those tables is rewritten in the same run.
