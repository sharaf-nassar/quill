...
Output is `hero.png` (widget on the Usage view), copied to `live.png` because the LIMITS band the `#live` section sells now shares that same 360px frame, plus `trends.png`, `analytics-charts.png` (Charts view, anchor `#analytics`), `models.png`, `analytics-context.png` (Context view, anchor `#context`), and one shot per Manage section (`sessions.png`, `learning.png`, `instances.png`, `settings.png`). `memory.png` and `brevity.png` stay manual — each is a sub-panel inside a section rather than a section. Every grab is tested for zero standard deviation and the run exits non-zero when any shot came back blank, so the GL-surface failure below is reported rather than silently published.
...
What is published today came from the 2026-07-31 refresh, which replaced every shot of the deleted split-pane main window: `hero.png` and `live.png` are the widget on its Usage view (LIMITS band, six-hour chart, readout grid, session breakdown), `analytics-charts.png` is the Charts view and `analytics-context.png` the Context view, each cropped to the last row of its own content so the widget's dead space below the fold is not shipped. Those four were derived from the 360px renders in `specs/018-widget-ui-redesign/verification/` rather than a live grab, because the capture host could not run the packaged build; the view switcher therefore still carries its focus ring in the Charts and Context shots. `og-image.png` is recomposed from them (logo mark beside the Usage frame with the Charts view behind), so no published asset shows the old two-pane UI. `sessions.png`, `learning.png`, `memory.png`, and `brevity.png` are still the 2026-06-24 captures of the pre-Manage standalone windows and are queued for recapture. The root `README.md` mirrors the widget set from `screenshots/widget-{usage,charts,context}.png`.

The 2026-06-24 refresh (predating the widget UI) had recaptured every section on its own at a slim window width (480px main / 500–560px secondary windows) and fully populated, then cropped each shot tight to its content, replacing the earlier dual-pane shots. Its main-window conventions are now history: `hero.png` was the Live pane stacked above the Analytics dashboard on its 7-day Now view (captured tall at ~453px wide with fresh timestamps so neither pane showed a Paused badge; the Claude limits list trimmed to 5h / 7d / Opus / Sonnet by deleting the `seven_day_cowork` (Code) and `seven_day_oauth_apps` (OAuth) `usage_snapshots` rows before the shot and re-seeding afterward), and `live.png` was the Live rate-limit pane alone. The secondary-window conventions still hold for the shots that survive: `sessions.png` (Session Search with a result detail open), `learning.png` (Learning panel — five active rules above three discovered candidates plus the domain breakdown, no empty section), `memory.png` (Memories panel showing a clean "All Projects (4)"), and `brevity.png` (Brevity toggle, MiniMax excluded from the visible copy); `analytics-context.png` still needs context preservation enabled with `context_savings_events` rows, which the seeder populates. Capture filenames map 1:1 to the marketing site's anchored sections per `specs/001-marketing-site/data-model.md`; the page then displays each at or below its native retina width (see [[infrastructure#CI/CD Pipeline#Marketing Site]]).
...
Each launcher creates a stable per-user sandbox directory (`/tmp/quill-demo-$USER` on POSIX, `%TEMP%\quill-demo-%USERNAME%` on Windows). `scripts/run_quill_demo.sh` and `scripts/run_quill_demo.ps1` pass the same Claude and Codex directories to the seeder as `--projects-dir` and `--codex-sessions-dir`, then export them as `QUILL_CLAUDE_PROJECTS_DIR` and `QUILL_CODEX_SESSIONS_DIR` with `QUILL_DEMO_MODE=1`. Runtime discovery therefore reads the exact roots whose canonical sources populated SQLite; neither production session root is read. Both also isolate data and rules, run with `--no-backup --quiet`, then auto-discover and execute Quill. PowerShell uses direct array splatting so spaced paths are preserved. See [[backend#Data Paths#Demo-mode path override]] and `specs/001-marketing-site/contracts/launcher-cli.md`.
...
Deployment stamps hash the bundled managed assets and enabled features. A changed continuity capture script therefore makes the installed asset stale; the next [[src-tauri/src/integrations/manager.rs#repair_provider]] reinstalls it and refreshes the stamp without a version bump.

On redeploy Quill first strips every prior Quill entry across all events, then deletes any event key left with an empty array so unrelated user hooks survive and no `"PreCompact": []` cruft accumulates. The `_source: "quill-setup"` / `quill-context-preservation` markers on each entry are advisory only — Claude Code drops unknown keys when it rewrites `settings.json`, so the canonical ownership and dedup signal is the Quill scripts-directory path substring carried in every registered command; the removal pass matches on that path in addition to the markers.

Claude always installs the qbuild guard PreToolUse hook (matcher `Edit|Write|NotebookEdit`, with `MultiEdit` removed) and the Stop hook for token reports and session sync, whose `report-tokens` command now carries an explicit `timeout: 5`. PreToolUse and PostToolUse `observe.cjs` hooks ride with the activity tracking flag (see [[features#Settings Window#Integration Features]]) so privacy-conscious users can keep token stats but skip live tool-call telemetry; `observe.cjs`'s post-phase `HIGH_SIGNAL_POST` gate likewise narrows to `Bash`, `Edit`, `Write`, and `NotebookEdit` (`MultiEdit` removed) while its registered matcher stays `*`. When context preservation is enabled, Claude additionally installs SessionStart, UserPromptSubmit, PreCompact, PreToolUse, and Stop context hooks. Context hooks call `src-tauri/claude-integration/scripts/context-router.cjs` and `src-tauri/claude-integration/scripts/context-capture.cjs`, both of which try to load `context-telemetry.cjs` for context-savings events when that flag is also on (and tolerate its absence otherwise). Startup repair in [[src-tauri/src/integrations/manager.rs]] reads the full `IntegrationFeatures` set and reinstalls every enabled provider with the merged feature set.
...
Codex install and uninstall remove only Quill-owned `hooks.json` entries — Quill now registers its Codex hooks inline in `config.toml`, so its own `hooks.json` entries are the legacy ones; the `hooks.json` format itself is still a current first-class Codex format, so non-Quill entries are left intact — and delete `hooks.json` when no non-Quill hooks remain, then remove related hook trust state, managed config blocks, AGENTS blocks, and provider-owned asset directories. Codex deploys the same bounded-wait observation and session-sync behavior as Claude so a slow local widget cannot hold Codex hooks open until the host kills them.

Codex installs SessionStart, UserPromptSubmit, and Stop hooks unconditionally; the unconditional SessionStart group now emits a `None` matcher instead of an empty string (`Some("")`) so runtimes that treat `""` as a literal still match every session start. PreToolUse and PostToolUse `observe.cjs` hooks ride with the activity tracking flag and register on the `Bash|apply_patch` matcher so patch-based edits are captured alongside shell commands. When context preservation is enabled, Codex also installs SessionStart (likewise `None`-matched), UserPromptSubmit, PreToolUse, PreCompact, and Stop context hooks. When activity tracking is enabled, the installer additionally registers `hook-observe.cjs` on every one of the ten Codex hook events (`PreToolUse`, `PostToolUse`, `SessionStart`, `UserPromptSubmit`, `SubagentStart`, `SubagentStop`, `Stop`, `PreCompact`, `PostCompact`, `PermissionRequest`) with no matcher, so the Now-tab Hooks breakdown sees every fire — Codex rollouts do not log hook executions, so this observer is the only source of Codex hook data. The installer asks `codex app-server` for `hooks/list` metadata, then writes each Quill hook's `trusted_hash` through `config/batchWrite` so the trust state matches Codex's own hook-review model.

Codex has shipped hooks default-enabled since Codex 0.124.0, so `features.hooks = true` is not what turns them on — Quill writes the flag only to re-enable hooks for a user who had explicitly set `hooks = false`. The deployed `.cjs` bridges resolve a stable session id through a `session_id || conversation_id || id` fallback chain (`hook-observe.cjs` falls back further to `""`; `session-sync.cjs` still requires both an id and a `transcript_path` before it syncs). `hook-observe.cjs` also forwards an optional `agent_id` on every payload for future per-subagent attribution, and `observe.cjs`'s internal gate now admits both `Bash` and `apply_patch` tool calls to mirror the widened `Bash|apply_patch` matcher.
