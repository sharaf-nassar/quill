...
The dev server also defends against `tauri dev`'s unreliable Ctrl+C cleanup (tauri-apps/tauri #10343): SIGINT fells the intermediate `npm`/`sh` links before the CLI's recursive kill walks its `beforeDevCommand` tree, so the Vite process reparents and survives, holding port 8181 against every later run. `vite.config.ts#tauriParentWatch` activates only when the tauri hook chain spawned the server (`TAURI_ENV_PLATFORM` is exported to hook commands) and polls `process.ppid` every two seconds; a ppid change is precisely the orphan's signature, so the server closes itself and exits. A manual `npm run dev` sets no `TAURI_ENV_*` and is untouched, and Vite's in-process config-change restarts keep the same ppid. See `docs/solutions/environment/second-vite-server-strips-dev-css.md`.

Enforcement is inverted rather than allowlisted, because naming offenders one at a time (first the test runner, then ad-hoc scripts, then screenshot servers) kept regressing. `vite.config.ts#resolveCacheDir` grants the shared `node_modules/.vite` only to the bare `vite` CLI on its configured strict port — the process `npm run dev` and `tauri dev`'s `beforeDevCommand` spawn, recognized by a `process.argv[1]` ending in `vite/bin/vite.js` or the `.bin/vite` shim with backslashes normalized, so no shell prefix is needed on Windows. Every other invocation — the eleven test files calling `createServer()`, `scripts/web-bundle-isolation.test.mjs` calling `build()`, one-off scripts, and any CLI run carrying a `--port` override — lands in a private `<tmpdir>/quill-vite-<pid>` cache. `strictPort` makes a second no-override CLI die at bind, so no process can hold the shared cache against the dev server. The temp dir rather than another `node_modules` folder because these servers set `optimizeDeps.noDiscovery` and never commit a `deps/`, leaving roughly 39MB of temporaries per run that nothing reads twice; misclassification fails safe as an extra re-optimize, never a poisoned dev cache.
...
The hero is product-led and two-column on desktop: problem-first copy and install/source actions sit beside the whole 480×800 marketing capture of the freely resizable Usage widget. Its trust line states local data, no Quill account, and opt-out scrubbed crash reports. Under 980px it collapses to one centered column. The current marketing composition spotlights Claude Code, Codex, and Pi; MiniMax remains supported in-product but is omitted from these screenshots and lead copy.

The narrative is Usage, Models, Context, Search, Live, Learning, Memory, Brevity, Integrations, then install/trust. Widget assets are 960×1600 retina captures displayed at 480px so session identities stay readable; Tools assets are 1920×1360. Every image is shown whole except `#live`, whose `.shot-band` clips the shared Usage frame to LIMITS. Copy states evidence gaps, review-gated rule promotion, provider scope, and crash-report opt-out.
...
`scripts/capture_screenshots_docker.sh` is the publishing entry point: it runs Quill's real React frontend in Browser Mock Mode inside a network-disabled Chromium container, then replaces canonical PNGs only after every capture succeeds.

`Dockerfile.screenshots` contains Node, Chromium, the frontend sources, and the existing mock fixtures. It does not build Rust, launch Tauri, create a database, mount the host display, or read personal state. Vite serves the same `src/main.tsx` entry used by the desktop webview; because the page is dev/non-Tauri, [[frontend#Frontend#Browser Mock Mode]] installs `@tauri-apps/api/mocks` underneath the normal `invoke()` call sites. Production bundles cannot enter this path.

`scripts/capture_browser_screenshots.mjs` launches Vite and headless Chromium, then drives the rendered app through Chrome DevTools Protocol. It sets 480×800 or 960×680 CSS viewports at DPR 2, clicks the real view/range/section/tab controls, types the Session Search query, selects a result, scrolls Context settings, and captures from the browser surface. `?screenshot=marketing` removes the dev badge and selects the Claude/Codex/Pi marketing fixture profile without adding screenshot-only branches to production behavior.

Output is `hero.png` plus its exact `live.png` copy, `models.png`, `analytics-context.png`, `sessions.png`, `learning.png`, `memory.png`, `settings.png`, and `brevity.png`. Usage is 6H with Model grouping and live Claude/Pi/Codex agent rows; Models is 7D; Context is 6H; Sessions has three provider hits plus selected context; Memories has four projects; Integrations excludes MiniMax; Brevity is on. Widget PNGs must be 960×1600 and Tools PNGs 1920×1360. The marketing fixture disables the retention watermark and uses a Pi-routed Claude model, so no pruning disclosure or Gemini identity enters the Usage shot.

The Node driver validates PNG headers, dimensions, and the hero/live identity before it exits. The shell wrapper creates the runtime container with `--network none`, copies output into a host temporary directory, checks all nine files again, and only then installs them under `marketing-site/assets/screenshots/`. The README and marketing site share those canonical files; no mirrors are maintained.

`scripts/take_screenshots.sh`, [[infrastructure#Scripts#Demo Launcher]], and [[infrastructure#Scripts#Dummy Data Seeder]] remain lower-level backend/Tauri debugging tools. They are not the marketing screenshot source because their low-level database fixtures can drift from the frontend's maintained Browser Mock Mode and host X11 automation can steal focus.
...
Both the version probe and async OAuth token export use the host-command environment described in [[infrastructure#Infrastructure#Provider CLI Detection]]. Linux AppImage library overrides cannot leak into Pi's Node runtime; Quill's own bundled-library environment remains unchanged.
...
Tool requests use the main local URL for session history and the separate context origin for `/api/v1/context/*`. Both require exact loopback hostnames. Each tool carries its own time budget: `quill_execute` waits the requested `timeout_ms` (default 30 s, capped at the 120 s schema maximum) plus a 5 s reaping margin, `quill_fetch_and_index` waits 35 s to cover the server's 30 s remote hop, and history plus local context reads get a 10 s tool budget (FTS over a large store outgrew the 1500 ms lifecycle budget, which telemetry keeps); Pi's own abort signal is joined to every budget so cancellation stays immediate. A tool failure names its cause in bounded text under the one `quill_unavailable` error type: the time budget, the context-preservation gate on `403`, the server's own `error` reason on any other non-2xx that carries one (whitespace-collapsed and capped at 300 characters, so an upstream `404 Not Found` on a guessed URL reads as a rejection to correct rather than a dead Quill), or an unreachable Quill when no reason is available. History requests select the server's compact view and re-compact older oversized responses under the same 32 KiB ceiling. Every successful tool keeps its payload only in text content and returns `{ ok: true }` details so Pi does not persist a duplicate copy. Hook telemetry uses tool-execution start/end as its sole `PreToolUse`/`PostToolUse` pair, root settlement as `Stop`, and configured-child agent boundaries exclusively as `SubagentStart`/`SubagentStop`. Hook and routing requests carry lifecycle identity metadata; non-2xx and unavailable-server failures never change Pi behavior and log only under `QUILL_DEBUG`. Context telemetry remains dependent on context preservation and posts Pi routing events with all routing token estimates set to zero.
...
Host commands use [[src-tauri/src/config.rs#external_command]]. On Linux with `APPIMAGE` set, it removes `LD_LIBRARY_PATH` from the child only: bundled libraries can be incompatible with host Node, so inheriting the AppImage override can break even `pi --version`. This covers login-shell and package-manager lookup, provider probes and authentication, Codex app-server, MCP verification, Claude inference, and host utilities. Tokio commands retain the same environment through conversion from the standard command; sandbox wrappers replay its removals. Ordinary launches preserve the caller's library path. Quill's environment and updater relaunch remain untouched; context execution already clears its environment independently. [[provider-cli-tests]] pins the child boundary.
