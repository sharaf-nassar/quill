...
The seeded dataset needs grooming before it photographs well, and each item is a seeder gap rather than a capture problem — see [[infrastructure#Scripts#Dummy Data Seeder]]. `populate_dummy_data.py` stamps `schema_version` past migration 20 but creates `token_snapshots` without that migration's `is_sidechain`/`agent_id`/`parent_uuid` columns, so `get_session_breakdown` fails and the Usage view reads "Breakdown unavailable" until the columns are added by hand. It never writes `tool_actions.lines_added`/`lines_removed`, so TOK/LOC, LOC/HR and NET LINES read empty and the Charts view's CODE timeline reads "No code changes in this range". It seeds only `claude` token snapshots, so the chart draws one series and every session row carries the same badge. It emits six Claude limit buckets, two more than the 360px LIMITS row fits — the mockup's composition is three. It seeds MiniMax disabled, so the SETUP row the `#live` copy describes never renders. And it writes learned-rule `.md` files flat in the rules root while demo mode looks for them under `<rules>/{claude,codex,shared}`, so no rule is ever discovered on disk and the Learning section shows an empty ACTIVE RULES block.

The compositions each shot has to reach survived the window rewrite even though the windows did not: `sessions.png` wants a query typed and a result detail open, `learning.png` wants active rules above discovered candidates with no empty section, `memory.png` wants a clean "All Projects (4)" with one file per project, and `brevity.png` wants the toggle on with its explanation visible. Reaching them needs four manual steps the script does not take — typing a query into Session Search, opening the Learning section's Memories tab, opening Settings → Context, and removing the global `~/.claude/CLAUDE.md` that `claude_setup` recreates on every launch so the Memories count stays at four projects. `analytics-context.png` needs context preservation enabled with `context_savings_events` rows, which the seeder populates. The seeder's `PROJECTS` deliberately use single-segment paths (e.g. `/home/alex/gateway`, never `/home/alex/api-gateway`) because the panel's slug↔path round-trip ([[src-tauri/src/memory_optimizer.rs#project_path_to_slug]] encodes `/`→`-`, [[src-tauri/src/memory_optimizer.rs#slug_to_path]] decodes `-`→`/`) cannot tell an internal dash from a path separator; single-segment names keep the round-trip unambiguous so the panel shows clean, de-duplicated project headings. Capture filenames map 1:1 to the marketing site's anchored sections per `specs/001-marketing-site/data-model.md`; the page then displays each at or below its native retina width (see [[infrastructure#CI/CD Pipeline#Marketing Site]]).

The memory files the Memories panel lists are not seeded at all: [[src-tauri/src/memory_optimizer.rs#memory_dir]] resolves them under `dirs::home_dir()` rather than through `QUILL_CLAUDE_PROJECTS_DIR`, so they must be written into the isolated `HOME` as `$HOME/.claude/projects/<slug>/memory/<name>.md` with `type:` and `description:` frontmatter — one per seeded project, which is what produces the CONTEXT / CONVENTION badges and the one-line descriptions. The panel's "All Projects (N)" counts every per-project context file, which includes the global `~/.claude/CLAUDE.md` that `claude_setup` recreates on each launch (counted once per project — see [[src-tauri/src/memory_optimizer.rs#get_known_projects]]); to capture a clean "All Projects (4)" that file is removed after launch but before the Memories panel is first opened, since the panel counts on window mount and `claude_setup` does not run again until the next launch. Folding the isolated-`HOME` setup, the memory files, and the dataset grooming above into `run_quill_demo.sh` and the seeder — so the published shots are reproducible with one command — is still pending.
