...
Hook fires are durable SQLite audit rows and nothing else: they feed the Hooks breakdown, and live session and agent state comes from [[data-flow#Data Flow#Transcript-Derived Session Snapshots|the transcript scanner]] instead.

1. Claude transcript attachments provide its audit history, so no Claude hook posts to this endpoint. Under `activity_tracking` its `observe.cjs` posts tool-phase learning observations only.
2. Codex rollouts log no hook executions, so its generic observer is registered on the seven observed events (`PreToolUse`, `PermissionRequest`, `PostToolUse`, `PreCompact`, `PostCompact`, `UserPromptSubmit`, `Stop`) under the same gate, carrying hostname, session fallbacks, optional agent id, and event-scoped audit identity.
...
5. [[src/hooks/useBreakdownData.ts#useBreakdownData]] declares `hooks-observed-updated` for Sessions and Hooks. Mounted keys join the shared five-second fan-out. Sessions IPC merges exact retained rows, advances their activity from the current scan, and synthesizes range-scoped active roots before their first token snapshot without scanning audit history.

Session and subagent lifecycle events are deliberately never registered on either provider. Transcripts already carry that evidence retroactively and exactly, so registering them would deploy hooks into user config to observe something Quill can read off disk — including for sessions that started before Quill launched.
...
The main window polls enabled native providers and configured CPA for live rate-limit status, then stores source-aware results in shared usage tables.
