...
Session-start events also upsert durable cwd, hostname, and migration-42 `ephemeral` state into `live_analytics_sessions`. An accepted envelope atomically refreshes `pi_extension.{last_seen,protocol,extension_version,min_quill_version,last_error}` settings, but an unchanged handshake refreshes at most once per [[src-tauri/src/storage.rs#PI_EXTENSION_HEALTH_REFRESH]] window: rewriting five settings rows per envelope serialized a whole Pi fleet behind bookkeeping on the one primary connection, and provider status only resolves liveness at two- and fifteen-minute boundaries. Any changed handshake field still writes through immediately. Provider-status IPC derives never-connected, alive, idle, or stale health and a typed error from those rows. Protocol mismatch records typed health before returning `400`, while bad auth records nothing.

[[src-tauri/src/server.rs#start_server]] starts an immediate and 15-second Pi spool drain. It reads bounded snapshots of live-PID files without removing them, rename-claims dead files, and feeds each record through the same tracking or runtime validation and storage functions as HTTP ingest. UUID indexes make repeated live-file reads no-ops. File, directory, and age caps plus corrupt lines persist typed spool-gap health that later handshakes cannot clear; only dead or already claimed files are deleted.

Because a live writer's file is never removed, [[src-tauri/src/server.rs#PiSpoolOffsets]] remembers the bytes each file has already contributed and the next pass resumes there. Without it every pass re-read each live file from byte zero and spent the per-pass record budget re-ingesting records already stored, so the budget was exhausted on the oldest files and the newest never drained — and a freshly spawned sub-agent sorts last by mtime, so the `session_start` carrying its agent lineage was the record most likely to be starved. A throttled record rewinds the offset to its own start, a shorter file than its offset restarts from zero so a recycled pid cannot skip records, and offsets for files the enumeration no longer lists are dropped.

[[src-tauri/src/live_tracker.rs#LiveTracker#apply_paths]] folds the Claude and Codex transcripts the filesystem watcher reports. Each file carries the byte offset already consumed, so steady state parses only appended bytes; a trailing line without its newline is left for the next fold.

[[src-tauri/src/live_tracker.rs#LiveTracker#sweep]] is the Claude/Codex cold start, watcher-overflow recovery, periodic backstop, and pushed-session crash fallback. It uses [[src-tauri/src/sessions.rs#discover_claude_transcripts_in]] and [[src-tauri/src/sessions.rs#discover_codex_transcripts_in]], then releases any session silent past [[src-tauri/src/live_tracker.rs#IDLE_AFTER]]. Every non-terminal Pi push advances the same last-event clock. [[src-tauri/src/live_tracker.rs#modified_within_idle_window]] gates Claude transcript reads on modification time; Codex still indexes quiet ancestors before applying the same parse gate.
...
[[src-tauri/src/transcript_watcher.rs#admit_pending]] feeds Claude/Codex transcript evidence; `/api/v1/pi/track` is the independent pushed feed. Watcher batches go to the fold alongside retained ingest, while Pi pushes mutate the same map after authenticated validation and durable lifecycle persistence. Startup manages the tracker before either feed starts and applies saved activity-tracking and per-provider switches, so a disabled provider is never folded or push-created.
...
Pi live state and usage come only from authenticated extension events; transcripts remain a narrow Session Search input named by notify.

Lifecycle events create, continue, replace, or close stable session identities. Activity, model, lineage, and usage events mutate only an existing pushed identity, and the shared idle cutoff remains the crash backstop when shutdown never arrives.

Runtime messages arrive one per request, so a turn's `user_text` and the `asst_text` answering it never share a batch and a call-local pending slot can never pair them — Pi recorded no `response_times` at all until [[src-tauri/src/storage.rs#Storage#store_live_session_analytics]] began recovering the prompt from its own persisted `user_text` event, the newest in the chain that no completed turn has consumed. The recovered prompt is claimed once per batch, so the many `asst_text` events inside one turn still yield exactly one turn row, matching how the transcript providers count turns. A prompt in the batch supersedes an older recovered one, and the existing `response_secs IS NOT NULL` bookmark keeps replay from inventing turns.

The extension calls Session Search notify only when Pi supplies a transcript path and that file is already on disk. Pi names the session file at start but writes it only once the first assistant message lands, so notifying at start would name a path that does not exist yet and Pi has no unvalidated search fallback to absorb it; the turn-end repeat delivers the same identity and lineage once the transcript exists. Notify validation uses [[src-tauri/src/pi_session.rs#read_pi_session_header]] as a bounded v2/v3 probe, then indexing parses the named file. Ephemeral starts have no path, so they reach lifecycle, usage, and runtime analytics but never enter search.
...
Pi usage events insert once through [[src-tauri/src/storage.rs#Storage#store_pi_model_usage]], use an upstream `provider/model` id, store native input/output/cache-read/cache-write/total cost without exposing it, and share the stable `live:pi:<session-id>` owner with pushed runtime evidence. Lifecycle timestamps, not transcript paths, keep the owner active through shutdown. Each accepted runtime event advances compact open-turn state and adds only newly closed hourly turns; exceptional late ordering refolds retained source rows for exact replay. Confirmed retention can prune processed prefixes without losing the logical open turn. Migration 44 moves earlier source-less runtime and `pi-push:` model rows to this owner without changing record identity; migration 45 compacts existing live open turns. Event UUIDs deduplicate live/spool replay, and insert-time watermark filtering covers active and closed runtime, response, and usage evidence so late replay cannot restore pruned raw rows. Saved transcript-era Pi observations remain a disjoint pre-upgrade branch, so the union keeps current totals. Sessions and model history report spend truth with [[src-tauri/src/models.rs#ModelTokenScope]] fixed to `all-branches`. Test coverage lives in [[pi-model-usage-tests#Pi Model Usage Test Specs]].
...
8. Retained runtime, response, tool, skill, and hook rows persist in provider-aware SQLite snapshots owned by canonical transcript source. Pi runtime and model pushes use one lifecycle-owned session source, while other remote message and hook pushes keep source-less identities.
...
[[src-tauri/src/transcript_identity.rs#resolve_codex_native_identity]] preserves the first Codex child identity while accepting consistent ancestor restatements and rejecting conflicting parents or cycles. [[src-tauri/src/transcript_analytics.rs#resolve_claude_native_identity]] skips anomalous Claude records — a sidechain record restated in a parent file, or a record copied across a fork — counting them into `TranscriptRecordDiagnostics` instead of rejecting the source; only a source with no valid identity at all fails. Retained transcript analytics inventory admits only Claude and Codex; Pi uses notify-only search parsing and never reaches retained analytics identity resolution. That left `tool_actions` and `skill_usages` with no Pi producer at all, because the pushed live path deliberately carries no tool payloads, so [[src-tauri/src/server.rs#index_session_notify_payload]] persists both from the notify parse it already performs. [[src-tauri/src/transcript_analytics.rs#owned_tool_rows]] is the one builder both owners use: each supplies an `OwnedToolRowIdentity` — the retained parser its native chain identity, [[src-tauri/src/sessions.rs#pi_transcript_tool_rows]] Pi's flat one — so the action key and skill fan-out cannot drift apart and both paths dedupe on the same unique indexes. [[src-tauri/src/storage.rs#Storage#replace_pi_transcript_tool_rows]] replaces exactly those two tables under the live source key. It leaves `session_events` and `response_times` alone: they share that key but are owned by the pushed path, which sees ephemeral sessions and in-flight turns no transcript restates. A persistence failure is logged rather than raised, so it cannot cost the session its search documents. The dependency does not run the other way either: a Pi notify is admitted and drained even when the Tantivy index fails to open, because an unopenable index would otherwise silently take Pi's only tool and skill producer down with Session Search. Claude and Codex still short-circuit, having nothing left to do without an index. [[src-tauri/src/transcript_analytics.rs#parse_transcript_analytics_source]] performs a bounded stable read, decodes JSONL once with original line ordinals, and emits source-owned runtime, response, tool, skill, and hook rows. Response and idle pairs are computed only within that source and chain. [[src-tauri/src/transcript_analytics.rs#stamp_analytics_root]] validates native attribution before applying the cross-source resolved root.
