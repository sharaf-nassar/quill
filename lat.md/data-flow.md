...
[[src-tauri/src/live_tracker.rs#LiveTracker]] owns transcript-folded and Pi-pushed live session state, so a Sessions read costs a map lock instead of a directory walk. It is the sole owner of liveness and open-agent membership, so no read-time reconciler remains.
...
[[src-tauri/src/live_tracker.rs#LiveTracker#sweep]] is the cold start, watcher-overflow recovery, periodic backstop, and pushed-session crash fallback. It uses [[src-tauri/src/sessions.rs#discover_claude_transcripts_in]], [[src-tauri/src/sessions.rs#discover_codex_transcripts_in]], and [[src-tauri/src/sessions.rs#discover_pi_transcripts_in]], then releases transcript or pushed sessions silent past [[src-tauri/src/live_tracker.rs#IDLE_AFTER]]. Every non-terminal Pi push advances the same last-event clock. [[src-tauri/src/live_tracker.rs#modified_within_idle_window]] gates Claude and Pi transcript reads on modification time; Codex still indexes quiet ancestors before applying the same parse gate.
...
[[src-tauri/src/transcript_watcher.rs#admit_pending]] feeds transcript evidence; `/api/v1/pi/track` is the independent pushed feed. The drained admission batch goes to the fold alongside retained ingest, while Pi pushes mutate the same map after authenticated validation and durable lifecycle persistence. The watcher thread still supplies cold-start and recovery sweeps. Startup manages the tracker before either feed starts and applies saved activity-tracking and per-provider switches at construction, so a disabled provider is never folded or push-created.
...
[[src-tauri/src/model_usage.rs#parse_pi_model_usage_jsonl]] temporarily retains pre-upgrade v2/v3 assistant usage. New usage events insert once through [[src-tauri/src/storage.rs#Storage#store_pi_model_usage]], use an upstream `provider/model` id, store native input/output/cache-read/cache-write/total cost without exposing it, and own a synthetic `pi-push:<session-id>` source. Raw and completed-rollup Models reads include that source; once it exists, their shared active-source predicate excludes legacy adapter sources for the same session. Sessions and model history therefore report all-branch spend truth with [[src-tauri/src/models.rs#ModelTokenScope]] fixed to `all-branches`. Test coverage lives in [[pi-model-usage-tests#Pi Model Usage Test Specs]].
