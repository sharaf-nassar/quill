...
2. Every retained-worker pass and the `sync_search_index` command run [[src-tauri/src/sessions.rs#SessionIndex#sync]], which re-extracts only sources whose nanosecond mtime or size moved and prunes documents of sources a completely enumerated root no longer contains. `index_state.json` is keyed by canonical source key, the identity analytics and the live coordinator share, and remembers each successful source fingerprint plus its native session id. A separate rejection map records stable invalid versions without overwriting last-good documents or checkpoints. Missing native identity, including an empty Codex rollout, is rejected until bytes change; it is never a successful identity-free extraction. Legacy missing-path checkpoints still require revalidation. The sweep consults no migration re-ingest markers: derived analytics rows belong to retained reconciliation, and a `SCHEMA_VERSION` bump on the index directory is the one way to force a full re-extract
...
12. Result context resolves a hit's transcript through [[src-tauri/src/sessions.rs#find_session_path]]: the analytics source registry answers every provider with one indexed query, an exact chain or source-session match outranks a resolved-root match so a sub-agent chain opens its own file, and the path is re-validated against the configured root before it is read. A Claude parent that predates the registry still resolves by its `<session-id>.jsonl` filename; nothing walks or parses the corpus to find a session.
