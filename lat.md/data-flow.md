...
Pi live state remains tail-provable and bounded; active-branch reconstruction stays outside this fold.
...
[[src-tauri/src/model_usage.rs#parse_pi_model_usage_jsonl]] routes v2 and v3 transcripts through the shared Pi parser and emits only assistant-message usage. Each row uses Quill provider `pi`, an upstream `provider/model` id, and header-id plus entry-id dedupe; Pi cost data is ignored. [[src-tauri/src/transcript_identity.rs#resolve_pi_native_identity]] anchors the source on its header. Overview and rollup totals keep every tree branch because Pi stores shared ancestors by reference. Session reads retain only the parent chain of the last parsed entry and expose [[src-tauri/src/models.rs#ModelTokenScope]] as `active-branch`; they never label that number as an all-branch total. Test coverage lives in [[pi-model-usage-tests#Pi Model Usage Test Specs]].
