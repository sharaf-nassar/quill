...
Startup reconciliation allocates durable per-root generations, resolves each provider-qualified native chain independently, and resumes the migration-armed historical rebuild until all available roots finish. While the reingest marker is set, `force_full_reparse` bypasses the mtime/size fast path, the content-digest short-circuit, and the content-deterministic-failure short-circuit; suppression is honoured regardless and never bypassed. Unchanged sources owe nothing at all: a whole-root pass writes only for the sources it replaced or newly failed. Unrelated sessions sharing one provider directory retain distinct roots, and Tantivy indexing performs no analytics writes.
...
Live coverage no longer depends solely on the per-session `/sessions/notify` hook. [[src-tauri/src/transcript_watcher.rs#start]] drops non-rescan `EventKind::Access` events before the bounded callback channel, coalesces relevant JSONL mutations for at most one second, then reuses strict source validation and the canonical live queue. Remove, rename, notify-rescan, and bounded-buffer overflow signals request whole-root graph reconciliation and pruning. The first recovery after quiet runs promptly; recovery storms inside the 30-second cooldown still run Session Search and leave one deferred reconciliation armed until timeout. Each retained-worker pass enumerates roots once and shares that inventory between Session Search and whole-root analytics reconciliation. The independent 120-second rescan loop ([[src-tauri/src/lib.rs#spawn_transcript_rescan_loop]], see [[architecture#Background Tasks]]) remains the changed-source admission backstop. Whole-root unchanged sources write nothing; live unchanged notifications perform their single registry refresh and return `SuppressedUnchanged`.
