...
Hook fires are durable SQLite audit rows; the newest root terminal hook is also advisory negative evidence, while positive session and agent state still comes from [[data-flow#Data Flow#Transcript-Derived Session Snapshots|the transcript scanner]].
...
3. `POST /api/v1/hooks/observed` validates the provider-neutral payload — supported event, length caps, ISO-8601 timestamp — and rejects anything else with `400`. It holds no live state, so a rejection costs nothing beyond the dropped audit row.
...
[[src-tauri/src/live_tracker.rs#LiveTracker#sweep]] is the cold start, the watcher-overflow recovery, and the periodic backstop for missed filesystem events. It walks both provider roots through [[src-tauri/src/sessions.rs#discover_claude_transcripts_in]] and [[src-tauri/src/sessions.rs#discover_codex_transcripts_in]] — the same walkers retained inventory uses — folds whatever moved, and releases sessions silent past [[src-tauri/src/live_tracker.rs#IDLE_AFTER]] along with the file offsets they owned. [[src-tauri/src/live_tracker.rs#modified_within_idle_window]] gates the walk on modification time first: eviction drops an idle session's offsets, so folding a transcript untouched past the cutoff would re-read it from byte zero — thousands of files per pass on a real corpus — to rebuild a session the same sweep evicts again. A Codex rollout passes the walk whatever its age, because a quiet one still belongs in the thread index, and meets the same gate before it is parsed. Disabling activity tracking or one provider clears the state it covers, and the next sweep rebuilds it.
...
[[src-tauri/src/live_tracker.rs#TrackerState#codex_file]] keys every enumerated rollout by [[src-tauri/src/sessions.rs#codex_thread_id]] into an incremental thread index. The index covers the whole corpus rather than the live window, because a live spawn routinely names an ancestor that has been quiet for hours, and a batch is indexed in full before any of it is resolved so a child folded ahead of its parent still finds a chain to walk. [[src-tauri/src/transcript_scan.rs#codex_head]] then parses `session_meta` once per thread through [[src-tauri/src/transcript_identity.rs#codex_metadata]] — the parser retained ingest uses — and [[src-tauri/src/transcript_scan.rs#codex_root]] walks `parent_thread_id` / `forked_from_id` to the user thread that owns the spawn. Both answers are permanent state, so every later event on that rollout costs map lookups. A rollout nothing has written to for a whole [[src-tauri/src/live_tracker.rs#IDLE_AFTER]] window is indexed but not opened: it can hold neither a live session nor an open agent, and cold-parsing the whole corpus instead would cost gigabytes of bounded reads at startup.

First sight of a rollout, and any rewrite of one, parses from a bounded tail through [[src-tauri/src/transcript_scan.rs#read_codex_tail]] rather than from the head — a spawned rollout reaches hundreds of megabytes. Thereafter only appended bytes are parsed. A root rollout's head supplies the session's origin and project and the floor its activity falls back to, so a rewrite clears what the replaced file had contributed and its own tail answers instead.

[[src-tauri/src/transcript_scan.rs#codex_turn_boundary]] flips a sub-agent's open bit: the newest `task_started` / `task_complete` / `turn_aborted` in its own rollout, never a count, because a sub-agent thread outlives one turn and starts exceed completes plus aborts in most multi-turn rollouts. A tail window holding no boundary is itself the answer — a rollout only accumulates records inside a turn — so a cold start seeds the bit from whether the window reached the start of the file. Silence past the idle window still closes an agent whose turn never ended, and the boundary's own timestamp feeds that clock because a turn can pass without a substantive record. [[src-tauri/src/transcript_scan.rs#codex_activity_timestamp]] advances session activity from substantive user, assistant, reasoning, and tool content only, so post-Stop lifecycle, token bookkeeping, empty items, and the file's mtime cannot reopen a finished session. The model is the first one a rollout's `turn_context` records name, read by [[src-tauri/src/transcript_scan.rs#read_codex_model]] from the head parse's own handle.

## Transcript-Derived Session Snapshots

[[src-tauri/src/transcript_scan.rs#TranscriptScanner]] derived live Claude and Codex session and agent state by re-scanning transcripts on the read path. The tracker above has taken that path over.

The scanner and its reconciler are retired code kept only until the deletion pass removes them, and the evidence rules recorded here are the ones the fold now applies incrementally.

Each pass runs two stages. Stage one calls [[src-tauri/src/sessions.rs#discover_claude_transcripts_in]] — the same walker retained inventory uses, so the project tree is never traversed a second way — stats every enumerated file, groups them by root session through [[src-tauri/src/sessions.rs#claude_root_session_id]], and keeps only sessions whose newest byte landed inside the 15-minute idle window. That helper lives beside the walker so retained inventory and live scanning resolve a transcript's root by the same rule instead of restating it. Stage two parses those sessions, and because transcripts are append-only it holds a byte offset per file and reads only the tail; a trailing line without its newline is left for the next pass. Activity uses timestamped transcript content rather than file mtime and excludes `attachment` records, because Claude writes hook results after SessionEnd and that bookkeeping is not a reopen.
...
[[src-tauri/src/server.rs#ObservedSubagentState#refresh_from_transcripts]] ran the pass on the blocking thread `get_session_breakdown` owns, immediately before the merge, and no longer runs at all — the read holds a map lock instead. Stage one was the budget it paid: 50ms of directory walk plus 10ms of stat over 38 projects / 3640 files for Claude and 29ms over 5502 rollouts for Codex, with steady-state parsing near zero, so one throttle covers both providers at one pass per three seconds and a burst of widget reads shares a single walk each. Snapshots record no session end; normal terminal timing is projected separately from root terminal hooks, while sessions missing one age out through the staleness cutoff.

### Codex Rollout Snapshots

Codex records no spawn/result pair, but a corpus measurement found equivalent exact evidence elsewhere: every sub-agent gets its own rollout, and that rollout's own turn records say whether it is working.

Stage one calls [[src-tauri/src/sessions.rs#discover_codex_transcripts_in]] — again the retained inventory walker — and keys every rollout by [[src-tauri/src/sessions.rs#codex_thread_id]], the trailing uuid of its filename, which restates the file's own `session_meta` id on 5501 of 5502 rollouts measured (the odd one out has an unreadable head). Retained inventory resolves rollout ids through that same helper, and it rejects a malformed name rather than returning a truncated id. The id is only a locator; identity always comes from the head record, parsed by [[src-tauri/src/transcript_identity.rs#codex_metadata]], the same parser retained ingest uses, so which field names the spawning parent is decided in one place. Rollouts outside the idle window are skipped, so only fresh files and the ancestors they name are ever opened.

The root snapshot initializes activity from its bounded rollout tail, then parses only appended bytes. Timestamped user, nonempty assistant, reasoning, and valid tool records advance it; filesystem mtime, lifecycle, context, malformed empty items, and token bookkeeping cannot override a Stop. Truncation clears cached activity before the replacement is parsed.

[[src-tauri/src/transcript_scan.rs#codex_root]] walks `parent_thread_id` / `forked_from_id` to the user thread that owns a spawn. Nesting is real and bounded: measured depths run 1–3 across 4487 spawned rollouts, and every one resolves to a root the same corpus still holds. The hop count bounds the walk but is not reported — which root a sub-agent belongs to is observable, how many hops away it sits is not. Forked rollouts are never a separate population — no rollout carries `forked_from_id` without also carrying a spawn marker — so the fork field is a parent pointer, not a second concept.

[[src-tauri/src/transcript_scan.rs#codex_agent_running]] reads the newest `task_started` / `task_complete` / `turn_aborted` event in the rollout's last mebibyte, scanning backwards because a forward pass would cost the whole rollout (269MB across ten live sub-agents) to learn one bit. It is the newest boundary and never a count: a sub-agent thread outlives one turn — the parent re-triggers it, and measured turns per rollout reach 138 — and starts exceed completes plus aborts in most multi-turn rollouts, so counting would leave a permanent imbalance where the last boundary stays exact. Of 4487 spawned rollouts, 4448 end closed; the 39 left open all died mid-turn months ago and the idle window catches them. `inter_agent_communication_metadata` was rejected as a lifecycle source: its payload is only `{trigger_turn}`, carrying no agent identity.

[[src-tauri/src/transcript_scan.rs#read_codex_model]] takes the model from the first `turn_context` record naming one, continuing forward from the head parse's own open handle under the same mebibyte budget the turn tail spends. `turn_context` is head-clustered rather than tail-resident — a spawned rollout replays its parent's history before emitting its own turn records, so the tail of a long turn holds none — and across 900 sampled spawned rollouts every one names a model, at p50 line 6 / 94KiB and p90 line 8 / 131KiB, with 97% inside the window. Taking the first accepts that a spawned rollout's opening `turn_context` may be its parent's: no measured thread's own model differed from the first its file names, and 591 of 600 name one model for their whole life. Codex has no such thing as a low-cardinality agent type to fall back on instead — current rollouts carry `agent_nickname` (255 distinct) and `agent_path` (2880 distinct over 2973 rollouts) but no `agent_role` at all — so without this read an agent has no label until retained ingest arrives.
...
[[src-tauri/src/lib.rs#enqueue_retained_live_source]] admits validated notifications to one coordinator keyed by provider and canonical source key. Each entry tracks independent model and transcript pending, running revision, failure count, and ready time. A newer notification rearms both domains without letting an older completion clear it; one domain can finish or back off while its sibling and healthy sources continue. [[src-tauri/src/lib.rs#drain_model_usage_live_queue]] still owns the model permit and commit events.

[[src-tauri/src/lib.rs#spawn_startup_model_source_reconciliation]] re-admits the full retained inventory to the model side after runner state initializes, including when the one-time backfill is already complete. This recovers sources retained while Quill was stopped without requiring Session Search to open; startup, notify, and rescan admissions converge on the same canonical source entry.
