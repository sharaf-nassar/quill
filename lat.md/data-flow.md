...
3. The HTTP handler in [[src-tauri/src/server.rs]] validates [[src-tauri/src/integrations/codex.rs#is_supported_hook_event|the shared 11-event lifecycle set]] plus length caps and ISO-8601 timestamp shape, fast-acks `202 Accepted`, and dispatches the insert via [[src-tauri/src/storage.rs#Storage#store_codex_hook_observation]] on a `tokio::task::spawn_blocking`. Codex inserts and each rollup chunk share a narrow gate before the maintenance-aware ingest permit, so a hook arriving during a chunk waits for its transaction to commit instead of contending for SQLite. Codex identity is event-scoped (`hook_event` plus optional `:tool_name`) because the observer fires per-event, not per-script.
