...
A fork or clone copies the parent's `quill-tracking` entries into the child file. Persisted evidence takes only the entries naming the child's own header id, so the parent's lifecycle stays with the parent's file and the child is not rejected for carrying it.
...
2. Every retained-worker pass and the `sync_search_index` command run [[src-tauri/src/sessions.rs#SessionIndex#sync]], which re-extracts only sources whose nanosecond mtime or size moved and prunes documents of sources a completely enumerated root no longer contains. `index_state.json` is keyed by canonical source key, the identity analytics and the live coordinator share, and remembers each source's fingerprint plus the session id its last extraction produced; a source whose identity never resolved is fingerprinted with an empty id, so it is retried only when its bytes change and never re-extracts the corpus. The sweep consults no migration re-ingest markers: derived analytics rows belong to retained reconciliation, and a `SCHEMA_VERSION` bump on the index directory is the one way to force a full re-extract
