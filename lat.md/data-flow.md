...
Hook-reported tokens still flow into `token_snapshots` keyed by the parent `session_id` — Claude sub-agents share the parent's session id on disk, so each row also carries `is_sidechain`/`agent_id`/`parent_uuid` from migration 20. The [[backend#Tauri IPC Commands#Usage and Token Commands (15)]] `get_session_breakdown` rollup aggregates parent and sub-agent rows at query time so a sub-agent's tokens count toward the parent session's totals, and `get_llm_runtime_stats(scope = "parent_only")` is available when the widget runtime readout needs to exclude the sub-agent traffic instead.
...
2. Claude polling in [[src-tauri/src/fetcher.rs]] calls the Anthropic API with the local OAuth Bearer token and parses Claude bucket keys; when Pi is enabled and that native path fails, it obtains a short-lived Anthropic bearer from Pi's CLI credential boundary and retries without ever reading Pi's refresh token
