...
3. `POST /api/v1/hooks/observed` validates the provider-neutral payload. [[src-tauri/src/server.rs#ObservedSubagentState]] synchronously folds identifiable lifecycle evidence by provider, host, root, and agent, then emits `hooks-observed-updated` on a state change.
...
5. [[src/hooks/useBreakdownData.ts#useBreakdownData]] declares `hooks-observed-updated` for Sessions and Hooks. Mounted keys join the shared five-second fan-out; Sessions IPC then overlays nullable state synchronously and positive rows re-render without a new poller.

Disabling activity tracking removes both managed observer paths and invalidates current coverage. Provider disable clears only that provider. Re-enable stays unknown until a newer qualifying root epoch; Quill restart also returns unknown.
