...
3. `POST /api/v1/hooks/observed` validates the provider-neutral payload. [[src-tauri/src/server.rs#ObservedSubagentState]] synchronously folds identifiable lifecycle evidence by provider, host, root, and agent. A qualifying root start retains validated root cwd and activity time; later supported events advance only an already-active exact root. Subagent cwd never replaces the root project.
