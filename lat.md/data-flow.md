...
Stage one calls [[src-tauri/src/sessions.rs#discover_codex_transcripts_in]] — again the retained inventory walker — and keys every rollout by the thread id its filename ends with, which restates the file's own `session_meta` id on 5501 of 5502 rollouts measured (the odd one out has an unreadable head). The id is only a locator; identity always comes from the head record, parsed by [[src-tauri/src/transcript_identity.rs#codex_metadata]], the same parser retained ingest uses, so which field names the spawning parent is decided in one place. Rollouts outside the idle window are skipped, so only fresh files and the ancestors they name are ever opened.

[[src-tauri/src/transcript_scan.rs#codex_root]] walks `parent_thread_id` / `forked_from_id` to the user thread that owns a spawn, counting hops as spawn depth. Nesting is real and bounded: measured depths run 1–3 across 4487 spawned rollouts, and every one resolves to a root the same corpus still holds. Forked rollouts are never a separate population — no rollout carries `forked_from_id` without also carrying a spawn marker — so the fork field is a parent pointer, not a second concept.
