...
Pi usage events insert once through [[src-tauri/src/storage.rs#Storage#store_pi_model_usage]], use an upstream `provider/model` id, store native input/output/cache-read/cache-write/total cost without exposing it, and own a synthetic `pi-push:<session-id>` source. The removed adapter cannot refresh transcript sources after upgrade, so saved pre-upgrade rows and post-upgrade pushes are disjoint and both remain active. Event UUIDs deduplicate live/spool replay, while the parity corpus proves the upgrade union does not double count. Sessions and model history report spend truth with [[src-tauri/src/models.rs#ModelTokenScope]] fixed to `all-branches`. Test coverage lives in [[pi-model-usage-tests#Pi Model Usage Test Specs]].
