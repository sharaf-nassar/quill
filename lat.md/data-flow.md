...
Lifecycle-hook fires are surfaced in the widget Usage view's Hooks breakdown mode via two distinct paths because Claude and Codex log hooks differently.
...
2. **Codex path (live observer).** Codex rollouts do not record hook executions, so the installer registers `src-tauri/codex-integration/scripts/hook-observe.cjs` on all 11 Codex hook events when `activity_tracking` is enabled. On each fire the script reads stdin, builds a `{provider, session_id, agent_id, hook_event, tool_name, cwd, ts, hook_matcher}` payload (session id resolved through a `session_id || conversation_id || id` fallback, matcher always null because hook stdin omits the configured matcher), and POSTs to `POST /api/v1/hooks/observed` with bearer auth and a 1.5-second local timeout.
3. The HTTP handler in [[src-tauri/src/server.rs]] validates [[src-tauri/src/integrations/codex.rs#is_supported_hook_event|the shared 11-event lifecycle set]] plus length caps and ISO-8601 timestamp shape, fast-acks `202 Accepted`, and dispatches the insert via [[src-tauri/src/storage.rs#Storage#store_codex_hook_observation]] on a `tokio::task::spawn_blocking`. Codex identity is event-scoped (`hook_event` plus optional `:tool_name`) because the observer fires per-event, not per-script.
4. The server emits `hooks-observed-updated` after a successful Codex insert. [[src/hooks/useBreakdownData.ts#useBreakdownData]] declares that dependency beside the transcript and session channels; the process cache invalidates its Hooks key and joins mounted refreshes to the shared five-second-or-longer fan-out instead of issuing one IPC request per fire.
...
6. The widget's breakdown section ([[src/components/widget/views/UsageView.tsx#UsageView]]) renders one row per `hook_identity`, keeps `quill:` prefixes visible in the identity text, and carries the Claude/Codex tracking-asymmetry help in the Hooks header.

The privacy gate is the existing `activity_tracking` feature flag: toggling it off removes `hook-observe.cjs` and its 11 `[[hooks.<Event>]]` entries from the active Codex `config.toml`, stopping new Codex observations. Claude-side ingestion is unaffected because Quill never writes anything to Claude transcripts — the data flows from the user's transcript to Quill, not the other way around.
...
[[src-tauri/src/storage.rs#Storage#replace_model_source]] folds each retained replacement batch into [[backend#Backend#Database#Schema#Hourly Analytics Rollups|model_usage_hourly]] inside the same immediate transaction as raw observation and source metadata writes. It refolds the source's unpruned rows, then advances `rollup_generation` once when rollup rows changed; any later failure rolls back raw evidence, rollup rows, and generation together.
...
[[src-tauri/src/lib.rs#spawn_startup_model_source_reconciliation]] re-admits the full retained inventory after runner state initializes, including when the one-time backfill is already complete. This recovers sources retained while Quill was stopped without requiring Session Search to open; duplicate startup, search, notify, and rescan admissions still converge on the same canonical source-key queue.
...
The always-on transcript rescan loop ([[src-tauri/src/lib.rs#spawn_transcript_rescan_loop]]) sends every source modified past its watermark to both [[src-tauri/src/lib.rs#enqueue_model_usage_live_source]] and [[src-tauri/src/lib.rs#enqueue_transcript_analytics_live_source]]. Periodic discovery therefore covers model evidence as well as runtime analytics when a session hook never fires, without translating source keys or merging sibling work.
