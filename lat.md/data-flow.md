...
Hook evidence has two consumers: durable SQLite audit rows feed the Hooks breakdown, while complete root/subagent lifecycle payloads also feed a bounded current-process Sessions overlay.

1. Claude transcript attachments still provide general audit history. Under `activity_tracking`, its existing `observe.cjs` additionally posts `SessionStart`, `SubagentStart`, `SubagentStop`, and `SessionEnd` with provider, normalized hostname, root session, source, agent, cwd, and producer time.
2. Codex's generic observer remains registered on all 11 events under the same gate. It adds hostname and official `SessionStart.source` to the existing root/agent payload while preserving session fallbacks and event-scoped audit identity.
3. `POST /api/v1/hooks/observed` validates the provider-neutral payload. The hook path derives the session's complete next [[src-tauri/src/server.rs#SessionSnapshot]] and hands it to [[src-tauri/src/server.rs#SnapshotReconciler]], which keeps the last write per provider, host, and root session by observation time. A qualifying root start retains validated root cwd and activity time; later supported events advance only a session whose start is known. Subagent cwd never replaces the root project.
