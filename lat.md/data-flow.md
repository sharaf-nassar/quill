...
[[src-tauri/src/live_tracker.rs#LiveTracker]] owns Claude/Codex/Pi transcript folds and Pi-pushed live state, so a Sessions read costs a map lock instead of a directory walk. It is the sole owner of liveness and open-agent membership.

`POST /api/v1/pi/track` accepts bearer-authenticated envelopes in two shapes and routes each on its declared protocol. Protocol-2 lifecycle commits before live mutation and answers typed dispositions. Protocol-1 envelopes carry stable event ids for activity, model, lineage, cumulative-token, or per-message usage events and accelerate the same live state as hints, without a generation acknowledgement, a durable receipt, or a reporter-subject header match, because the persisted snapshot overwrites whatever they report. Validation finishes before mutation. Hostnames reject control characters and normalize once to the lowercase short storage key; Pi runtime messages use the same key before analytics storage. Only session start may create state; later events update an existing identity, shutdown removes it, and stale shutdown cannot close a newer startup/reload continuation. Replacement starts remove their prior id first. Demo mode mutates nothing.

Per-message usage persists through [[src-tauri/src/storage.rs#Storage#store_pi_model_usage]] before its token delta reaches LiveTracker. The Pi-only host/session/event UUID index makes live replay a no-op, so only a newly inserted hint advances cumulative live tokens; persisted reconciliation later corrects the same canonical source.

Session-start events also upsert durable cwd, hostname, and migration-42 `ephemeral` state into `live_analytics_sessions`. Authenticated Pi tracking, hook, and context telemetry carry one canonical reporter subject: normalized host, process instance, install channel, protocol, reporter version, Quill build, and capability digest. [[src-tauri/src/storage.rs#Storage#record_pi_reporter_failure]] records compatibility, lifecycle, configured-child, source, transport, and typed saturation failures for that subject; only evidence from the same subject and dimension recovers them. Bad authentication records nothing.
...
[[src-tauri/src/live_tracker.rs#LiveTracker#sweep]] is the cold start, watcher-overflow recovery, periodic backstop, and pushed-session crash fallback for all three providers. It uses [[src-tauri/src/sessions.rs#discover_claude_transcripts_in]], [[src-tauri/src/sessions.rs#discover_codex_transcripts_in]], and [[src-tauri/src/sessions.rs#discover_pi_transcripts_in]], then releases any session silent past [[src-tauri/src/live_tracker.rs#IDLE_AFTER]]. Every non-terminal Pi push advances the same last-event clock. [[src-tauri/src/live_tracker.rs#modified_within_idle_window]] gates Claude and Pi transcript reads on modification time; Codex still indexes quiet ancestors before applying the same parse gate.
...
[[src-tauri/src/transcript_watcher.rs#admit_pending]] feeds Claude/Codex/Pi transcript evidence; `/api/v1/pi/track` is the independent pushed feed. Watcher batches go to the fold alongside retained ingest, while Pi pushes mutate the same map after authenticated validation and durable lifecycle persistence. Startup manages the tracker before either feed starts and applies saved activity-tracking and per-provider switches, so a disabled provider is never folded or push-created.
...
[[src-tauri/src/live_tracker.rs#TrackerState#pi_file]] reads that header through [[src-tauri/src/pi_session.rs#read_pi_session_header]] — the same bounded v2/v3 probe notify validation uses — and remembers the answer on the file's own tail entry, so a warm sweep re-reads no headers. The header also supplies the session's origin and project and the floor its activity falls back to before the first prompt is typed, and it never overwrites what a push already established.

[[src-tauri/src/live_tracker.rs#fold_pi_line]] then folds only appended bytes. Substantive `user`, `assistant`, and `toolResult` entries advance activity, so custom extension records, the reporter's own `quill-tracking` entry, and thinking-level and compaction markers cannot reopen a finished session. An assistant message names the model answering and the `usage.totalTokens` it cost; a `model_change` names a switch no message has answered under yet. A nested child's own `session_info` names its role only when its label repeats the run id the structural path states. Both pass the same validation the pushed model event does. Pi writes no end record, so closure is the shared idle cutoff, the same one Claude and Codex answer to.

The fold and the push read the same assistant messages, so the file's cumulative total is kept on its own field and published rather than added: [[src-tauri/src/live_tracker.rs#LiveTracker#add_pi_live_tokens]] stops contributing once the file has folded a valid `usage.totalTokens`, including zero, and the two producers reach one number in either order. A rewritten file is shorter than the offset already consumed, which clears both the activity and the total it had contributed before the replacement is folded whole. `process_instance_id` and `recovering` exist only because pushed state dies with its process, so a folded session claims neither.

### Pi Push Rules

Pi live state still accelerates from authenticated extension events, while persisted sessions are authoritative for durable lifecycle, runtime, usage, tool, skill, and search recovery after downtime.
...
The extension calls Session Search notify only when Pi supplies a transcript path already on disk; notify remains the lowest-latency changed-source path. Startup inventory, the filesystem watcher, and the 120-second rescan recover the same persisted source without notify. Validation uses [[src-tauri/src/pi_session.rs#read_pi_session_header]] as a bounded v2/v3 probe, then shared parsing indexes messages and schedules authoritative reconciliation. A missing path provides no persisted evidence to invent.
...
Pi live usage hints and persisted native assistant usage converge on one canonical host-qualified source through [[src-tauri/src/storage.rs#pi_source_key]]. Live hints insert once through [[src-tauri/src/storage.rs#Storage#store_pi_model_usage]]; persisted reconciliation authoritatively replaces that source's unpruned observations, token snapshots, rollups, and model registry in the same transaction as lifecycle/runtime/tool evidence. Each accepted runtime hint advances compact open-turn state and adds only newly closed hourly turns; confirmed retention prunes processed prefixes without losing the logical open turn. Migration 44 owns earlier source-less runtime and `pi-push:` rows, migration 45 compacts live state, and migration 46 rekeys both domains by normalized host plus session after a verified schema-45 backup. Event UUIDs deduplicate overlapping hints, and insert-time watermark filtering prevents replay from restoring pruned raw rows. Sessions and model history report spend truth with [[src-tauri/src/models.rs#ModelTokenScope]] fixed to `all-branches`. Test coverage lives in [[pi-model-usage-tests#Pi Model Usage Test Specs]].
...
8. Retained runtime, response, tool, skill, hook, Pi lifecycle/receipt, and Pi native-usage rows persist in provider-aware SQLite snapshots owned by canonical source. Pi live hints use that same host-qualified owner; other remote message and hook pushes keep source-less identities.
