...
The system has eight primary data pipelines connecting hook scripts, the HTTP server, the database, and the frontend.
...
Hook-reported tokens still flow into `token_snapshots` keyed by the parent `session_id` — Claude sub-agents share the parent's session id on disk, so each row also carries `is_sidechain`/`agent_id`/`parent_uuid` from migration 20. The [[backend#Tauri IPC Commands#Usage and Token Commands (17)]] `get_session_breakdown` rollup aggregates parent and sub-agent rows at query time so a sub-agent's tokens count toward the parent session's totals, and `get_llm_runtime_stats(scope = "parent_only")` is available when the widget runtime readout needs to exclude the sub-agent traffic instead.

An accepted live Pi usage event writes one `turn` snapshot for any known lifecycle origin, ephemeral or ordinary; event-UUID dedupe on the observation insert gates that write, so replay cannot double tokens. Retained Pi reconciliation authoritatively replaces the same host/session's persisted turn and summary snapshots from native JSONL usage, letting history recover while Quill was closed. Migration 48's snapshot kind keeps both producers' spend in token history while only turns increment token, host, project, and hourly turn counts. Sessions unions zero-token ephemeral origins until usage arrives and returns their additive badge flag.
...
## Startup Database Migration Pipeline

Schema upgrades expose progress and preserve a verified pre-change recovery point before normal storage-dependent services start.

1. [[src-tauri/src/storage.rs#Storage#startup_migration_plan]] reads the existing schema without normal setup. A current or new database stays on the direct startup path.
2. An outdated database hides `main` and opens the transient migration webview; `StartupGate` prevents either webview from mounting storage-dependent hooks while status is not `ready`.
3. [[src-tauri/src/lib.rs#start_storage_migration]] returns from Tauri setup, then opens storage on a worker and publishes process-local snapshots plus `startup-status` events.
4. Before migrations 46 or 48 rebuild/re-key analytics tables, storage requires free space equal to twice the database size and creates a versioned sibling with SQLite `VACUUM INTO`, which includes committed WAL state. It verifies integrity and schema before and after atomic publication.
5. Transaction rollback handles ordinary migration errors; the separately verified copy covers interruption, filesystem failure, or faulty migration code and can be restored with Quill stopped. [[pi-model-usage-tests#Pi Model Usage Test Specs#Analytics Migration Backup Preflight And Recovery]] pins retry and restore.
6. Progress is determinate only while backup bytes have a trustworthy denominator; verification, rebuild, and finalization remain named indeterminate stages.
7. Success runs normal setup, restores the user's persisted main-window geometry, publishes `ready`, closes migration, and reveals `main`. Failure keeps normal hooks gated and routes to the fatal recovery dialog.
...
4. The handler fast-acks `202 Accepted` and writes [[backend#Backend#Database#Schema#Hook Invocations]] on a blocking worker through [[src-tauri/src/storage.rs#Storage#store_hook_observation]], emitting `hooks-observed-updated` on success and `hooks-ingestion-error` on failure. On that same worker a Pi hook carrying `X-Quill-Pi-Process` first runs [[src-tauri/src/server.rs#prove_pi_session_from_hook]], the same-process liveness proof that reopens a `recovering` lifecycle row; its outcome never changes the acknowledgement or the audit row.
...
[[src-tauri/src/live_tracker.rs#fold_pi_line]] then folds only appended bytes. Substantive `user`, `assistant`, and `toolResult` entries advance activity, so custom extension records, `quill-tracking` lifecycle entries, thinking-level, and compaction markers cannot reopen a finished session. An assistant message names the model answering and its `usage.totalTokens`; a `model_change` names a switch no message has answered under yet. A nested child's own `session_info` names its role only when its label repeats the run id the structural path states. Pi writes no end record of its own, so a session without reporter proof closes at the shared idle cutoff, the same one Claude and Codex answer to — but a reporter-announced `session_end` is remembered as a tombstone that outranks the file's remaining recency: [[src-tauri/src/live_tracker.rs#TrackerState#pi_file]] refuses to fold a tombstoned identity, a new `session_start` for it clears the tombstone, and entries older than the idle window are pruned because a file that quiet cannot fold anyway.
...
A restart restores both directions of that lifecycle memory: open durable rows rehydrate as recovering sessions, and recently closed rows seed end tombstones through [[src-tauri/src/live_tracker.rs#LiveTracker#seed_pi_ended_sessions]] via [[src-tauri/src/storage.rs#Storage#load_pi_recently_closed_sessions]], so the startup sweep cannot re-fold a child whose end predates the restart. Seeding races that sweep, so it also drops an already re-folded session as long as it claims no process; one a new process has re-announced stays. Rehydrated rows read as `recovering` until the owning process's next hook proves them live or the idle window closes them; see [[features#Features#Transactional Pi Lifecycle]].

A fork or clone copies the parent's `quill-tracking` entries into the child file. Persisted evidence takes only the entries naming the child's own header id, so the parent's lifecycle stays with the parent's file and the child is not rejected for carrying it. The persisted parser removes the known historical same-file `resume` self-reference before strict decoding; live protocol validation still rejects it, and no transcript is rewritten.
...
Persisted Pi assistant and summary usage owns one canonical host-qualified source through [[src-tauri/src/storage.rs#pi_source_key]]. Assistant rows retain nullable reasoning, normalized/bounded stop reason, and measured error presence without adding reasoning to totals that already include it. `compaction` and `branch_summary` usage emits stable `pi_summary_v1` summary observations; missing model identity stays missing, compaction keeps `tokens_before`, and summary spend enters token snapshots and hourly/provider totals without entering turn counters. `thinking_level_change` records become source-owned `session_setting_events` rows, where the active level is the latest `(timestamp, source_ordinal)` at or before a message; unobserved history stays NULL and repeated levels remain evidence. The last `session_info` name by source ordinal becomes the source's registry `session_name`, and a cleared name clears it. Reconciliation replaces that source's unpruned observations, token snapshots, rollups, setting rows, and model registry in the same transaction as lifecycle/runtime/tool evidence. Migration 44 owns earlier source-less runtime, migration 45 compacts live state, migration 46 rekeys both domains by normalized host plus session after a verified schema-45 backup, and migration 48 gives token snapshots an explicit `turn`/`summary` kind so old rows remain turns. Insert-time watermark filtering prevents replay from restoring pruned raw rows. Sessions and model history report spend truth with [[src-tauri/src/models.rs#ModelTokenScope]] fixed to `all-branches`; summary rows contribute coverage tokens but never model segments or switches. Test coverage lives in [[pi-model-usage-tests#Pi Model Usage Test Specs]].

[[src-tauri/src/model_usage.rs#prepare_model_source_reconciliation]] stages stable fingerprints and native graph identity into an owned plan before any replacement transaction. Per-source parsing runs on the fixed decoder and drops observations and diagnostic payloads before the next source is prepared; equal-content file bytes are never retained across the root. Commit reparses one source, checks its exact content fingerprint and native identity/counts against the plan, stamps its resolved root, and uses the existing generation-guarded replacement. Drift leaves the plan incomplete and last-good data intact, so it cannot authorize pruning. Claude/Codex analytics use [[src-tauri/src/transcript_identity.rs#read_stable_transcript]] with the 256 MiB input cap. Pi uses [[src-tauri/src/pi_session.rs#read_stable_pi_session]] to stream records under separate input, record, and retained-evidence budgets. Both readers reject path/open-handle drift and fingerprint exact original bytes before replacement. Filesystem layout hints can report conflicts but cannot override transcript-native parent metadata.
...
[[src-tauri/src/sessions.rs#SessionIndex#sync]] scans retained Claude, Codex, and Pi roots through their concrete walkers. Pi identity comes from its bounded native header plus local normalized hostname, so startup search and analytics inventory address the same source without waiting for `sessions/notify`.

[[src-tauri/src/transcript_watcher.rs#start]] recursively watches distinct Claude, Codex, and Pi transcript roots. It admits debounced JSONL changes through strict validation. Missing or failed watches retry every 120 seconds, and ambiguous canonical roots are rejected. Watcher construction or channel failure requests recovery before retrying initialization. Periodic model recovery admits at most 128 durable obligations per pass, including never-seen sources, unchanged transient failures, and backwards-mtime changes. Versioned content rejections settle unchanged bytes. Each model outcome completes its own source rather than treating successful diagnostic persistence as successful ingestion. See [[pipeline-recovery-tests]].
...
2. Every retained-worker pass and the `sync_search_index` command run [[src-tauri/src/sessions.rs#SessionIndex#sync]], which re-extracts only sources whose nanosecond mtime or size moved and prunes documents of sources a completely enumerated root no longer contains. Each successful source fingerprint and native session id is committed in an internal Tantivy checkpoint document alongside that source's messages. Startup recovers successful ownership from those checkpoints, not the independently flushed `index_state.json` sidecar. A separate rejection map records stable invalid versions without overwriting last-good documents or checkpoints. Missing native identity, including an empty Codex rollout, is rejected until bytes change; it is never a successful identity-free extraction. Legacy missing-path checkpoints still require revalidation. The sweep consults no analytics migration re-ingest markers: derived analytics rows belong to retained reconciliation. Schema upgrades preserve indexed history before current retained sources are indexed under explicit ownership.
...
6. Provider-specific parsers enrich messages: Claude tool blocks and Codex function/custom tool calls become tools_used, files_modified, code_changes, commands_run, and tool details. Pi indexes user, assistant, and `custom_message` entries once by id in source order, converts assistant `toolCall` blocks into the same metadata, and attaches each matching `toolResult` as a 10 KiB `full_output` plus a 300-byte command preview.
7. Tantivy stores provider, message_id, session_id, content, role, custom_type, project, project_path, host, timestamp, git_branch, and enriched metadata. Pi result roles and non-context `custom` entries are excluded, while `custom_message` injected context indexes under its own role with a searchable `custom_type`; Claude and Codex provider-native search roles remain visible. Schema version 10 adds source ownership and checkpoint fields through a staged, validated migration with interrupted-rename recovery. Historical documents without provable ownership remain unattributed and cannot be pruned by local absence scans.
8. Retained runtime, response, tool, skill, hook, setting, Pi lifecycle/receipt, and Pi native-usage rows persist in provider-aware SQLite snapshots owned by canonical source. Pi persisted sources use that same host-qualified owner; other remote message and hook pushes keep source-less identities.
...
12. Result context first selects the exact retained `source_key` carried by the hit. Pushed and historical unattributed owners report context unavailable instead of borrowing another source's content. Legacy requests without a selector use [[src-tauri/src/sessions.rs#find_session_path]]: the analytics source registry answers every provider with one indexed query, an exact chain or source-session match outranks a resolved-root match so a sub-agent chain opens its own file, and the path is re-validated against the configured root before it is read. A Claude parent that predates the registry still resolves by its `<session-id>.jsonl` filename; nothing walks or parses the corpus to find a session.
...
One transcript parse emits both search messages and runtime events.

`ExtractedMessage` feeds the search index, while `ExtractedEvent` feeds the [[backend#Database#Schema#Code and Runtime Metrics]] `session_events` table. Parser outputs carry content identity only; retained-source reconciliation stamps canonical root, chain, agent, and sidechain identity.

The search index drops Claude `tool_result`-only user messages and empty assistant blocks, admits user, assistant, and injected-context roles for Pi, and preserves intentional Claude/Codex provider-native roles. The event stream still carries every non-meta `user`/`assistant` line with a non-empty timestamp — classified by content shape into `user_text`, `user_tool_result`, `asst_text`, `asst_thinking`, or `asst_tool_use`. Retained-source reconciliation owns transcript analytics persistence; Tantivy startup and `/sessions/notify` indexing never delete or insert those rows. The `/sessions/messages` remote-push handler stores source-less events and flattened tool rows through [[src-tauri/src/storage.rs#Storage#store_live_session_analytics]], consuming ordered event kinds when supplied and retaining the one-event `(role, content, tools_used)` heuristic for older clients. Additive `custom_message` rows index their bounded content and optional `custom_type` without an event; one-name tool rows carry bounded object details, nullable error state, and bounded image count, while old envelopes write NULL evidence.
...
Pi pushes stable turn and tool message ids through the same endpoint. Turn/input and turn-end map to user/assistant response boundaries; tool execution maps to the canonical tool-use/result pair. Live Pi pushes still reject `asst_thinking`, because that path has no thinking evidence; retained Pi transcript parsing emits it from `thinking` blocks before text and tool use.
...
Feature 009 adds a third sibling to this dual emission: the same Claude JSONL walk peels off `type:"attachment"` records whose `attachment.type` begins `hook_` (e.g. `hook_success`, `hook_failure`, `hook_timeout`, `hook_blocked`) via [[src-tauri/src/sessions.rs#extract_hook_invocation_from_attachment]], producing one [[backend#Database#Schema#Hook Invocations]] row per fire. The extractor carries hook content only; retained-source reconciliation stamps canonical root, chain, sidechain, and agent identity. Codex rollouts do not emit attachment records, so Codex hook telemetry instead arrives live via `POST /api/v1/hooks/observed` from a deployed observer script.
...
Startup reconciliation allocates durable per-root generations, resolves each provider-qualified native chain independently, and resumes migration-armed historical rebuilds until their owning roots finish. The global `transcript_analytics_reingest_pending` marker forces every provider, while migration 48 arms only `pi_transcript_analytics_reingest_pending`; Pi's older persisted-source recovery marker shares that provider-scoped path. A scoped marker bypasses freshness short-circuits only for Pi roots, never Claude or Codex. The global marker clears only after every root succeeds; each Pi marker clears exactly once after every available Pi root succeeds and remains armed across source failure or an absent Pi root. Forced reparse bypasses the mtime/size fast path, content-digest short-circuit, and content-deterministic-failure short-circuit; suppression is honoured regardless. Unchanged sources owe nothing: a whole-root pass writes only for sources it replaced or newly failed. Unrelated sessions sharing one provider directory retain distinct roots, and Tantivy document mutation performs no analytics writes; production sweep admission first reconciles changed Search sources through the shared Search/analytics boundary.

Failure is isolated per source. `RootReconciliationFault` separates a source-scoped failure from a `RootUnavailable` root that simply produces no prune proof and from a `Database` fault — failure of the bounded diagnostic upsert itself — which is the only one that abandons the run, because after it nothing can retain last-known-good state. For Pi, the owning failure call also marks the reporter subject associated through `pi_session_lifecycle` as `reconciliation_failed`; committed persisted-source recovery clears that same process's source dimension. Reconciliation replaces only source-owned rows, and pruning is gated on enumeration completeness alone: a failed source cannot block it, because it is still a member of the enumerated key set the pass hands the prune. [[src-tauri/src/storage.rs#Storage#prune_transcript_analytics_sources_for_root]] deletes exactly the rows of that root outside that set whose `seen_generation` does not exceed the pass's, so a source a concurrent live commit stamped newer survives even when the pass never saw it. It surfaces a row-decode failure while collecting prune keys instead of swallowing it, so a partial key set cannot look like a complete one. [[src-tauri/src/storage.rs#Storage#record_transcript_analytics_source_failure]] fingerprints and skips unchanged only content-deterministic classification failures: invalid UTF-8/metadata, oversized sources, and identity failures. Transient storage, graph, and live-source failures keep no fingerprint and retry every pass.

Live notifications use the shared provider-plus-source coordinator. Its transcript job services Search and transcript analytics with independent committed fingerprints: failure rearms the job, but successful consumers skip unchanged work on retry. Model work keeps its independent completion and retry state. Scoped reconciliation combines the changed source with persisted sibling identities and reparses only descendants whose resolved root moves. Provider/root permits are acquired before filesystem enumeration and held through absence pruning, serializing full inventory-through-prune and scoped prepare-through-commit lifecycles. Explicit-inventory callers revalidate missing candidates after acquiring permits; registry writes also reject older generations.

Live coverage no longer depends solely on the per-session `/sessions/notify` hook. [[src-tauri/src/transcript_watcher.rs#start]] drops non-rescan `EventKind::Access` events before the bounded callback channel, coalesces relevant JSONL mutations for at most one second, then reuses strict source validation and the canonical live queue. Remove, rename, notify-rescan, and bounded-buffer overflow signals request whole-root graph reconciliation and pruning, and so does the watcher's own 120-second tick: it folds live evidence first and then arms the same recovery request, so an otherwise idle Quill still reconciles and prunes every root at that cadence, subject to the 30-second cooldown. The first recovery after quiet runs promptly; recovery storms inside the cooldown still run Session Search and leave one deferred reconciliation armed until timeout. Each retained-worker pass enumerates roots once and shares that inventory between Session Search and whole-root analytics reconciliation. A recovery pass also admits every source whose mtime advanced since the previous recovery pass to the live coordinator through [[src-tauri/src/transcript_watcher.rs#admit_changed_sources]], the changed-source backstop for a session whose hook never fires; whole-root reconciliation already covers the transcript domain, so this admission is what keeps Claude/Codex model work current without a second walk (see [[architecture#Background Tasks]]). Whole-root unchanged sources write nothing; live unchanged notifications perform their single registry refresh and return `SuppressedUnchanged`.
...
Heavy transcript work shares a fixed decoder and one source-lifetime budget, without caching parsed transcripts.

[[src-tauri/src/transcript_work.rs#with_source]] admits one heavy source lifetime FIFO, before raw reads or decoding and through normalized-output consumption, Search/SQLite commit, and drop. Nested synchronous consumers reuse that admission. [[src-tauri/src/transcript_work.rs#decode]] uses a single persistent named standard thread and bounded mailbox, so temporary JSON allocations do not rotate across Tokio blocking workers. Inventory, root graph orchestration, and stat-only unchanged checks remain outside the decoder; whole-root and model loops release the source budget between sources. Existing provider/root serialization still protects graph generations and pruning. Ordinary live reconciliation reads/parses its one changed source once and resolves it against compact persisted identities. Only graph-changing descendant fan-out falls back to identity inventory plus one-at-a-time commits; it never retains a vector of parsed snapshots.

[[src-tauri/src/transcript_analytics.rs#reconcile_retained_source_with_hints]] is the shared owning boundary for canonical notify and changed-source sweep work. The sweep first skips current Search fingerprints so an unchanged inventory never lists/refreshes an analytics root once per source; analytics-only retries remain on the live queue and whole-root recovery, and pushed hint updates bypass this no-hint sweep prefilter. A stable, identity-validated parsed source supplies Search documents and analytics from one extraction. Search commits independently before the analytics transaction; failed/suppressed analytics allows Search-only fallback when its own checkpoint is stale. [[src-tauri/src/sessions.rs#SessionIndex#replace_retained_source]] advances its in-memory `IndexedSource` canonical path/fingerprint/hints only after successful document commit, rolls back writer errors, and rejects source drift. Owning sweep, whole-root reconciliation, and live-drain batches flush Search checkpoints once after processing, including successful sources from a partially failed batch; the legacy notify recovery path flushes its single operation. [[src-tauri/src/sessions.rs#SessionIndex#save_state]] streams the map into a same-directory temporary file, flushes and synchronizes its contents, then atomically replaces the checkpoint under the state lock. A failed live-batch flush rearms Search without hiding successful analytics events. Legacy notify failures keep their pending generation and reuse capped backoff; older completions cannot clear newer payloads, and Search invalidation still fires after a partial commit. A sweep emits committed analytics updates before propagating a Search/prune/persistence error. A crash before a batch flush can replay committed documents idempotently; no directory synchronization or full power-loss durability is claimed. Read/size/identity failures preserve previous documents; an identity-less never-indexed source can still be fingerprinted without inventing documents. Complete inventories plus a last-moment absence check for known in-root canonical paths prove Search removal, so a newer live commit cannot be pruned by an older inventory. For Pi, a supported header with a different native ID also proves the old source was replaced; unreadable/unsupported headers retain last-good documents.

Notify hints belong only to the admitted source. Graph-changing fan-out passes no new hints to descendants, preserving their existing Search metadata, and records classification failures against the failing descendant while retaining its last-good analytics.

Search and analytics use the same 256 MiB stable raw-input cap. Pi decoding consumes each line's owned JSON directly into Pi entries rather than retaining and cloning a complete `Vec<Value>`; the raw buffer drops before row extraction. Identity-only Pi inventory validates header and tracking evidence but drops message payloads immediately. This bounds concurrent materialization, not inherent normalized output size or allocator RSS retention. No allocator cap or periodic trim is used. Decoder self-submission and source admission from the decoder thread are explicitly rejected before waiting. Exact checks and synthetic measurement recipes are in [[transcript-memory-tests#Transcript Memory Test Specs]].

The generic admission helper is an owning-callsite contract. Retained/model scopes return only compact identities, commit outcomes, or errors after dropping full payloads; model preparation mutates only compact plans. Learning awaits a blocking-pool task for digest construction, so source-admission waits and fetch/redaction/compression do not suspend sibling `tokio::join!` streams. It holds admission through fetch, redaction, and existing 48 KiB-budget compression, returning only the digest. Search context drops raw input and unrelated extracted rows under admission, but transfers the final explicitly requested `SessionContext` response to its caller without changing response semantics. That response can still be large; response-byte limits and client context caches are separate residual concerns, not a proven cause of this background incident. Context read failures propagate and extreme windows use saturating end arithmetic. Interactive context requests deliberately join the same FIFO and can wait behind earlier source work through decoding and consumer commits. This trades context latency for bounded transient memory; there is no separate interactive latency guarantee or bypass that could recreate concurrent large materializations.
...
A notification arriving during an older attempt must rearm both domains, and the older completion must not clear that work. The same test pins newest pushed Search hints and confirms watcher admission without hints cannot erase them.
...
`notify` access events without a rescan flag must be discarded before they consume any bounded callback-channel slot or set its overflow flag. Mutation events, watcher errors, and rescan notices — including rescan-flagged access events — must remain deliverable through that same channel.
...
Bursts during that scan produce at most one follow-up scan. Any recovery request upgrades the coalesced work so whole-root analytics reconciliation is not lost. Each worker pass enumerates retained roots once and shares that inventory between Session Search and whole-root analytics reconciliation.

##### Startup Backfill Sequencing

The watcher must complete its cold live-tracker fold before requesting startup whole-root reconciliation on the retained worker.

This ordering keeps current session models and liveness available before a migration-armed historical Pi backfill starts, and startup must not run a second independent reconciliation worker beside the watcher-owned pass.

##### Recovery Cooldown

Recovery storms must not launch whole-root reconciliation more than once per 30 seconds or lose the final requested pass.

The first recovery after quiet runs promptly. During the cooldown, intervening wakes still scan Session Search immediately, keep recovery armed across scan completion, and run the deferred whole-root pass on timeout without requiring another request.
...
A notify whose ownership validation is unavailable answers 503 and is not retried by the server: the watcher and the recovery pass re-admit any changed source once its root is readable again, so a retry queue would only duplicate them. Invalid candidates are dropped. A validated canonical source enters the shared coordinator in one non-fallible state mutation after managed state exists, so admission cannot partially accept one analytics domain. Session Search availability remains independent.
...
Wall-clock tool and thinking durations exist only in the live Pi stream, so the managed extension appends them as persisted-only `quill-tracking` span receipts that retained reconciliation consumes at parse time.

Nothing is sent live and no receipt becomes a `pi_event_receipts` row.

[[src-tauri/pi-integration/quill.ts#appendSpanReceipt]] writes one `tool_span` per finalized tool call (`tool_call_id`, `started_at_ms`, `ended_at_ms`, keyed by `tool_execution_start`/`tool_execution_end` `toolCallId`) and one `thinking_span` per finalized thinking block (`message_id`, `content_index`, same times). Thinking boundaries come from `message_update.assistantMessageEvent` `thinking_start`/`thinking_end`, buffered per content index and flushed at `turn_end`, where [[src-tauri/pi-integration/quill.ts#persistedMessageId]] finds the owning entry on `sessionManager.getBranch()` by object identity with the message Pi just persisted; a message Pi did not persist yields no receipt. Buffers are bounded (32 thinking blocks per message, 256 tool calls in flight) and an end without a start is dropped, never guessed. The hot path is one property read and a type comparison per streamed delta: the pinned microbenchmark measures under 10 ns and no heap growth across 200,000 `text_delta` events. The payload shapes are verified against pi-coding-agent 0.84.0, the floor `src-tauri/src/integrations/pi.rs` enforces, and the real-loader test proves the installed Pi emits a `tool_span` for its probe call.

The receipt kinds ship behind the `span-receipts` capability and reporter `0.3.0` as one exact-pair release with the receiver. [[src-tauri/src/pi_session.rs#parse_pi_session_records]] routes `quill-tracking` entries whose `data.event` is `tool_span`/`thinking_span` to [[src-tauri/src/pi_tracking.rs#decode_protocol_v2_span_receipt]] instead of the lifecycle decoder (both share the outer-entry, schema, and reporter prelude), so a span never reaches the closed lifecycle variants and a malformed span is kept as an undecoded [[src-tauri/src/pi_session.rs#PiSpanEntry]] rather than a source failure. [[src-tauri/src/transcript_analytics.rs#pi_span_durations]] folds decoded spans into `tool_actions.duration_ms` by `tool_use_id` (last receipt per call id wins) and sums a message's thinking blocks (last receipt per block wins) into the assistant observation's `reasoning_duration_ms`. Malformed spans and spans naming another session leave their durations NULL and count into the bounded `malformed_spans`/`first_malformed_span_ordinal` diagnostic. Older reporters append no spans, so every duration stays NULL without any parse failure; no backfill is claimed for historical sessions.
...
2. Claude polling in [[src-tauri/src/fetcher.rs]] calls the Anthropic usage API (`?cedar_ember=1`, Claude CLI user agent) with the local OAuth Bearer token and parses Claude bucket keys plus banked [[features#Features#Live Usage View#Limit Resets]]; when Pi is enabled and that native path fails, it obtains a short-lived Anthropic bearer from Pi's CLI credential boundary and retries without ever reading Pi's refresh token
...
5a. Each Codex rate-limit snapshot may also carry a `credits` object (`balance`, `hasCredits`, `unlimited`). The fetcher extracts the first non-null, non-unlimited credit balance and returns it as a `ProviderCredits` entry alongside the buckets. The app-server response's `rateLimitResetCredits` rows become `provider_resets`; the Pi WHAM and transcript fallbacks cannot see them, so those reads report resets as unknown and the poll keeps the last reported rows
6. If the direct Codex app-server request fails and Pi is enabled, [[src-tauri/src/fetcher.rs]] obtains a short-lived `openai-codex` bearer from Pi, derives the ChatGPT account id from its JWT payload, and normalizes the ChatGPT WHAM primary/secondary windows; if that path is unavailable it falls back to the newest `token_count` transcript event in `~/.codex/sessions/**/*.jsonl` so older Codex installs can still populate base usage rows
...
8e. Configured CPA is exclusive for live, cold-cache, web, and tray usage. Inventory preserves runtime-only and file-backed account health. [[src-tauri/src/cpa/poll.rs#refresh]] merges fresh recognized passive windows, schedules fair bounded full reads for incomplete/stale coverage, and persists coherent `usage.cpa.state` plus newly observed history rows atomically. Failed reads retain original timestamps and window sets with typed cached/paused errors. Full success replaces the authoritative window set. History is never restamped as a new live observation. Legacy hydration selects the latest complete account snapshot rather than each window's last historical row.
Configured window polling is an explicit opt-in to Quill-induced off-device transmission: Quill asks loopback CPA to call Anthropic or OpenAI quota endpoints with the selected account token. The management key and upstream token remain local to Rust and CPA, but each scheduled window call is still an external request attributable to Quill's poll cadence. A Codex account whose usage reports banked reset credits adds one reset-credit list read to its window call. Spending a reset adds a profile read and a claim, or a consume call, only when the operator confirms one.
8f. Unconfigured CPA has null impact: no client is constructed, no HTTP request or `cpa_phase_ms` log occurs, and the additive CPA arrays stay empty. A configured source adds a one-way URL fingerprint to the in-memory cache key; the management key never enters that key or any diagnostic. Pool aggregates are recomputed through [[features#Live Usage View#CPA Pool Aggregation]] as per-window means and never stored. Default Codex identities normalize by duration rather than primary/secondary position; scoped limits include the full metered-feature identity so unrelated equal-duration windows cannot collapse. [[src/App.tsx#App]] combines enabled native status with the masked `get_cpa_connection_status` configured flag, so a completed no-source response cannot mark the titlebar live and an empty configured CPA inventory remains an active source.
...
[[src-tauri/src/lib.rs#set_cpa_connection]] holds the usage-refresh lock while validating the loopback inventory and transactionally replacing connection settings/runtime state. Provider capability is checked independently during regular polling, not gated by connect-time smoke results. Endpoint changes purge old CPA history; same-endpoint reconnect retains analytics. Management rejection persists retry suppression until explicit reconnect. Keys stay Rust-owned and never enter responses.

Disconnect acquires the same usage-refresh lock before [[src-tauri/src/integrations/manager.rs#clear_cpa_connection]] transactionally purges CPA connection/runtime/history under `integration_mutation_guard`. Cache invalidation occurs before releasing the refresh lock, so no old poll can repersist deleted rows. Refresh resumes only after release.
