...
Retention deletes transcript-owned model observations older than its confirmed cutoff, but never selects synthetic `pi-push:` rows and retains each source inventory row so reconciliation, revision detection, and cursor progress remain valid. [[src-tauri/src/storage.rs#Storage#replace_model_source]] re-reads the durable watermark in its replacement transaction and filters pre-cutoff `observed_at_ms` rows before inserting, preventing a revised or backfilled source from resurrecting pruned evidence. `usage_snapshots` and `token_snapshots` remain outside this policy: they are live-hook/API state without source replay semantics, and their bounded cleanup lifecycles remain independent.
...
After runtime backfill completes, the Runtime card unions source-owned hourly rows with a provider-agnostic fold of source-less live events over the same window. A partial live timestamp index bounds this branch without changing rollup ownership or pruning invariants.
