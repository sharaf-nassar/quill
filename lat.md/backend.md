...
### Startup migration surface

Database upgrades keep a responsive, explicit startup window instead of leaving the transparent main window apparently hung.

[[src-tauri/src/storage.rs#Storage#startup_migration_plan]] performs a read-only schema check before normal setup. When an existing database is below the supported schema ceiling, [[src-tauri/src/lib.rs#show_migration_window]] hides (but never resizes or moves) `main` and creates a centered, non-resizable, undecorated, transparent, always-on-top 520×300px `migration` webview. The migration label is excluded from window-state tracking and shares the default capability only for normal Tauri IPC. A database already at the current schema keeps the direct startup path and never flashes the migration surface; a read-only probe failure falls back to authoritative read-write `Storage::init`, preserving WAL recovery behavior. If the transient window cannot be created, `main` is shown again and the same direct startup path remains available instead of misreporting a database failure.

The pre-migration copy is the recovery boundary for migrations 46 and 48, which rebuild or re-key persisted analytics tables. SQLite transaction rollback protects ordinary migration errors, while the separately verified pre-change database also protects against interruption, filesystem failure, or a faulty new build; Quill can be stopped and the versioned sibling backup restored before retrying. The exact restore contract is pinned in [[pi-model-usage-tests#Pi Model Usage Test Specs#Analytics Migration Backup Preflight And Recovery]].

[[src-tauri/src/lib.rs#start_storage_migration]] returns from Tauri setup so the webview can paint, then runs `Storage::init_with_progress` on a worker thread. [[src-tauri/src/storage.rs#MigrationProgressCallback]] reports named preparation, backup, verification, publishing, rebuild, finalization, and completion stages without coupling storage to Tauri. During `VACUUM INTO`, a bounded monitor reads the temporary backup file length every 250ms and reports bytes copied against the source database size as an approximate phase denominator; stages without a trustworthy denominator remain indeterminate.

[[src-tauri/src/lib.rs#get_startup_status]] exposes the latest process-local status before storage exists, while the `startup-status` event pushes subsequent changes. [[src/components/MigrationView.tsx#MigrationView]] renders the same state as a compact Flat Polish instrument with stage copy, approximate percentage, bytes, smoothed backup-only ETA, accessible determinate/indeterminate progress semantics, reduced-motion behavior, and an explicit quit action. Button quit and native close terminate directly while startup is not ready, before window-state exit persistence can overwrite the still-unrestored `main` cache. After `ready`, the same close handler allows Rust's programmatic migration-window close instead of exiting the newly initialized app. Once any event has arrived, `StartupGate` ignores an older point-read snapshot. Only after migration succeeds does [[src-tauri/src/lib.rs#finish_setup]] restore persisted `main` geometry, create managed state, start listeners and background work, build the tray, publish `ready`, close `migration`, and reveal `main`; the migration webview renders nothing after `ready`, preventing a transient widget mount.

[[src-tauri/src/lib.rs#initialize_storage_or_report_fatal]] publishes the process-wide storage handle or returns `None`, and the setup path abandons the rest of startup rather than running against absent storage. It never calls `process::exit` itself: a bare exit made a failed migration look like an app that silently refuses to launch. [[src-tauri/src/lib.rs#report_fatal_storage_failure]] instead hides (not closes — a close on the last window requests app exit and would race the dialog away) every window and queues a `tauri-plugin-dialog` error dialog naming the failure and the database folder. The dialog offers Open Database Folder or Quit and exits either way; [[src-tauri/src/lib.rs#fatal_storage_message]] strips the `SCHEMA_TOO_NEW:` marker and substitutes upgrade-or-move-aside guidance because a too-new database is not an integrity failure. The dialog cannot be shown synchronously because setup runs inside the event loop's `Ready` handler and `blocking_show` would freeze the thread it needs. Because termination then hangs off a callback, a session with no working dialog backend would leave a hidden UI-less process alive forever, so a watchdog armed alongside the dialog exits after `FATAL_STORAGE_DIALOG_TIMEOUT` (60s) regardless. Callback and watchdog race for one function-scoped `AtomicBool`, so exactly one terminates and the loser is a no-op.
...
Both writers use WAL, foreign keys, a 30-second busy timeout, and transactional source replacement. Python serializes complete reads and transactions on its shared connection. Purge removes cache references before sources in one transaction. Requests and serialized responses are capped; Rust indexing reads at most the byte budget plus one overflow byte off the executor. Fetch rejects non-global destinations and pins every redirect to validated numeric addresses while preserving Host and TLS identity, without ambient proxies. Execute checks `context_preservation`, canonicalizes its working directory under configured roots, and rejects destructive command patterns. One deadline covers child exit and bounded pipe draining. Unix timeouts kill the process group; Windows kills the direct child and bounds pipe waiting, without claiming descendant cleanup. Large execution results return bounded previews and indexed output references; persistence failure reports the completed command outcome rather than implying retry is safe. See [[pipeline-context-rust-tests]] and [[pipeline-context-python-tests]].
...
[[src/web/httpTransport.ts]] sends the exact `{ cmd, args }` envelope to `POST /api/web/invoke` and maps the closed response envelopes back to Tauri-style promise resolution or rejection. [[src-tauri/src/web_allowlist.rs#is_permitted_command]] is the only command boundary: its exact default-deny match permits the seventeen cache/local-storage monitor reads and excludes `fetch_usage_data`, `refresh_usage_data`, retry/backfill and maintenance commands, every setter, and every `plugin:*` command. No prefix, suffix, alias, or command count widens that boundary. An authenticated command denial is a JSON `403`; host, budget, and session refusals are empty-body `403`s before dispatch.

Desktop configuration serializes as `{ config: { enabled, port, allowlist }, pairing_code }`; status serializes as `{ running, bound_addr, reachable_urls, last_error }`. Reachable URLs use concrete interface IPs, explicit ports, IPv6 brackets, and a trailing slash: the bare origin is correct because an unpaired `GET /` redirects to the pairing page. Settings no longer renders them — it links the bound socket instead — so `reachable_urls` is currently a produced field with no consumer, retained rather than removed because it is part of the published status contract and its round-trip fixtures. Pairing sets `quill_web_session` for 30 days with `Path=/`, `HttpOnly`, and `SameSite=Strict`, omitting `Domain`, `Secure`, and `Expires` for the intentionally plain-HTTP listener. Cross-language payloads and round-trip fixtures are normative in `specs/029-web-ui-server.md#web-transport-protocol-contract`.
...
The defaults are `enabled=false`, `port=19878`, and `allowlist=[]`.
`web_ui.last_error` is listener status rather than writable configuration;
absent means `null`.

`web_ui.host_policy` is retired. Its `all` value bypassed the allowlist
entirely, so one click made the list decorative — a hole, not a setting. The key
is read only to be blanked on the next save, so a downgrade cannot rediscover a
stale `all`; a database still carrying one loses the bypass on load, leaving the
allowlist as the only answer and an empty one as loopback. Migration is closed
by omission.
...
`QUILL_CONTEXT_PORT`. The allowlist accepts IPv4/IPv6 literals or RFC-1123
hostnames; wildcards, malformed entries, and more than 64 entries fail at the
settings boundary. Accepted entries trim surrounding space, lowercase
hostnames, normalize IP spelling, sort, and deduplicate.

CIDR ranges are now rejected. An entry is a name a browser may put in a URL, and
a `Host` header carries one name — no browser sends `/24`. Ranges were coherent
while the list filtered peer addresses; under a name check they could only be
stored as entries that never match, so they fail at entry instead.
...
Empty or loopback-only allowlists bind `127.0.0.1`; the first entry naming
anything else binds `0.0.0.0`. Listing a name only another device would use is
the act that asks for a network-visible socket, so the bind follows the list
rather than a separate switch, and the feature stays closed until asked for. Disable persists first and then
...
fallback, so no route can be mounted outside the host, budget, size, and time
bounds.

Host, request-budget, and session refusals are empty-body `403`s decided before a route reads Quill data, which makes "not a partial render" hold for pages, assets, and invokes. Malformed invokes remain empty-body `400`s, oversized bodies `413`s, and a timed-out request `408`.

The allowlist is compared against the request's `Host` name, lowercased with
any port stripped, plus the implicit `localhost`, `127.0.0.1`, and `[::1]` so
the desktop's own link works whatever is configured. Nothing is resolved: an
entry is matched as a name, so this machine's `/etc/hosts`, DNS, or search
domain cannot widen or collapse it. The gate adopts the durable commit, so a
failed transition keeps the previous set.

This is a rebinding defence, not access control — a hostile domain pointed at
this address is refused, but any device that can route to the port may still
connect using an allowed name, and the pairing credential is what stops it.
Rate limiting still keys on the accepted socket's address delivered through
Axum `ConnectInfo`, which no header can forge. The authenticated class still
verifies a `quill_web_session` cookie through
...
The entry document is the exception, and it is mounted separately for that reason. `/` is the address a human types or bookmarks, so an unpaired navigation is answered `303` to `/pair` by [[src-tauri/src/web_server/router.rs#pair_or_document]] rather than refused — making the user find a bootstrap-only URL is a worse contract than the listener noticing for itself. [[src-tauri/src/web_server/router.rs#pair_page]] closes the loop from the other side, answering `303` to `/` when [[src-tauri/src/web_server/gates.rs#session_is_live]] already recognizes the cookie; the two redirects fire on opposite session states and so cannot cycle.

Nothing else moved. `/assets/*`, the invoke route, and the fallback keep the empty-body `403`, because a redirect is only meaningful for a navigation and would be nonsense as an answer to an asset or an RPC. The redirect discloses nothing new either: `/pair` is already public to the same peer, and the peer filter and per-peer budgets still run ahead of it, so a denied peer is refused on `/` exactly as before. The document itself is still never served without a session.
...
[[src-tauri/src/web_server/assets.rs]] serves the browser bundle's asset graph
inside the authenticated request class, so an unpaired peer never receives a
chunk. The entry document it also renders is mounted by the router under the
redirecting gate instead, since `/` is a navigation.
...
| Pi session messages | 4,000 req/min |
...
| POST | `/api/v1/pi/track` | Ingest versioned Pi lifecycle events |
...
`response_times`, `skill_usages`, `hook_invocations`, and
`session_setting_events` are inserted unfiltered there and keep full history —
they are not retention targets, and
...
The siblings are `response_times`, `skill_usages`, `hook_invocations`, and
`session_setting_events`; the registry assertion covers its fingerprint,
nullable session name, status, and generation. Surviving tool rows retain their
nullable error, details, image-count, and duration evidence exactly; suppressed
rows leave none of those columns behind.
...
The same `tool_actions` insert loop binds NULL for `full_input`, `full_output`,
and migration-48 `details_json` whenever the row's category is
...
The grep behind the original payloads is closed. The only SQL readers of `full_input` or `full_output` are
...
The row itself stays. `get_session_breakdown` scans and `COUNT(*)`
`tool_actions` with **no** category filter, so dropping `tool_detail` rows
would deflate per-agent tool-call counts. Omitting the three payload columns keeps every
row-shaped invariant — row count, `action_key` uniqueness, `agent_id`,
`message_id`, timestamps, and nullable error/image/duration evidence — and costs only the dead bytes.
...
count: the three payloads that must vanish and the row plus non-payload evidence
that must not.

##### Tool Detail Rows Land Without Payloads

A `tool_detail` row remains present while its three payload columns read back NULL.

Given one `tool_detail`, one `code_change` and one `command` row that all carry
payloads, the other two keep their values. `is_error`, `result_image_count`, and
`duration_ms` remain bound on every category.
...
The database schema is versioned through migration 49 and includes usage, token, model analytics, context savings, learning, rule governance, session indexing, memory optimizer, code, runtime, retention aggregates, and metadata tables.
...
- **token_snapshots** — Raw token usage per provider/session (provider, session_id, hostname, timestamp, input/output/cache tokens, cwd, observation kind). Migration 48 adds `observation_kind IN ('turn','summary')` with legacy/default rows fixed to `turn`. Indexed on provider-aware timestamp, session, and cwd paths.
- **token_hourly** — Hourly aggregates per provider/host (total tokens, turn_count). Summary tokens enter totals while only `turn` snapshots increment `turn_count`. Unique on (hour, hostname, provider).
...
Migration 20 added `is_sidechain`, `agent_id`, and `parent_uuid` to `token_snapshots` for provider-agnostic sub-agent attribution; the [[backend#Tauri IPC Commands#Usage and Token Commands (17)]] `get_session_breakdown` rollup aggregates across all sidechain rows by `session_id` so a sub-agent's tokens count toward its parent session row. Hook-reported snapshots written before migration 20 stay tagged `is_sidechain=0` (a future CLI repair utility is documented as a TODO in [[src-tauri/src/storage.rs]]).

Pushed Pi usage writes `turn` snapshots for known lifecycle origins, gated once by model-observation event UUID. Retained Pi reconciliation also writes assistant and summary snapshots under one canonical source replacement; summary rows contribute spend to token history, provider totals, and cleanup aggregates but never fabricate turns in token stats or host/project breakdowns.
...
- **model_usage_observations** — Normalized turn, token, and summary facts with exact raw model identity, a nullable indexed `derived_model_id` attribution column, nullable token dimensions, resolved session/chain ownership, and source-local ordering. Pi assistant rows store the validated upstream `provider/model`, nullable reasoning tokens, normalized-or-bounded stop reason, and measured error presence. Pi `compaction` and `branch_summary` rows use stable `pi_summary_v1` source identity, direct token evidence, nullable compaction `tokens_before`, and missing model evidence unless the entry explicitly supplies a valid provider/model pair; reasoning stays an informational dimension and is never added to token or cost totals. Migration 43 adds nullable Pi event UUID and native input/output/cache-read/cache-write/total cost fields plus a Pi-only partial unique event index. Migration 48 transactionally rebuilds the table to extend `observation_kind` to `turn`/`token`/`summary`, preserves every row and named index, and adds nullable `reasoning_tokens`, `stop_reason`, `had_error`, `tokens_before`, and `reasoning_duration_ms`.
...
Retention deletes source-owned model observations older than its confirmed cutoff, including active host-qualified Pi detail. The owner stays active and registered, while every later Pi usage insert checks the durable watermark before restoring raw evidence. Each source inventory row survives so reconciliation, revision detection, and cursor progress remain valid. [[src-tauri/src/storage.rs#Storage#replace_model_source]] re-reads the same watermark in its replacement transaction and filters pre-cutoff `observed_at_ms` rows before inserting. `usage_snapshots` and `token_snapshots` remain outside this policy because their bounded cleanup lifecycles are independent.
...
Codex `turn_context.payload.model` is retained as tokenless per-turn evidence by the model adapter, not Session Search. Live agent model resolution uses it only when an unresolved Codex agent exactly matches a retained child `chain_id` under the same provider and analytics root; missing exact matches stay unknown.
...
[[src-tauri/src/storage.rs#CacheKey]] identifies command-specific request dimensions and a 30-second wall-clock bucket so sliding windows naturally expire at a bucket boundary. [[src-tauri/src/storage.rs#get_or_compute]] checks the 45-second TTL and a [[src-tauri/src/storage.rs#TableVersions]] fingerprint made from indexed high-water markers. Model overview also fingerprints `rollup_meta.rollup_generation`; bucket and context commands keep their own probes. Every access evicts entries older than 45 seconds before probing, including failed-probe paths. Insertion retains at most 64 request variants per typed cache, evicting the oldest inserted variant when needed. A failed probe bypasses the cache and computes fresh. Each [[src-tauri/src/storage.rs#Storage]] owns its typed caches.

[[src-tauri/src/storage.rs#Storage#get_model_usage_overview]] serves the Models-page and Usage-chart overview from one read-only deferred-transaction snapshot as a [[src-tauri/src/models.rs#ModelUsageOverviewResponse]]. During rollup build it preserves the established raw temp-table path. After completion, `scoped_overview` contains aggregate-grain closed UTC hours plus raw leading/current-hour rows, so it never rematerializes the full closed observation range; provider filtering remains sargable. The response carries range totals with input/output/cache dimensions, a `summary_usage` bucket (count and token total of unattributed summary-kind observations, read from raw rows in both rollup modes because hourly rows cannot split token amounts by kind — summaries whose raw rows were retention-pruned stay in totals but leave the bucket), per-model reach rows, a running-now entry per provider, fixed-bucket per-model session and token activity, model-less token activity by CLI provider, project × model session data, model combinations, and parent-versus-subagent token totals. The Usage view re-groups those token buckets by recorded CLI, Pi upstream prefix, or exact provider-qualified model without changing the total. Exact facet routing and non-decomposable exceptions are inventoried in [[backend#Backend#Database#Schema#Hourly Analytics Rollups]].
...
##### Model Source Nickname Persistence
...
- **runtime_turn_state** — One row per provider/source bookmark. Transcript rows keep the highest finalized raw rowid and open start; live rows also persist the processed rowid plus open start, last time/kind, and retention conformance. Open turns are not folded into `runtime_hourly`.
...
After runtime backfill completes, the Runtime card combines source-owned hourly rows, compact active host-qualified Pi open-turn state, and a provider-agnostic fold of source-less live events over the same window. Sessions reads the same Pi state per selected session, so active runtime and current-turn rate remain measured without raw-tail scans. Each accepted in-order Pi event advances state and adds newly closed turns to hourly authority; a late event refolds retained rows once for exact replay. Closing Pi seals state directly. The global transcript backfill preserves live rollups and state because live rowids may interleave with its transcript-only contiguous blocks.
...
- **tool_actions** — Tool invocation details behind `get_code_stats`, `get_batch_session_code_stats` and sub-agent discovery (provider, message_id, session_id, tool_name, category, file_path, summary, full_input/output, plus `is_sidechain`, `agent_id`, and `parent_uuid` from migration 20, nullable `lines_added`/`lines_removed` from migration 33, and nullable `is_error`, `details_json`, `result_image_count`, and `duration_ms` from migration 48). Indexed on provider/session, message_id, file_path, category, and the provider+session+sidechain / provider+session+agent pairs. `full_input` is truncated to 10KB, so the `lines_added`/`lines_removed` counts for `code_change` rows are computed at ingest from the untruncated input. The code-stats queries select those persisted counters directly and conditionally project `full_input` only when either counter is NULL; these legacy rows keep the tolerant parser while migrated rows never materialize the payload. Retained transcript rows are committed through source-owned snapshot replacement, and Pi's through the narrower notify writer that stands in for it; rows written either way with `category = 'tool_detail'` carry NULL in `full_input`, `full_output`, and `details_json`, while error/image/duration evidence survives the carve-out — see [[backend#Backend#Database#tool_detail payload carve-out]]. The Pi retained parser fills `is_error`, `details_json`, and `result_image_count` from the matching `toolResult` entry ([[src-tauri/src/sessions.rs#extract_pi_session]]) and `duration_ms` from `tool_span` receipts ([[src-tauri/src/transcript_analytics.rs#pi_span_durations]]); Claude and Codex producers leave all four NULL.
...
Live Codex rows and Claude root terminal rows are inserted by [[src-tauri/src/storage.rs#Storage#store_hook_observation]] from the `POST /api/v1/hooks/observed` background task. Codex identity remains event-scoped (`hook_event` with optional `:tool_name`) because its generic observer cannot identify sibling scripts. Other Claude rows come from transcript attachments. Both live observer paths are gated by the existing `activity_tracking` feature and preserve user-owned hook configuration.

The provider-neutral endpoint accepts Codex's known events plus Claude StopFailure and length-caps identity fields. [[src-tauri/src/models.rs#ObservedHookObservation]] carries only what an audit row stores — provider, session, optional hostname, event, tool, cwd, matcher, agent id, and producer time — and ignores extra fields so older payloads stay audit-compatible. Sessions projects the newest valid root Stop, Claude StopFailure, or SessionEnd per provider, session, and host after retained and observed-only rows merge; this projection does not change activity ordering. An open Claude agent is named from its own transcript by the live fold, so a Sessions read schedules no rescan to resolve one. Retained model reconciliation's committed `model-analytics-updated` event still invalidates Sessions directly.

Rust integration config generation and retained reconciliation share [[src-tauri/src/sessions.rs#SessionIndex#local_hostname]], including environment, platform, file, and command fallbacks. [[src-tauri/src/live_tracker.rs#SessionKey]] keys the live fold by provider, trimmed short-form ASCII-case-folded hostname, and root session, then agents by `agent_id`; retained display hostnames remain unchanged. An agent leaves the fold only when its own closure evidence lands, and [[src-tauri/src/live_tracker.rs#LiveTracker#sweep]] releases a session 15 minutes after its newest transcript byte — five default three-minute usage-poll intervals. An evicted session and a row the fold never covers, such as every remote host, both read as null rather than a false zero.

Known limitation (Claude side): outside managed root-terminal observers, transcript extraction sees only `hook_*` attachments, so a hook that leaves no attachment record is invisible to the Hooks breakdown.

#### Working Context Store

The MCP context store keeps large transient context out of the analytics database.

The Python MCP tools in [[src-tauri/claude-integration/mcp/tools/context.py]] create `~/.config/quill/context/context.db` with `sources`, `chunks`, `executions`, and `fetch_cache` tables. SQLite FTS5 is used when available, with a LIKE fallback so older SQLite builds still search indexed chunks. Context data stays on the machine running the MCP server.
...
Every event carries a `category` from a closed taxonomy: `preservation` (content written to the MCP context store and kept out of the LLM transcript), `retrieval` (preserved content read through `quill_get_context_source`), or `routing` (router guidance, search snippets, and bounded `quill_execute` results — transcript cost, not savings). The canonical mapping lives in [[src-tauri/src/context_category.rs#derive_category]] and is mirrored by `deriveCategory` in `src-tauri/claude-integration/scripts/context-telemetry.cjs` and `_derive_category` in [[src-tauri/claude-integration/mcp/tools/context.py]]. Producers set `category` explicitly, the server derives it from `(eventType, decision)` only as a fallback, and [[src-tauri/src/storage.rs#backfill_context_event_categories]] applies the mapping to historical rows. Migration 19 re-runs that backfill and zeroes saved/preserved token fields outside preservation/retrieval.

The HTTP server accepts batches from routing hooks and MCP tools, deduplicates with `INSERT OR IGNORE`, and emits `context-savings-updated`. Analytics queries aggregate by time bucket, provider, category, event type, source, decision, and cwd while leaving large content in the MCP store. `CONTEXT_SAVINGS_AGGREGATES_SQL` sums byte and token-indexed/returned columns across every event, while saved and preserved tokens remain gated to `category IN ('preservation', 'retrieval')`. `CONTEXT_SAVINGS_CATEGORY_TOTALS_SQL` reports preserved, retrieved, and routing totals; `CONTEXT_SAVINGS_RETENTION_SQL` computes `sources_retrieved / sources_preserved` over in-window distinct `source_ref` values.
...
`transcript_analytics_sources` stores canonical transcript ownership plus explicit `source_kind` identity for lifecycle-owned live sources and migration-48 nullable `session_name`. `live_analytics_sessions` stores project, cwd, host, ordered lifecycle time, and closure state. Migration 42 adds Pi's retained `ephemeral` compatibility column; migration 43 adds pushed usage identity and store-only costs. Migration 44 migrates prior Pi NULL-runtime and `pi-push:` evidence to `live:pi:<session-id>`, and migration 45 compacts each existing Pi owner once. Migration 46 rekeys all Pi-owned runtime/model/tool/skill/aggregate rows to the unambiguous `pi:session:v1:<hex-normalized-host>:<hex-session-id>` owner, changes the root to `pi:sessions`, seeds prior open rows as `recovering`, and arms persisted-source reconciliation plus spool cleanup. Migration 48 arms `pi_transcript_analytics_reingest_pending` without setting the global all-provider marker, so new Pi evidence backfills without forcing unchanged Claude or Codex roots.
...
Migration 46 creates `pi_session_lifecycle` and `pi_event_receipts`. Persisted open sessions enter lifecycle storage and `live_analytics_sessions` as `recovering`; their own process's hook telemetry reopens them, their own later end closes them, and silence past the idle window closes them as a crash. At server startup [[src-tauri/src/storage.rs#Storage#load_pi_recently_closed_sessions]] selects closed Pi rows no older than [[src-tauri/src/live_tracker.rs#IDLE_AFTER]] and seeds tracker tombstones after recovering rows, preventing still-recent files from reopening the completed occurrence. Migration 47 drops the obsolete reporter-health table and saturation settings. Migration 48 generalizes the backup routines to any exact pre-migration schema, requires twice the database file size in free-space headroom, publishes and verifies `<usage.db>.schema-47.backup`, rebuilds model observations in one transaction with row/index assertions, and leaves failed rebuilds at schema 47 for retry or restore. Exact recovery commands and measured evidence are pinned in [[pi-model-usage-tests#Pi Model Usage Test Specs#Analytics Migration Backup Preflight And Recovery]] and [[pi-model-usage-tests#Pi Model Usage Test Specs#Analytics Migration Measurement]]. The durable `pi_spool_cleanup_pending` marker disables production legacy-spool import while persisted-source reconciliation owns readiness for artifact deletion.
...
[[src-tauri/src/transcript_watcher.rs#start]] owns startup whole-root reconciliation independently of Session Search or Analytics-window mounting. It folds current live transcripts first, then requests the blocking historical pass on its one retained-scan worker; later recovery and search scans coalesce on that same bounded scheduler, so retained parsing never blocks watcher folds or the unconditional live sweep. An existing empty root proves an empty inventory; a missing, unreadable, or unavailable configured root is incomplete and cannot authorize pruning or marker clearance.

Reconciliation compares canonical source key/path and last-good status before reading content. Matching nanosecond mtime and size skip unchanged sources. Claude/Codex can also skip replacement after an equal-content hash; Pi streams and parses changed versions together. Migration 49 adds separate serialized rejection checkpoints to transcript and model registries, preserving successful fingerprints, identities, digests, and child rows. Legacy failed rows have no current-policy rejection, so they receive one fresh attempt without rewriting existing metadata during migration.

Stable content failures store canonical path, attempted fingerprint, parser policy, and a bounded diagnostic. Matching rejections settle both live and retained work without decoding or rewriting diagnostics. Source changes or a parser-policy increment rearm them. I/O, graph, drift, and storage errors remain retryable and cannot poison a source checkpoint. Global and Pi-scoped reingest markers clear after complete inventory and settled source attempts, including current-policy rejections; transient failures and incomplete roots keep them armed. Rejection is not successful ingestion or deletion suppression.
...
[[src-tauri/src/storage.rs#Storage#refresh_unchanged_transcript_analytics_sources]] is now reached only by the live single-source path, whose one refresh is already one transaction. It stays batch-shaped for the part callers depend on: it returns the source keys whose rows did not update because the root generation moved under a concurrent run, so a stale-generation verdict remains per source rather than one aggregate. Production calls [[src-tauri/src/storage.rs#Storage#refresh_unchanged_transcript_analytics_source]], which collapses that one-source stale-key set to a boolean; the batch form remains the storage-level test contract.

`replace_transcript_analytics_snapshot` replaces all six owned analytics tables, including `session_setting_events`, and the source registry in one transaction; Pi snapshots extend that same transaction across lifecycle, receipts, turn/summary token snapshots, native model observations, unpruned model rollups, and the model source registry. Summary observations and snapshots are replaced by stable entry identity, enter every token aggregate, and contribute zero to turn counts. Valid empty snapshots remove only that source, while suppression, identity drift, stale generation, or any final insert failure leaves every prior row intact. Owned inserts use `INSERT OR IGNORE` through prepared statements, and generation guards reject stale plans before rows change. The `session_events`, `tool_actions`, and Pi usage inserts obey the retention watermark so delete-and-reinsert cannot resurrect pruned history. Shared tool/skill helpers keep notify and retained keys identical.
...
[[src-tauri/src/storage.rs#Storage#replace_pi_transcript_tool_rows]] remains the notify fast-path writer for `tool_actions` and `skill_usages`. It uses the same canonical host-qualified source key and shared insert helpers as retained reconciliation, so a later authoritative snapshot replaces identical ownership rather than creating a second row family. It deliberately touches no runtime, lifecycle, receipt, token, or model table; the shared coordinator owns those source-complete replacements.

Session, project, host, and completed-root deletion remove every source-owned analytics table, including `session_setting_events`, in the owning transaction. A retained project/host match expands through provider-qualified analytics roots before leaving every sibling source as a suppressed tombstone. Source-less project/host rows use only exact recorded live origin; direct session deletion also catches unmapped legacy live rows. Committed deletions emit `transcript-analytics-updated` only when transcript rows changed.
...
[[src-tauri/src/storage.rs#Storage#replace_transcript_analytics_snapshot]] is one transaction across all six owned tables plus the registry, so a failure at the last statement must undo every delete and insert before it.

A replacement that violates the registry `CHECK` after the owned tables were already rewritten must restore the prior rows exactly, leave a sibling source of the same root untouched, and leave the registry generation unadvanced. Pi coverage repeats this at the wider boundary: lifecycle, receipts, runtime, response, tool, skill, native usage, token, rollup, and both registry families all roll back together. The positive half asserts the other edge: an empty snapshot is a legal replacement that clears exactly one source and no sibling.
...
##### Migration 39 Codex Backfill Scope

Migration 39 deletes only Codex rows that retained transcripts can replay.

Codex rows disappear from session events, response times, tool and skill facts, model observations and hourly rollups, both source registries; Claude rows survive, and hook invocations plus retention daily aggregates remain byte-authoritative.

##### Migration 39 Idempotence

Reopening a database that already ran migration 39 must not repeat its deletes or column addition.

The version gate records one v39 row and leaves the durable reingest marker for the later successful Session Search sweep.

##### Migration 41 Codex Replay Scope

Migration 41 replays only active Codex transcripts through the modern message extractor while preserving last-good analytics until replacement succeeds.

Active `ok` Codex source rows become `stale`, retaining fingerprints and child rows; suppressed, failed, and Claude sources remain unchanged. The provider-scoped Session Search marker is rearmed, and one v41 row makes restart idempotent.

##### Modern Codex Turn Extraction

Modern Codex `response_item` user and assistant messages produce retained response pairs, while mixed-format transcript copies do not inflate turn counts.

User `input_text` and assistant `output_text` become messages and runtime events. Adjacent cross-format messages with equal role and text within 10 ms coalesce in favor of `response_item`; unmatched legacy messages remain authoritative.
...
Stable invalid UTF-8, resource budgets, native identity, and Pi protocol parse failures are content-deterministic. Metadata I/O, storage, graph, and drift failures have no rejection checkpoint. Live and retained attempts use the same classification.

The fixture marks a cleanly parsing transcript `failed` at its current fingerprint, so an `ok` status after the next pass would prove it was reparsed. The status and diagnostic sentinel must stay untouched. Appending moves the fingerprint and retries in full. A current-policy rejection remains settled during forced reingest; a parser-policy change requires a new attempt.
...
A discovered source seeded behind the pass generation keeps its row, generation, and all six owned tables; an undiscovered source at the same generation loses all of them. A source stamped past the pass generation — a live commit racing the pass — survives even though the pass never enumerated it.
...
Sibling sub-agents whose windows overlap in wall-clock time must not be merged into one interval, and no chain may be counted twice. The same fixture pins the `INDEXED BY idx_se_timestamp_chain` plan for range analytics. Session projection instead reads only listed provider/root/host candidates through retained source, hourly, turn-state, and event indexes. Lifetime family totals stay null until runtime backfill completes and remain independent of the selected token range. An observed agent's exact successfully reconciled source can publish its own runtime while that global historical backfill is incomplete or failed; failed sources and absent chains stay null. The same covered chain loop separately counts distinct sidechains and sums only their runtime; root runtime never enters those agent fields. Root current-turn evidence comes only from the root open tail. At one backend timestamp, root and observed-open sidechains advance under the 5-minute continuity or 6-hour tool-wait cap. Terminal and timeout cases freeze accrual, while concurrent accruing chains contribute independently.
...
Codex extraction maps legacy user and agent text, modern user `input_text` and assistant `output_text`, reasoning, function/custom calls, and call outputs into the five runtime event kinds. Developer, administrative, and empty message items do not become runtime text events. Stable native identities or source record ordinals keep source replay deterministic.
...
Two storage-independent commands keep the migration webview responsive before normal managed state exists.

[[src-tauri/src/lib.rs#get_startup_status]] returns the process-local startup snapshot, and `quit_during_startup` exits directly so neither the button nor a native close can persist the hidden main window's unrestored geometry.

### Web UI Commands (4)

The 99 entries in [[src-tauri/src/lib.rs]]'s `tauri::generate_handler!` include four desktop-only Web UI commands: configuration read/write, live listener status, and pairing-code rotation.
...
### Usage and Token Commands (17)

Live usage and token analytics commands back provider quota, history, breakdown, and context-savings views.

`fetch_usage_data`, `refresh_usage_data`, `get_cached_usage_data`, `use_limit_reset`, `get_usage_history`, `get_snapshot_count`, `get_token_history`, `get_token_stats`, `get_activity_series`, `get_widget_activity_stats`, `get_token_hostnames`, `get_host_breakdown`, `get_session_breakdown`, `get_skill_breakdown`, `get_skill_project_breakdown`, `get_hook_breakdown`, `get_context_savings_analytics`.

[[src-tauri/src/lib.rs#use_limit_reset]] is the one usage command that spends account state: it redeems a banked limit reset for a direct login or a CPA account and returns refreshed usage, as described in [[features#Features#Live Usage View#Limit Resets]]. The browser allowlist denies it by default.
...
[[src-tauri/src/storage.rs#Storage#get_activity_series]] counts distinct `session_id` and distinct `cwd` per bucket. Those counts are distinct *within* a bucket and deliberately do not sum to a range total — a session spanning three buckets is counted in each — and snapshots with no `cwd` are left out of the project count instead of being folded into an invented "unknown" project. The widget's readout grid no longer draws these two series; the Usage Explorer is their remaining consumer.

[[src-tauri/src/storage.rs#Storage#get_widget_activity_stats]] answers the widget's Tools, Turns, Prompts, and Reasoning readouts as one [[src-tauri/src/models.rs#WidgetActivityStats]] on the same bucket grid, from three grouped reads in one deferred view-reader snapshot. Tool calls count every `tool_actions` row in range with `COUNT(is_error)` as the failure-rate denominator, so a range whose providers record no outcome (Claude and Codex rows currently carry NULL) reads as unmeasured rather than as a 0% failure rate. Prompts count root-chain `user_text` session events (`is_sidechain = 0`), because a subagent's prompt is the agent's message rather than the operator's, with distinct provider-qualified sessions as the per-session denominator. Turns and reasoning read `model_usage_observations` turns under the active-source predicate every other model-evidence aggregate applies; `SUM(reasoning_tokens)` stays `None` when no in-range turn reported the dimension, and the reasoning share's denominator is the output of reporting turns only, never every provider's output. Tool and prompt rows are raw and unaggregated, which is safe because retention's 30-day floor matches the 30-day cap on widget ranges.

Range-scoped reads share one displayed vocabulary — `1h`, `6h`, `24h`, `7d`, `30d`, and (where supported) `all`. `get_token_stats`, `get_host_breakdown`, `get_project_breakdown`, `get_session_breakdown`, `get_skill_breakdown`, `get_skill_project_breakdown`, and `get_hook_breakdown` take a `range` string instead of the earlier `days: i32`, so hour-granular selections are not rounded up. [[src-tauri/src/storage.rs#range_to_duration]] also enumerates only four internal comparison ranges: `2h`, `12h`, `2d`, and `14d`. Token, code-history, and runtime readers share that duration helper, making each internal lower bound exactly twice its displayed window without accepting arbitrary range grammar. Code-history bucket widths divide those windows at the current/prior midpoint, while comparison token rows stay intact so downsampling cannot fold evidence across it. `ModelRange` retains its separate public enum and gains no internal comparison variants. Only `learning.rs` still requests one of these (`14d`); the widget dropped its comparison fetch when the readout trend chips were removed, so the remaining three are an accepted grammar with no current caller rather than a live path.

Claude live usage comes from `https://api.anthropic.com/api/oauth/usage?cedar_ember=1` via [[src-tauri/src/fetcher.rs#fetch_claude_usage]] using the local OAuth token and Claude Code's `claude-cli/<version> (external, cli)` user agent from [[src-tauri/src/config.rs#claude_user_agent]], never below the verified reset-capable version floor; the same response carries the banked resets parsed by [[src-tauri/src/fetcher.rs#parse_claude_resets]]. If that native path fails while Pi is enabled, [[src-tauri/src/integrations/pi.rs#oauth_bearer_token]] asks the Pi CLI for an `anthropic` bearer with at least five minutes remaining and retries the same API; Quill never reads or writes Pi's refresh token. [[src-tauri/src/fetcher.rs#parse_buckets]] reads the flat top-level keys — `five_hour` ("5 hours") and `seven_day` ("7 days") still drive the aggregate windows and the tray indicator's short/weekly metrics. The API moved per-model weekly limits out of the flat `seven_day_sonnet`/`_opus`/`_cowork`/`_oauth_apps` keys (now returned as `null`) into a structured `limits` array; [[src-tauri/src/fetcher.rs#parse_scoped_weekly_limits]] reads each `kind: "weekly_scoped"` entry and emits a `UsageBucket` labeled by `scope.model.display_name` (e.g. `Fable` from the codenamed `omelette` slot), keyed `weekly_scoped_<model>` with `sort_order: 1`, deduped by label against the flat buckets. [[src-tauri/src/cpa/quota.rs#parse_claude_usage]] reuses that normalization, account-qualifies each key, and maps the legacy `iguana_necktie` fallback onto `weekly_scoped_fable` so mixed CPA account response shapes still aggregate into one Fable window. The `session` and `weekly_all` limits are skipped because the flat keys already cover them. Because the API returns dropped keys as `null`, their last snapshot would otherwise linger as a ghost tile in the cached live view; [[src-tauri/src/storage.rs#Storage#get_latest_usage_buckets]] prunes any bucket whose newest snapshot is more than an hour older than the provider's most recent fetch (buckets from one fetch share a timestamp, so a paused provider prunes nothing), while history and stats queries read snapshots directly and are unaffected.

Codex live usage now comes from `codex app-server` `account/rateLimits/read` instead of transcript-only scraping. The backend normalizes the returned `rateLimitsByLimitId` map into provider buckets so Quill can store both the base Codex windows and model-specific limits such as Codex Spark in the same usage tables, while preserving the legacy base Codex bucket keys for history continuity. Model-specific `limitName` values are abbreviated for display via [[src-tauri/src/fetcher.rs#abbreviate_codex_model]]; Codex Spark limits omit the model version (`GPT-5.3-Codex-Spark` → `Spark`), while other names drop the redundant `GPT-` prefix and `-Codex` infix. The fetcher shares the one-shot stdio helper described in [[infrastructure#Infrastructure#Codex Integration Deployment#App-Server Request Contract]], which resolves the Codex executable path, augments the user's login-shell `PATH` with the launcher and symlink-target directories so Node-backed npm installs still start from desktop-launched Quill, ignores unrelated frames such as the `initialize` response, and deserializes only the matching request id. Usage polling passes a thirty-second deadline because the request round-trips to the ChatGPT backend. If the direct app-server request fails while Pi is enabled, Pi exports a short-lived `openai-codex` bearer and Quill sends it plus the JWT payload's ChatGPT account id to the WHAM usage API, normalizing its primary and secondary windows. If that fallback is unavailable, the fetcher uses transcript `token_count` `rate_limits`. Only the app-server read reports `rateLimitResetCredits`, so [[src-tauri/src/fetcher.rs#fetch_codex_usage]] returns resets as unknown from either fallback and the poller keeps the last reported ones.
...
`get_session_breakdown` remains provider-agnostic at the row level and rolls up parent plus subagent token and response rows. The retained base query initializes family and agent runtime totals plus `observed_agents` to null with `observed_only: false`. The Tauri command passes the live tracker's folded identities into retained selection, then overlays that fold by exact provider and root session plus trimmed short-form ASCII-case-folded hostname. Matching retained rows gain exact open-agent membership and valid current-process activity without changing their display hostname, including rows outside the initial SQL limit and rows without cwd; only roots with no retained row require a range-matching folded session with validated root cwd to synthesize, carrying `observed_only: true`. The newest valid root Stop, Claude StopFailure, or SessionEnd is projected afterward, followed by lifetime runtime from completed hourly rollups plus open tails. Family runtime sums every native chain; agent count and runtime include retained sidechains only; each observed agent receives only its exact successfully reconciled chain runtime and accrual bit even if unrelated historical runtime backfill is incomplete or failed. Failed or absent observed chains remain unknown. Lifetime aggregates still publish only after runtime backfill completes. Source-less Pi sessions deliberately keep family and agent runtime evidence null rather than reporting zero. Provider, session, and hostname remain isolated; a measured `0` is preserved and missing evidence stays null. Invalid, stale, filtered, or out-of-range roots never synthesize; terminal projection can immediately mark a newly synthesized row inactive. The base SQL contains no live-agent, runtime, retention, or hook-audit enrichment.

The query first materializes range-, provider-, and hostname-scoped token groups and ranks them with a range-bounded indexed response-time maximum. It enriches the requested top-N plus exact retained identities the tracker holds live state for; the overlay then applies live activity, deterministic ordering, and the final limit. That same enrichment phase fills `model_id` with each candidate's ranked primary model from `model_usage_observations` — the token-then-turn-ranked derivation [[backend#Backend#Database#Schema#Model Analytics Evidence]]'s `get_model_sessions` uses, correlated per candidate rather than re-scanning the corpus, so it never touches the ranking stage above it. A retained row keeps this label after aging out of live coverage; the overlay's own live-folded model still outranks it whenever the fold has folded one, since that is newer evidence. After the overlay, [[src-tauri/src/storage.rs#Storage#populate_session_analytics_evidence]] joins two nullable fields onto every final row, live-only rows included: `session_name` from the analytics registry through the same bounded chunked lookup search enrichment uses, and the range-scoped `failed_tool_calls` count from `tool_actions.is_error` — NULL when no in-range tool row carries error evidence, because unmeasured is not zero failures. Historical transcript and analytics storage remain available to their owning readers but do not project current agent or runtime state.
...
Model analytics IPC exposes overview, outcome-aggregation, paged-session, session-detail, and backfill operations through one structured, user-safe error contract.
...
[[src-tauri/src/lib.rs#get_turn_outcomes]] validates the same fixed range and optional provider, then aggregates [[src-tauri/src/storage.rs#Storage#get_turn_outcomes]]: turn-kind rows carrying persisted `stop_reason` or `had_error` evidence, partitioned per session, per attributed model (a null model id is the factual pre-attribution bucket), and per fixed overview-width window, each grouping summing back to the response totals. Denominators count only NOT NULL evidence — `stopReasonTurns` for aborted/error/length rates and `errorEvidenceTurns` for the error flag — so providers that do not yet record outcomes are excluded rather than read as success, and windows without evidence are omitted rather than reported as zero rows. Outcome columns exist only on raw observations, so evidence pruned with its raw rows leaves the aggregation.
...
Provider setup state survives app restarts in the settings table. The downgrade-readable `integration.providers.v1` array contains only Claude, Codex, and MiniMax; `integration.provider.pi.v1` stores Pi separately. Current readers merge both keys, prefer Pi's sidecar over a mixed legacy Pi entry, and parse legacy entries independently so unknown providers cannot discard known choices. The writer updates both keys atomically, which lets a pre-Pi build change legacy providers without erasing Pi. Three global feature flags — `context_preservation.enabled` (default false), `feature.activity_tracking.enabled` (default true), and `feature.context_telemetry.enabled` (default true, gated on context preservation) — drive which optional Quill assets get deployed into Claude Code and Codex.
...
`set_cpa_account_enabled` toggles one account's CPA routing, as described in [[features#Features#Settings Window#CPA Account Controls]].
...
`get_provider_statuses`, `rescan_integrations`, `confirm_enable_provider`, `confirm_disable_provider`, `get_context_preservation_status`, `set_context_preservation_enabled`, `set_minimax_api_key`, `set_cpa_connection`, `set_cpa_account_enabled`, `clear_cpa_connection`, `get_cpa_connection_status`, `get_runtime_settings`, `set_runtime_settings`, `get_integration_features`, `set_activity_tracking_enabled`, `set_context_telemetry_enabled`, and `set_brevity_enabled`.

At startup, [[src-tauri/src/integrations/manager.rs]] verifies enabled, detected Claude and Codex providers against the stored context-preservation setting. Codex verification resolves the install-state home, parses semantic feature/MCP values, and requires `hooks/list` to return every exact Quill handler enabled and trusted; substring matches no longer count as installed. Hook discovery uses a process-only local-provider override, so verification and repair do not initialize model-provider authentication or change the saved provider configuration. Missing or stale assets trigger an idempotent reinstall, while repair failures leave the provider enabled but persist `last_error` and an error setup state. After hook repair, the retirement pass removes obsolete session-context artifacts and rows while preserving the working-context store and unrelated analytics; provider-hook cleanup must succeed before data purge begins.
...
Fields include provider, message_id, session_id, content, role, custom_type, project, project_path, host, timestamp, git_branch, tools_used, files_modified, code_changes, commands_run, tool_details, and display text. Provider/project/host support facets, project and host are stored for hit metadata, and project_path keeps exact cwd identity. Pi search admits user, assistant, and `custom_message` documents; Claude and Codex retain intentional provider-native roles. Pi `custom_message` entries index their extension-injected context under a bounded 10 KiB content cap with their `customType` as an exact-match `custom_type` term, regardless of Pi's `display` flag; non-context `custom` entries stay excluded. Pi tool calls attach to assistant enrichment, while matching tool results are capped at 10 KiB in `ToolAction.full_output` and never become documents. Stored at `~/.local/share/com.quilltoolkit.app/session-index/`; schema version 10 adds exact `source_key` ownership and internal checkpoint documents committed atomically with each retained replacement. Upgrades stage and validate a replacement index, close handles, then promote it with deterministic interrupted-rename recovery. Unprovable historical ownership remains `legacy:unattributed`, preserving remote-only history at the cost of possible historical duplicates and unavailable context. It never grants local pruning authority. `index_state.json` is no longer successful ownership authority. See [[pipeline-search-tests]].
...
When a transcript is reprocessed, Quill coalesces repeated search `notify` requests per session and applies each Tantivy rewrite under one writer lock with a single commit. One retained-source coordinator separately tracks model and transcript completion under canonical `(provider, source_key)`; transcript replacement still spans all six tables in one transaction. The mtime sweep replaces only the canonical source's documents, even on first sight of a file. Other physical transcripts and host-qualified pushed sessions sharing native session/message ids remain independent.

Migration-armed forced scans clear their active reingest markers only after every discovered transcript extracts and indexes without a per-file retryable error and the Tantivy writer commits. A logged read, delete, or add failure is retryable and keeps the markers set for a full retry on the next scan. A terminal extraction failure — provider identity unresolvable, or a malformed/unsupported record — is deterministic for that file's content, so it never counts toward the markers' failure tally; the file's fingerprint is still cached like any attempted file, so later scans skip it as permanently failed instead of reattempting it every sweep, and its failure logs once instead of every pass.
...
The HTTP API also accepts provider-tagged notify and direct message ingestion. Local Claude full-transcript sync is Stop-scoped, while direct message ingestion still appends atomically for incremental remote updates. Project filters keep display-name facet matching for the UI and accept an exact absolute cwd for MCP callers; exact cwd identity separates same-named directories. BM25 scoring plus snippet generation power the shared search UI with provider filters and badges. The default search response retains full stored content for the desktop UI; `view=compact` omits it, returns snippet-and-identity hits, and stops before the serialized response exceeds 32 KiB for model-facing clients. Plain upper dates include the complete UTC day. Effective page size caps at 100 and offset plus limit at 10,000. Context accepts the hit's `source_key`, never substitutes local context for a pushed or unattributed owner, and caps windows at 20 neighbors per side and responses at 64 KiB. Message and response truncation are explicit; the UI exposes context failures rather than indefinite loading.

[[src-tauri/src/sessions.rs#validate_retained_notify_source]] validates one `notify` path against only its configured provider root, canonical containment, and supported layout without walking transcript history. Quill admits a canonical source once to the provider/source coordinator, whose transcript job shares extraction with Search and keeps failed-consumer retries independent through committed fingerprints. Canonical Pi payload identity must match the persisted header or return HTTP 400 before admission or writes. Small pushed Search display/lineage hints travel with the coordinator revision; they cannot change native analytics ownership. Before canonical admission, optional host and lineage ID/reason fields are capped at 256 bytes, while project, cwd, and git branch accept up to 4096 bytes (Pi sends cwd as project). Oversize fields return HTTP 400 without enqueue or consumer writes; empty values retain existing absent/clear semantics. Legacy Claude/Codex paths that fail the stricter retained-source policy still coalesce for Search only; Pi never uses that fallback. Direct message payloads append Tantivy documents and atomically store source-less runtime rows plus recorded live origin through [[src-tauri/src/storage.rs#Storage#store_live_session_analytics]]. Additive injected-context rows index without runtime events; flattened remote tool rows also store bounded evidence through that transaction.
...
[[src-tauri/src/fetcher.rs]] is the only remaining consumer of the Claude Code OAuth credential in the codebase. It powers the [[features#Live Usage View]] band by polling `api.anthropic.com/api/oauth/usage`, and spends banked limit resets with the same token, and was intentionally not migrated as part of feature 003 (see `specs/003-cc-inference-migration/spec.md` FR-015). A 401 from that endpoint is treated as a stale access token (a muted "Paused" state), so the only logged-out warning path runs [[src-tauri/src/config.rs#claude_logged_in]], which spawns `claude auth status --json` UNCONFINED to read the `loggedIn` boolean without touching the credential store — see [[data-flow#Usage Bucket Fetching]] step 8d.
...
A dev run must not corrupt the installed app's data, but it must still be reachable by the providers — and only one Quill at a time can be.
...
The handshake is machine-global. Every identity listens on 19876/19877, publishes `~/.config/quill/config.json` through [[src-tauri/src/data_paths.rs#shared_config_dir]], and signs it with the one `auth_secret` under [[src-tauri/src/data_paths.rs#shared_app_data_dir]]. A provider resolves that contract from one fixed path — the Pi extension hardcodes it — and cannot know which build wrote it, so a namespaced second copy is a contract nobody reads and a second listener pair only produces providers reporting into a Quill the user is not looking at. `QUILL_PORT` and `QUILL_CONTEXT_PORT` still override the ports, because a maintainer who names a port means it.

One handshake means one process. [[src-tauri/src/server.rs#start_server]] binds before it starts anything else and routes `AddrInUse` to [[src-tauri/src/lib.rs#report_fatal_port_conflict]], which hides the windows, names the conflict in an error dialog, and exits — the same queued-dialog shape as the fatal storage path, since setup cannot block the event loop it needs. A silent log line would present as an app that launched and then did nothing.
...
Those repairs render feature flags into production's providers, so a dev identity's flags must start as the user's rather than the struct defaults. [[src-tauri/src/integrations/manager.rs#inherit_production_features]] runs inside `load_integration_features`: a namespaced identity whose database holds none of the user-set flags (`context_preservation`, `activity_tracking`, `context_telemetry`; brevity is excluded because its legacy migration persists it on first read) copies production's persisted flags once through the read-only [[src-tauri/src/storage.rs#read_settings_at]]. After that seed the dev database owns its flags like any other identity. Without it a fresh dev launch rendered `context_preservation: false` into the shared Pi extension and every Pi started afterwards lost its tools and router until production redeployed.
