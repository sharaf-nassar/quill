...
[[src-tauri/src/storage.rs#CacheKey]] identifies a command, model range, optional provider, selected-history model, and 30-second wall-clock bucket so sliding windows naturally expire at a bucket boundary. [[src-tauri/src/storage.rs#get_or_compute]] checks the 45-second TTL and a [[src-tauri/src/storage.rs#TableVersions]] fingerprint made from each trusted table's indexed high-water marker. The TTL bounds delete staleness. A failed SQLite probe bypasses the cache and computes fresh rather than serving data whose validity cannot be checked. Each [[src-tauri/src/storage.rs#Storage]] owns typed caches for the aggregate, overview, and history commands, preventing results from one database instance from leaking into another.
