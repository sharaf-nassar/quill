...
The provider-neutral endpoint accepts Codex's known events plus Claude StopFailure and length-caps identity fields. [[src-tauri/src/models.rs#ObservedHookObservation]] carries only what an audit row stores — provider, session, optional hostname, event, tool, cwd, matcher, agent id, and producer time — and ignores extra fields so older payloads stay audit-compatible. Sessions projects the newest valid root Stop, Claude StopFailure, or SessionEnd per provider, session, and host after retained and observed-only rows merge; this projection does not change activity ordering. A Sessions read that still shows an open Claude agent without a resolved model schedules coalesced changed-source scans near 10 and 30 seconds, feeding only exact transcript-derived child models into retained model reconciliation. Its committed `model-analytics-updated` event invalidates Sessions directly, so naming does not wait for another read.

Rust integration config generation and retained reconciliation share [[src-tauri/src/sessions.rs#SessionIndex#local_hostname]], including environment, platform, file, and command fallbacks. The in-memory registry keys sessions by provider, trimmed short-form ASCII-case-folded hostname, and root session, then agents by `agent_id`; retained display hostnames remain unchanged. Each scan replaces a session's whole snapshot, so membership is bounded by what the transcripts still show rather than by an accumulating fold. Reads drop a snapshot 15 minutes after its newest transcript byte, five default three-minute usage-poll intervals, and a session the scan does not cover simply has no snapshot, so both cases read as null rather than a false zero.
