...
`transcript_analytics_sources` stores canonical transcript ownership plus explicit `source_kind` identity for lifecycle-owned live sources. `live_analytics_sessions` stores project, cwd, host, ordered lifecycle time, and closure state. Migration 42 adds Pi's `ephemeral` flag; migration 43 adds pushed usage identity and store-only costs. Migration 44 migrates prior Pi NULL-runtime and `pi-push:` evidence to `live:pi:<session-id>`, marks pre-upgrade rows sealed, refolds them, and records later start/end ordering without using transcript paths as authority. Migration 45 adds compact live open-turn state and refolds each existing Pi owner once. Sessions reads retain ephemeral origins without a transcript. Migration 30 sets `transcript_analytics_reingest_pending` for the later retained-source rebuild.
...
[[src-tauri/src/storage.rs#Storage#replace_pi_transcript_tool_rows]] is the second owned writer, narrowed to the two tables `replace_transcript_analytics_snapshot` would have written for Pi if Pi reached retained analytics at all. It replaces `tool_actions` and `skill_usages` under the same `live:pi:<session-id>` key through the same two insert helpers, so both writers share the owned partial unique indexes rather than the source-less ones, and the watermark filter and `tool_detail` carve-out are one implementation. `skill_usages` is not a retention target and the helper inserts every row it is given; Pi restates a whole transcript per notify, so it filters its own pre-cutoff skill rows at the call rather than making the shared helper configurable. It deliberately does not touch the other three tables: `session_events` and `response_times` share that key but belong to the pushed path, whose ephemeral sessions and in-flight turns no transcript restates.
