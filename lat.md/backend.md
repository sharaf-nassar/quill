...
- **model_usage_observations** — Normalized turn, token, and summary facts with exact raw model identity, a nullable indexed `derived_model_id` attribution column, nullable token dimensions, resolved session/chain ownership, and source-local ordering. Pi stores its upstream provider and model as one validated `provider/model` identity, preserving the only evidence the Usage graph may use for its LLM grouping. Migration 43 adds nullable Pi event UUID and native input/output/cache-read/cache-write/total cost fields plus a Pi-only partial unique event index. Migration 48 transactionally rebuilds the table to extend `observation_kind` to `turn`/`token`/`summary`, preserves every row and named index, and adds nullable `reasoning_tokens`, `stop_reason`, `had_error`, `tokens_before`, and `reasoning_duration_ms`. The storage structs and bindings carry these fields now; retained Pi producers deliberately leave them NULL until their owning capture items land.
