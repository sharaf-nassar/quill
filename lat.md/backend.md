...
`/api/v1/pi/track` uses its own 4,000-request sliding window, enough for four times the 1,000-event/minute single-event load. It serves two envelope shapes and dispatches on the protocol each body declares, after shared bearer auth, the 1 MiB body bound, the integration-enabled gate, that sliding window, and demo-mode gating. Protocol 2 is the transactional lifecycle path. Protocol 1 is the live hint path, whose envelope is `{protocol, extension_version, min_quill_version, last_error?, events[]}` with events sharing `{event_uuid, session_id, hostname, timestamp, type}` and carrying type-specific activity, model, lineage, cumulative-token, or per-message usage fields; it persists extension handshake health, returns typed JSON for `400`, `401`, `429`, and `503`, and answers `202 {"status": "accepted"}` with no generation acknowledgement and no durable receipt, because reconciliation stays authoritative over every hint. A protocol neither shape claims answers `426` with the required generation. Demo mode and bad auth ingest nothing.
...
Migration 46 also creates `pi_session_lifecycle`, `pi_event_receipts`, and bounded-key `pi_reporter_health`. Persisted open sessions enter lifecycle storage and `live_analytics_sessions` as `recovering`; only their own later end closes them. The durable `pi_spool_cleanup_pending` marker disables production legacy-spool import while later deployment cutover owns artifact deletion.
...
The query first materializes range-, provider-, and hostname-scoped token groups and ranks them with a range-bounded indexed response-time maximum. It enriches the requested top-N plus exact retained identities the tracker holds live state for; the overlay then applies live activity, deterministic ordering, and the final limit. Historical transcript and analytics storage remain available to their owning readers but do not project current agent or runtime state.
...
Migration-armed forced scans clear their active reingest markers only after every discovered transcript extracts and indexes without a per-file error and the Tantivy writer commits. A logged read, identity, delete, or add failure keeps the markers for a full retry on the next scan.
