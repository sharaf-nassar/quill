...
The session breakdown first materializes range-, provider-, and hostname-scoped token groups, ranks them with a range-bounded indexed response-time maximum, and materializes the requested top-N before turn, project, and sub-agent enrichment. Raw enrichment uses the exact timestamp lower bound. Retained sub-agent evidence uses every daily aggregate whose UTC day overlaps the range, including the partial first day so pruning cannot hide an agent that still belongs to the displayed window.
