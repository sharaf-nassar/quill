...
[[src-tauri/src/sessions.rs#canonicalize_hook_identity]] forms the aggregation key: paths inside `~/.config/quill/scripts/` or `~/.config/quill/codex/scripts/` collapse to `quill:<basename>` so the QUILL chip and per-machine identity stay stable, `${CLAUDE_PLUGIN_ROOT}/<dir>/<file>` is kept verbatim because the unexpanded env-var prefix is the only stable plugin-scoped identifier the transcript provides, any other absolute path is reduced to its basename, and records with no `command` (older Claude transcripts) fall back to `hookName`. The verbatim command is preserved in `script_command_raw` (truncated to 2048 chars at a UTF-8 boundary) for forensic drilldowns.
