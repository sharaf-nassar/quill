...
`/api/v1/pi/track` uses its own 4,000-request sliding window after shared bearer auth, the 1 MiB body bound, the integration-enabled gate, that window, and demo-mode gating. It accepts only transactional protocol-2 lifecycle envelopes, validates the exact reporter generation and canonical reporter subject, commits before live mutation, and returns typed `400`, `401`, `409`, `426`, `429`, and `503` outcomes. Demo mode and bad auth ingest nothing. Pi session files supply model, usage, activity, agent rails, and source-owned response times through persisted reconciliation; `/sessions/messages` no longer receives Pi runtime acceleration.
