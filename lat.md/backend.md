...
The database schema is versioned through migration 36 and includes usage, token, model analytics, context savings, learning, rule governance, session indexing, memory optimizer, code, runtime, retention aggregates, and metadata tables.
...
Range-scoped reads share one time vocabulary — `1h`, `6h`, `24h`, `7d`, `30d`, and (where supported) `all`. `get_token_stats`, `get_host_breakdown`, `get_project_breakdown`, `get_session_breakdown`, `get_skill_breakdown`, `get_skill_project_breakdown`, and `get_hook_breakdown` take a `range` string instead of the earlier `days: i32`, so an hour-granular selection is answered at that granularity instead of being rounded up to a whole day. Every one of them derives its lower bound from [[src-tauri/src/storage.rs#range_from_timestamp]], which delegates to [[src-tauri/src/storage.rs#range_to_duration]]; the bucketing matchers in [[src-tauri/src/storage.rs#Storage#get_usage_history]], [[src-tauri/src/storage.rs#Storage#get_token_history]], [[src-tauri/src/storage.rs#Storage#get_code_stats_history]], and [[src-tauri/src/storage.rs#context_savings_bucket_expr]] carry a matching `6h` arm so no reader silently answers a 24h window for a 6H request. `ModelRange` gains a `SixHours` variant with the same `6h` wire name, and `14d` stays in `range_to_duration` purely as the insights session-selector lookback.
...
`get_session_breakdown` is provider-agnostic at the row level and rolls up parent + all sub-agent rows for each session: `total_tokens`, `turn_count`, `last_active`, and the input/output/cache columns sum across `is_sidechain ∈ {0, 1}`, and each row carries two new fields — `has_subagents: bool` and `subagent_count: u32` (COUNT DISTINCT `agent_id`) — that gated the deleted analytics pane's expandable sub-agent tree; the widget's Sessions breakdown reads only the rollup and marks the count rather than expanding it. The `(provider, session_id, is_sidechain)` index added in migration 20 keeps each `UNION`'d branch on an index scan.

`get_skill_breakdown` returns recognized skill-use counts from the `skill_usages` table for the widget's Skills breakdown mode. It accepts the active range string, optional Claude/Codex provider filter, all-time mode, and a capped limit; rows sort by `total_count DESC, skill_name ASC` and include provider sub-counts plus `last_used` and a `project_count` (`COUNT(DISTINCT cwd)`) carried on each row. The widget renders one flat row per skill and has no project drilldown, so `project_count` is informational there.
...
Live-usage IPC carries one structured exception to the plain-string contract: `UsageData.provider_errors` is `Vec<UsageProviderError>`, where each entry pairs the provider with a typed [[src-tauri/src/models.rs#ProviderErrorKind]] discriminator (`Network`, `Config`, `Auth`, `RateLimit`, `Server`) alongside the human-readable message. `RateLimit` is currently inert (429 responses persist a silent cooldown timestamp rather than emitting a visible pill); the variant is reserved so future code can opt into a "Rate-limited" pill without another payload change. The frontend uses the discriminator to collapse all `Network` entries into a single offline state on the titlebar sync pill ([[src/components/widget/WidgetTitleBar.tsx#WidgetTitleBar]]), so a multi-provider outage produces one signal instead of N, while [[src/components/widget/LimitsSection.tsx#LimitsSection]] keeps per-provider rows for non-network failure modes. The flow that drives this — transport-error cooldown, exponential half-jitter backoff, and the on-success counter clear — is documented under [[data-flow#Data Flow#Usage Bucket Fetching]] step 8b.
