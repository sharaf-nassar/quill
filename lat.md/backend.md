...
The database schema is versioned through migration 48 and includes usage, token, model analytics, context savings, learning, rule governance, session indexing, memory optimizer, code, runtime, retention aggregates, and metadata tables.
...
Invalid UTF-8, source metadata, size, native identity, and the Pi `PiSession` and `PiSourceIdentity` parse failures are the content-deterministic classes; storage, graph, drift, and live-source failures carry no fingerprint.
