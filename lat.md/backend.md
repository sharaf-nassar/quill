...
Public surface: [[src-tauri/src/cc_client.rs#invoke_typed]] for schema-validated structured output and [[src-tauri/src/cc_client.rs#invoke_text]] for free-form prose. Model routing: Haiku for pattern extraction and prose work, Sonnet for synthesis. Schema enforcement is delegated to Claude Code via `--json-schema`; the schema string is produced from the existing `JsonSchema`-deriving Rust type via `schemars::schema_for!` and memoized per concrete type in a process-wide cache so it is computed only once.
...
The structured `--output-format json` envelope returned by every call carries per-call metadata (input/output tokens, cache stats, model id, durations, cost, stop reason, permission denials) that is captured into [[src-tauri/src/cc_client.rs#InferenceCallMetadata]] and persisted on the parent run record's `inference_metadata` JSON column for both `learning_runs` and `optimization_runs`. This is storage-only in the current feature; the UI does not yet surface it.
