...
would deflate per-agent tool-call counts. Omitting the two columns keeps every
row-shaped invariant — row count, `action_key` uniqueness, `agent_id`,
`message_id`, timestamps — and costs only the dead bytes.
...
The database schema is versioned through migration 43 and includes usage, token, model analytics, context savings, learning, rule governance, session indexing, memory optimizer, code, runtime, retention aggregates, and metadata tables.
