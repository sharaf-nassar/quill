...
Restart integration uses the same persisted file snapshots through a configuration-only commit path, but keeps a dedicated transaction marker under Quill's cache. Its settings, ownership state, hook assets, shell script, and touched RC files roll back together; startup cleanup recovers an interrupted restart transaction before other restart maintenance.
