...
- **Transcript watcher and recovery pass**: [[src-tauri/src/transcript_watcher.rs#start]] owns every periodic transcript walk. Its 120-second tick folds live evidence and arms one recovery pass on the retained-scan worker, which enumerates the roots once and, from that inventory, admits each source changed since the previous pass to the live coordinator, reconciles and prunes every root, and syncs Session Search. One coordinator tracks independent model and transcript completion, retry, and events. Startup whole-root transcript reconciliation is the same pass after the cold live fold, while [[src-tauri/src/lib.rs#spawn_startup_model_source_reconciliation]] separately re-admits retained Claude/Codex model inventory. Unchanged sources are cheap stat-only no-ops.
