...
Generic Pi lineage uses a separate rail labeled `live linked session` or `live linked sessions`; those children remain rows with parent markers and never enter agent metrics. Processes explicitly marked as Pi subagents instead fold into the parent's agent count, runtime, activity, and open-agent rail and never render a child row. Session Search keeps both kinds linked by stable provider-qualified parent id.
