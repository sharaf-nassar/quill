...
[[src/mocks/installBrowserMock.ts#installBrowserMock]] calls `mockWindows` and `mockIPC` from `@tauri-apps/api/mocks`, routing every `invoke()` to [[src/mocks/ipcFixtures.ts#handleInvoke]], and normally adds a fixed `MOCK DATA` badge. `?screenshot=marketing` suppresses only that badge and selects the marketing fixture profile; both controls remain dev-only because the whole mock import is behind `import.meta.env.DEV`. The fixture handler returns provider, usage, token/code, model, session-search, learning, memory, settings, and context data plus benign Tauri plugin defaults.
