...
Before mounting any storage-dependent route, `StartupGate` reads `get_startup_status` and subscribes to `startup-status`. `ready` mounts the normal routed view; `starting`, `migrating`, and `error` render [[src/components/MigrationView.tsx#MigrationView]] instead, so hooks cannot issue storage IPC while a schema migration is still running. Once the event stream has delivered a value, an older point-read snapshot is ignored; `ready` is terminal for that webview. The `migration` label renders nothing after `ready` while Rust closes it, so it cannot briefly mount widget hooks. Reaching `ready` also re-reads the crash-reporting preference because the module-scope privacy-first read may have run while storage was unavailable.

One root Suspense boundary wraps whichever routed view mounts, and [[src/components/RootErrorBoundary.tsx#RootErrorBoundary]] wraps that in turn. Suspense answers pending, never rejected: with nothing catching, React unmounts the whole tree, so a failed lazy chunk painted a window with the page background and nothing else — no message, no route, nothing to act on. A dev server re-optimizing after a new dependency is enough to cause it. The boundary reports the error text and offers the reload that fixes the common case, because a blank window is never an acceptable failure state.

The sibling dev-only failure — the window renders but every stylesheet rule is silently gone because a Vite restart's re-optimization raced the window's module graph — is covered by a stylesheet watchdog in [[src/main.tsx]]: after load, an empty computed `--surface` token triggers one self-reload, latched through `sessionStorage` so a genuinely broken stylesheet cannot loop, and cleared on success so a later drop heals again. `import.meta.env.DEV` keeps it out of production builds. See `docs/solutions/environment/second-vite-server-strips-dev-css.md`.
 Zoom persistence is stored in localStorage from the explicit `?view=` value (`quill-zoom-{view}`) and supports Ctrl+/-, Ctrl+0 with a 0.5-2.0x range via Tauri's native webview zoom API, falling back to CSS `zoom` only outside Tauri; a production migration webview derived only from its label therefore shares the `quill-zoom-main` key. Ctrl+F is blocked to prevent the webview's native find-in-page (no search UI exists). A `ToastProvider` context wraps all views for notifications, [[src/hooks/useIntegrations.ts]] gates provider-dependent secondary windows when no provider is enabled, and [[src/windows/SessionsWindowView.tsx]] refreshes the session index on demand before loading search facets.

The [[features#Settings Window]]'s Web tab is the desktop-only control surface for the browser entry. It discloses the unredacted monitor data and plain-HTTP boundary before enablement, configures port and peer policy, shows the bound-versus-reachable status, and can rotate the pairing credential; [[features#Settings Window#Web UI Section]] owns its detailed interaction contract.

`web.html` loads the separate browser entry, [[src/web-main.tsx]], under the HTTP-only CSP documented in [[infrastructure#Infrastructure#Build Configuration#Frontend Build#Crash Transport CSP]]. The entry installs [[src/web/httpTransport.ts]] before dynamically importing shared monitor code, imports neither the desktop main entry nor its mock fixture path, and does not register desktop find, zoom, or Manage accelerators. The shim posts same-origin invokes, maps their closed response protocol to Tauri-style promises, and implements event plus window/webview plugin calls as client-local no-ops; the server still denies every `plugin:*` command. Crash reporting and the updater are absent, leaving browser-native Ctrl+F and Ctrl+plus/minus/zero behavior intact.

[[src/web/WebShell.tsx]] renders shared Limits and view bands below a brand-only header; it has no titlebar controls, context menu, or desktop management action. Its `wg-shell--web` class squares the widget chrome and sizes the shell against `100dvh` so a phone URL bar never hides the scroll column. Above 640px it becomes a different shape entirely. One density means a wider viewport buys nothing — past roughly 1.8x the 360px design width the limit meters stretch into runways and the readout grid into gaps — so the measure caps at 640px. But a capped column still claiming `100dvh` draws its own edges down through a bordered void under the footer, which reads as a layout that failed rather than one that chose. So past the cap the shell stops claiming height it is not using: `height: auto` with `100dvh` kept only as a ceiling, a full hairline border and 12px radius, and vertical centring from a `grid` rule on `html[data-view="web"] body`. `.wg-scroll` still carries `flex: 1 1 auto; min-height: 0`, so a viewport shorter than the content clamps the panel and scrolls inside it. Below the cap none of this applies — edge-to-edge and full-height is right on a phone, and the squared chrome stands.

`web.html` carries `data-view="web"` as a static attribute rather than setting it from [[src/web-main.tsx]], matching the `data-view` hook [[src/main.tsx]] applies per desktop route; the page-level layout depends on it, so applying it after hydration would flash. Under `(pointer: coarse)` or at 430px-and-narrower viewports, web-only rules in `src/styles/index.css` give every operable control the DESIGN.md 44x44 CSS-pixel touch minimum: strip members grow their real boxes, chart grouping reflows above the chart, and hooks-help plus CPA-disclosure controls expand with transparent `::before` hit areas. The desktop widget has none of these rules; monitor bands reflow without clipping or horizontal scroll from 360px up.

[[src/web/useWebMonitorData.ts]] reads only cached usage, provider status, and CPA status through the shared invoke cache, renders unavailable inputs as gaps, and revalidates stale mounted subscribers every 55 seconds only while visible plus on focus. Its ordinary isolated 20-sample qualification recorded 55.056 seconds p95 against the 60-second budget in `specs/029-web-ui-server.md#final-qualifying-sequence--55-second-poll`. Its no-provider state directs the user to enable a provider on desktop without a browser action.

### Window Routes

Normal operation uses three routed Tauri windows; schema upgrades add one transient migration window.

The main widget, consolidated Manage workspace, and release-notes viewer use the `?view=` URL parameter under the shared root Suspense boundary. An explicit query value takes precedence; otherwise the `migration` label loads plain `index.html` for Windows-safe path handling and the frontend derives the migration view from the current webview label. Browser mock mode uses `?view=migration` because its mocked webview label remains `main`.
...
| `?view=manage` | [[src/windows/ManageWindowView.tsx]] | Rail-navigated Manage workspace; tool UIs (Sessions, Explore, Learning, Settings) embedded as sections |
| `?view=release-notes` | `ReleaseNotesWindow` | Browse published GitHub release notes |
| `migration` window label | [[src/components/MigrationView.tsx#MigrationView]] | Transient database-upgrade progress; closes before the hidden main window is revealed |

The former per-tool windows (`sessions`, `learning`, `runs`, `settings`) were retired into Manage sections, and run history folded into the Learning section. All three remaining routes are reachable without an enabled provider — the Manage workspace gates each tool section inline (Settings always renders), so the former `BlockedWindow` per-window provider-blocking was removed.

Each normal route mounts [[src/components/WindowResizeHandles.tsx]] as a sibling of its view. It suppresses itself on macOS, where AppKit supplies the resize frame; on Linux and Windows main takes `widget` geometry while `manage` and `release-notes` take `roomy`. The fixed, non-resizable migration webview mounts no resize handles.
...
The left rail has four flat sections — Sessions, Explore ([[features#Features#Usage Explorer]]), Learning (Rules / Memory / Runs), and Settings — with a signal-blue active indicator and a footer "Live" affordance back to the widget. The active section persists to `localStorage` (`quill-manage-section`) and accepts a `?section=` deep-link (the titlebar settings key opens `manage` at Settings). A deep-link value may name a settings tab as `settings:<tab>` — the shell owns the Settings tab state so the same link works whether the workspace is created or already open, which is how the widget's empty state lands users directly on Integrations. It still renders in the pre-widget roomier density on the Graphite Stack — the stated migration exception in DESIGN.md §6 — and converts to the flat plane in its own pass rather than piecemeal. Each section's content reuses the tool's existing window-view component, lazy-loaded and rendered with its own window chrome (titlebar/close) suppressed via `manage.css`; provider-dependent sections (Sessions, Explore, and Learning) show an inline no-provider state while Settings stays reachable. The Learning section's Runs toggle opens run history as an inline right-docked panel (folded in from the former floating `runs` window). The standalone tool windows, their `?view=` routes, and capabilities entries have been retired. A rail Search affordance and `⌘K` / `Ctrl K` open the [[src/components/CommandPalette.tsx]] — a substring-filtered list of the sections plus Back-to-Live and Close-Tools actions, navigated with arrow keys and Enter. The titlebar, its launcher button, and the palette's Close action display the label "Tools"; the window label, `?view=manage` route, `manage.css`, and component names are unchanged.
...
[[src/mocks/installBrowserMock.ts#installBrowserMock]] calls `mockWindows` and `mockIPC` from `@tauri-apps/api/mocks`, routing every `invoke()` to [[src/mocks/ipcFixtures.ts#handleInvoke]], and normally adds a fixed `MOCK DATA` badge. `?screenshot=marketing` suppresses only that badge and selects the marketing fixture profile; both controls remain dev-only because the whole mock import is behind `import.meta.env.DEV`. The fixture handler returns provider, usage, token/code, model, session-search, learning, memory, settings, and context data plus benign Tauri plugin defaults. Its `get_startup_status` fixture returns a realistic determinate backup only for `?view=migration`; every other mock route starts `ready`.

Widget fixtures answer per range rather than serving one fixed window. `get_model_usage_overview` includes aligned model token buckets plus model-less rows, so the Usage graph can regroup the same evidence. The marketing profile substitutes five readable Claude/Codex/Pi model series with staggered six-hour peaks, routes Pi through Claude Fable rather than Gemini, and removes MiniMax from statuses, LIMITS errors, and model observations. It also substitutes three all-live root sessions — one per provider — whose explicit agent counts, agent models, runtimes, turns, and token totals fill the Usage Sessions breakdown. Its retention policy has no watermark, so the voluntary-pruning disclosure does not appear in product screenshots. Other range, activity, code, and runtime fixtures keep the normal comparison-window contract.
...
Model analytics fixtures cover only `get_model_usage_overview` today; the paged model-session and per-session model-history islands remain deleted. Session Search has three marketing hits, one per Claude/Codex/Pi, with a populated selected context. Memories has four provider-aware project files, and the marketing feature profile turns Brevity on so every published Tools view is non-empty.
...
- **ConfirmDialog** ([[src/components/ConfirmDialog.tsx]]) — Shared confirmation modal used for destructive provider cleanup and provider installation confirmation, driven from the Integrations settings tab, and for spending a banked limit reset from the widget's Limits band, where a failed spend stays open with its display-safe error beneath the description. Built on the native `<dialog>` element via `showModal()`, so Escape dismissal arrives through the `cancel` event and the scrim is `::backdrop` styling rather than a managed overlay. Its author styles restore `margin: auto` after the global reset so the dialog stays centered in its owning webview.
...
A CPA pool's native disclosure button exposes `aria-expanded` and uses the widget chevron convention. Expanded content is capped at six indented account rows followed by `…and N more`; each row repeats the pool's window schema, rendering missing fetches as `—` with a neutral track rather than `0%`. Claude always projects its expected 5-hour and 7-day slots, then appends returned model-scoped weekly windows such as Fable to both the aggregate and account grid. Codex derives its shared schema from full scope-plus-duration identities present anywhere in the pool, so a globally absent 300-minute limit removes both its utilization column and reset while an account-only window keeps the column with gaps for accounts that lack it. On desktop, a borderless 10px power icon sits immediately left of each account name, wired to [[lat.md/features#Features#Settings Window#CPA Account Controls]]. It is vertically centered on the name and occupies the existing indentation gutter, preserving the 70px name column and meter alignment. There is no track, background fill, or visible On/Off text. The icon uses the neutral text token when enabled and faint token when disabled, with a brighter hover state. Its transparent native button retains a 24×24px desktop hit area, global keyboard focus, a stable account-specific accessible name, and `role="switch"` with confirmed `aria-checked` state. The tooltip names Enable or Disable for the current state. While saving, the icon dims, the tooltip reads `Saving account state…`, and `disabled` plus `aria-busy` prevent another activation without changing checked state. The icon does not animate. Banked [[lat.md/features#Features#Live Usage View#Limit Resets]] fill the identity space beneath the account name as its [[lat.md/frontend#Frontend#Components#Widget Limits Band#Reset Summary]]. CPA's unavailable flag is not labeled. Other non-ready enabled accounts show `COOLING` beneath the resets. Failures render inline below account status. Ready accounts need no extra badge. The read-only browser omits switches and Use keys, keeps each reset summary and its View list, and retains muted `DISABLED` labels for disabled accounts. Account status labels have no added letter spacing so labels and lamps fit within the 70px identity column without overlapping meters. When every account is cooling, the collapsed provider identity adds a light-red `COOLING` cue; expansion removes it because the account labels supply the detail. Any ready, disabled, or unavailable account suppresses that aggregate cue. Routing `status_message` remains in the expanded row's title. Rows omit diagnostic paragraphs for observation times, quota state, retries, credits, and model cooldowns. Degraded source state remains in the existing sync control and account status labels; auth/server failures cannot produce a live sync label. Cached account and pool percentages, bars, and reset timers retain utilization-based severity colors. Source errors and quota freshness never override numeric severity; only elapsed resets or missing values produce neutral cells. Scoped Codex labels retain their feature name. CPA `config`/`auth` failures without a snapshot render `SETUP`; configured transport failures render `UNAVAILABLE`. Accounts from unsupported CPA providers become one neutral `+N other accounts` line without a swatch or new identity.

#### Reset Summary

[[src/components/widget/LimitsSection.tsx#ResetSummary]] puts an identity's banked resets in the empty space under its name as one clickable line: use count and earliest expiry.

The line reads `↺ 4 · Oct 1`: the count at Text Hi, then the earliest local month-day expiry, with no date when no reset is dated. Its tooltip and accessible name carry the full sentence, such as `4 resets, first expires Oct 1, 2026, 11:00 AM`. In an account row it sits level with the meters' countdown row, so resets never make an account row taller or add a row. A direct row's 60px identity column stacks it under the provider name. No reset takes a severity or provider hue.

The line is a borderless disclosure key (`aria-expanded`) with the widget hover fill. Clicking it opens a popover on the band's raised surface, dropped over the meters and closed by another click, an outside click, focus leaving it, or Escape. While open, the line becomes the popover's tab: same surface and hairline, open at the bottom, with the list hanging flush from its left edge and square at that corner, so the list visibly belongs to the line that opened it. Opening moves nothing, and each reset's label starts on the glyph's edge. It lists resets by expiry, undated last. Resets that share a label, expiry, and usability fold into one line with a `×N` count, so same-day Codex credits read as one line. Each line shows the label, local expiry date and time, and a Use key or a faint `Not yet` for a Claude grant queued behind the next one.

Every Use opens the shared confirmation dialog. Its title counts the identity's uses (`Use 1 of 4 Claude resets?`) and its description names the exact reset and its full expiry. On touch-capable browser viewports the line grows a transparent 44px hit area. The browser monitor keeps the line and popover but has no Use keys.
...
[[src/components/widget/views/UsageView.tsx#UsageView]] is the widget's default view and the product's core surface: hero chart, insight line, a two-tier readout grid, the switchable breakdown, and the totals footer.

Every band reads the region's selected range, so chart, delta, insight, all ten readouts, the primary sparklines and the footer always describe the same window — a band quietly using a different window would be a lie about the instrument. The chart's CLI, Provider, and Model controls regroup token buckets from [[src/hooks/useModelAnalytics.ts#useModelAnalytics]] while keeping the same model-evidence total in the headline, legend, insight split, and footer. A compact always-on legend sits left of the toggle, one swatch and label per plotted series, hidden along with the chart whenever the range has no tokens; Model grouping shortens each label to the agent rail's family name (`formatSessionModel` in [[src/utils/format.ts]]) instead of the raw model id, while CLI and Provider keep their existing labels. Provider is the user-facing name for the upstream-provider grouping; its persisted internal value remains `llm` so existing preferences survive the rename. Model is the persisted default; malformed or unavailable local storage falls back to Model without blocking the current selection. Its delta is momentum *inside* the range (the back half of the buckets against the front half) rather than a comparison with the previous window: a headline delta whose evidence the chart does not draw is not evidence.

The readout grid is two tiers of one grammar. The primary row is what moves every minute of a live session, four across: **Runtime**, **Tools** (tool calls), **Turns** (LLM turns), and **Net lines**, each a 15 px value, label, swatch, and sparkline. The secondary tier is the slower context: **Prompts**, **Reasoning**, **Tok / LOC**, **LOC / hr**, **Sessions**, and **Projects**, at a 12 px value and 7 px label with no sparkline. That label is one step under the 8 px type floor, taken deliberately for this tier and recorded as the stated exception in DESIGN.md §3: it holds the tier a clear step below the primary row's 9 px, and it is what lets `REASONING` stay a whole word (53 px against 58 px of content) instead of the `REASON.` abbreviation 8 px forced. Six across needs about 53 px of content per cell for that longest label, which the widget has only from 420 px up, so at its 360 px default and down to the 320 px floor the secondary tier is two rows of three; the primary row stays four across throughout because its labels fit to the floor and it is the row that must read as one line. Both tiers are hairline-ruled regions of the one surface: every cell after the first in a row takes a left rule, and rows past the first take a top rule.

A readout may carry one secondary figure at the right end of its value line: bare 10 px tabular text (9 px in the secondary tier) in the same Text Hi mix as the label, a tier below the value, with no box, because a readout cell is a region of the one surface and a filled capsule inside it would be the grid's first box. Each is a measurement with a stated denominator or absent, never a zeroed default. Runtime carries the total as a multiple of the selected window (`2×` for two hours in a `1h` range, `0.5×` for thirty minutes; rounded to one decimal, `<0.1×` for tiny non-zero totals), because a total above `1×` is the only visible evidence of parallel sessions. Tools carries the failure rate over calls that recorded an outcome, and reads nothing when no provider in range records one, since Claude and Codex rows carry no `is_error` today and a `0%` there would be a lie. Turns carries active runtime per turn, dividing the runtime total by LLM turns rather than by the runtime read's own `turn_count`, which counts idle-split active intervals. Prompts carries prompts per session over sessions that have one. Reasoning reads as the share itself — reasoning tokens over the output of the turns that report the dimension (only Pi does today, which the cell's tooltip states) — because the proportion is what changes with model behaviour while the raw count only grows with volume; the token total is its secondary figure. A share below one percent reads `<1%` rather than `0%`.

The chip sits at the right end of the value line in both tiers, centred on the value, and it is optional where the value and label are not. The value line wraps and clips to its own line height: a chip that fits sits at the right edge; one that does not drops below and out of view rather than overflowing the cell rule, squeezing the value, or shifting the rows out of alignment. Four across at the 360 px default leaves 69 px of content, which holds `385` plus `3%` and `45m` plus `2.1×` but not an hours-long runtime beside its multiple; that chip returns from roughly 420 px up. Colour carries meaning and nothing else: each readout inherits its fixed metric hue across value, label, swatch, secondary figure, and sparkline, binding them into one scannable unit while label text, the swatch, and chart shape preserve non-color cues. Green and red survive only on the chart delta, where they are assigned by *meaning* rather than arrow direction.

Per-metric sources: runtime and its sparkline from [[src/hooks/useLlmRuntimeStats.ts#useLlmRuntimeStats]]; tool calls, turns, prompts, and reasoning with their series from [[src/hooks/useWidgetSeries.ts#useWidgetActivityStats]]; tokens-per-LOC and LOC-per-hour from [[src/hooks/useCodeInsights.ts#useCodeInsights]]; sessions from the runtime read and projects from the breakdown; net lines from [[src/hooks/useCodeStats.ts#useCodeStats]], bucketed as `lines_added − lines_removed`. The footer's In/Out/Cache totals come from the same model-usage overview as the selected chart grouping, so no nearby number can contradict its chart. `webSurface` retains those totals but omits the desktop Manage action.

The insight line carries one computed insight per window, chosen by the rotation rule in [[src/components/widget/views/insightLine.ts#selectInsightLine]] rather than pinned to a single source. Every candidate restates figures the view already read for that same window: context savings from [[src/hooks/useContextSavingsStats.ts#useContextSavingsStats]], the cached-token volume behind the footer's `Cache` percentage, and the selected chart grouping behind the hero chart. A candidate speaks only when its figure exists and is non-zero, so nothing is ever zeroed or padded to keep the line occupied; with no eligible candidate the line is simply not drawn.

Priority is fixed and ordered by how much of the story the rest of the widget does not already tell — savings appears nowhere else, the cached-token volume is only implied by the footer, and the selected chart grouping is already drawn directly above, so it speaks last. The first eligible candidate wins, and a higher-priority candidate whose source has not answered yet holds the line empty rather than letting a lower one appear and be swapped out a moment later; a failed read counts as answered-with-nothing, so one broken source cannot mute the line. Selection is a pure function of the window and its resolved data — no clock and no counter rotates it under the reader, so the same window with the same data always states the same thing.

The breakdown switches five modes over one main-row grammar — status dot, name, optional metadata, identity chip, primary value, activity — filled per mode: Sessions (provider, tokens, live runtime or inactive recency), Projects (session count, tokens), Hosts (turns, tokens), Skills (range-scoped uses, Claude/Codex/Pi counts, last used), Hooks (QUILL chip where Quill-deployed, Claude/Codex/Pi counts, fires, last fired). Sessions add agent totals, turns, and lifetime runtime to that main row; only open agents add a wrapping second rail. Rows without open agents stay 30 px. Session identity pairs shrinkable project text with a reduced-scale unboxed full provider name, both centered against the full row height. Flagged Pi rows add an 8 px neutral EPHEMERAL badge using only the brightness ladder and hairline token, never severity color. Grid-assigned agent, turn, lifetime, token, and activity columns stay aligned across rows and fit the 320 px minimum. A missing optional agent total leaves its deliberate grid slot empty instead of shifting later metrics. The visible Projects readout uses one secondary project request except while Projects is selected, when the selected breakdown result supplies both regions; no command-and-args key mounts twice. The Sessions header summarizes all returned sessions in the selected range, not just the five visible rows: an amber dot and count for inactive sessions with background work running, followed by the existing green dot and active count. Zero counts are omitted; ordinary inactive sessions do not contribute to the amber count. Header counts and row dots share the same liveness predicate, including Pi recovery handling. Tooltips and accessible labels name each count. The count group wraps below the mode controls when needed at narrow widths. The Hooks header retains its Claude/Codex audit-tracking disclosure.
...
When the overview's `summaryUsage` reports any compaction/branch-summary observations, a "Summaries (unattributed)" entry renders beneath the ranked rows behind a soft hairline: dim label, faint count, bright tabular token figure, hover copy naming what the spend is. It deliberately has no swatch, no chip, and no progress track — summary spend has no model identity, and dressing it as a model row would fabricate one — but its token figure sits on the ranked rows' shared token column so the band's numbers reconcile against totals that already include that spend. Zero observations render nothing: absence of summaries is not a claim this band needs to make.

Three disclosures keep a compact home here. Coverage states the share of token activity that carries model evidence whenever it is short of 100%, because activity recorded before a chain's first observation stays unattributed instead of being assigned a model. A retained-history line appears only while that source inventory needs attention, carrying its state, processed-source count, and desktop Retry control. `webSurface` retains the status but omits that backfill action. A separate model-index line reads persisted `buildingIndex` on mount and committed [[src/hooks/useRollupBackfill.ts#useRollupBackfill]] events while present; it shows observation counts, raw-evidence fallback, stopped recovery, or the completed refresh. Emptiness is a claim the view has to earn from final backend scope.
...
[[src/components/widget/views/UsageView.tsx#UsageView]] keeps project plus its compact provider mark and the session's own short model family label, retained agent count plus bot icon and runtime, displayed root-turn count plus lifetime runtime, tokens, and activity on the 30 px main rail. The root's model label reuses the agent rail's family shortener (`formatSessionModel` in [[src/utils/format.ts]]) with a neutral label color rather than the rail's live Meter Green, keeps the raw model id in its tooltip and ARIA, and renders nothing only when the row carries no model at all: the live fold's own model when it has folded one, else the retained row's ranked primary model from its recorded usage, so a session keeps its label after aging out of live coverage instead of losing it. The displayed count adds one to completed backend turns only while `live && current_turn_runtime_active` proves a current open root turn; observed-only rows without that evidence stay unavailable. The lifetime runtime uses subdued Meter Green under that same predicate and otherwise stays faint. It shows minutes-only values with a spaced `m` suffix; hour/day colon forms omit leading zero fields, leave the first visible field unpadded, pad later fields to two digits, floor incomplete minutes, and permit days above 99. The turn count sits before runtime without a divider; a message glyph precedes the spaced count. Its ARIA label distinguishes the active turn from completed turns and retains singular grammar. Each Sessions datum has its own opaque, zero-delay hover title without repeating the visible value. While live, the right edge shows an adaptive current-turn clock: seconds below one minute, then minute, hour, and day colon fields with the first visible field unpadded and later fields two digits. It floors partial seconds, keeps a title-only tooltip and human-readable ARIA, and renders a neutral em dash when unknown; inactive rows retain muted recency. A one-second local clock advances family total at `active_runtime_rate` and current turn at one second per second only while `current_turn_runtime_active` is true. SessionEnd-aware liveness also suppresses any stale observed-agent snapshot that has not cleared yet.

The main rail's fixed no-wrap lifetime group orders retained agent count, one 9 px Agent Orchid bot icon, then agent-only active runtime. Positive totals remain visible after every agent closes; zero or unavailable totals omit the group when no current agents exist. An open list keeps unknown totals as em dashes rather than inferring them from its membership. The second rail exists only for those open agents, starting with one decorative Meter Green bot icon whose restrained opacity pulse stops under reduced-motion, followed by no-wrap model/runtime pairs that wrap at the 320 px width floor without another nested separator. Open model names use Meter Green as explicit healthy/live status while their runtimes stay muted neutral. Known agent-runtime baselines advance from the shared producer timestamp by the number of observed agents with known runtime and an active accrual bit; unknown baselines remain unknown. Long models ellipsize; instant `.wg-row-datum` tooltips replace delayed native titles and retain full identity. Claude tiers sort Opus → Sonnet → Haiku → Fable, Codex tiers sort Sol → Terra → Luna, and other exact labels sort stably after known tiers; an agent whose model has not folded yet is omitted from the rail entirely rather than rendering a role or `?` placeholder chip. Pi agents reuse those Claude and Codex family labels from each raw model id; when a validated launcher role exists, tooltips and accessible labels keep both that role and the raw model id.

`SessionBreakdown.active_runtime_secs` is nullable lifetime family runtime and stays null until runtime backfill completes, except pushed live Pi agents and durably closed Pi lifecycle children can supply known baselines immediately. Nullable `agent_count` and `agent_runtime_secs` describe retained sidechains plus every explicitly marked Pi descendant whose durable `pi_session_lifecycle` parent edges resolve to the visible root. Durable edges use the live resolver's 64-edge, cycle, missing-parent, and cross-host rules; current live Pi children union by session id, so their runtime replaces rather than doubles the durable entry. A closed child's `origin_at_ms` to `closed_at_ms` duration remains after `SessionEnd`; an open or recovering durable row establishes count but never claims runtime or liveness without the live fold. Root runtime never enters those values. `current_turn_runtime_secs` is nullable root open-tail evidence, `current_turn_runtime_active` controls its one-second accrual, and shared nullable `runtime_as_of_ms` timestamps every baseline. `active_runtime_rate` remains the additive family accrual rate. `turn_count` is the host-qualified lifetime count of completed root prompt-response turns; in-flight and sub-agent turns do not contribute. `observed_agents` is current-only: it preserves each open agent's identity, nullable source-local runtime, and accrual bit used for agent-total extrapolation, but never reconstructs an ended durable child. A successfully reconciled chain or pushed Pi agent can publish those fields before global historical runtime backfill completes, while a failed or absent chain stays unknown. `observed_only` still renders unavailable tokens and turns as em dashes; a retained or pushed live Pi row renders its real cumulative token total without a provider-specific gap. `background_tasks_running` is live-fold evidence that a pi-background-tasks launch has no terminal notification yet; the row's status dot turns `--meter-amber` (`data-live="background"`) only while the session is otherwise inactive, because a live session's green dot already says it is working. The dot carries the shared `wg-row-datum` instant tooltip (`Active`, `Inactive, background task running`, `Inactive`) with a matching accessible label; the tip is left-anchored because a 6 px anchor at the row edge would clip a centered one.
...
Generic Pi lineage uses a separate rail labeled `live linked session` or `live linked sessions`; those children remain rows with parent markers and never enter agent metrics. Each child's chip reuses the agent rail's `formatSessionModel` family shortener for its model id, keeping the raw id in the chip's accessible label, and falls back to the child's truncated session id when no model is known. Processes explicitly marked as Pi subagents resolve direct lineage through at most 64 edges and flatten into the visible root's agent count, runtime, activity, and open-agent rail; while any of them stays open the root's activity reads as current, so its turn-settle terminal cannot hide the rail mid-run. Missing, cyclic, over-depth, or cross-host proof stays an explicitly unlinked independent row only while live; late proof attaches it, and completion removes it. Recovering durable rows render unlinked without claiming liveness until same-process proof arrives. Session Search keeps direct links by stable provider-qualified parent id.

The retired EPHEMERAL badge is never rendered. Persisted Pi sessions use native source evidence; no-file sessions intentionally have no Sessions row rather than a special transient presentation.
...
|------|---------|----------------|
| `useWidgetActivityStats` | Tool-call, turn, prompt, and reasoning totals with their series for the widget readouts, on the same grid; invalidated by token, transcript-analytics, and model-analytics events | `get_widget_activity_stats` |
...
| `useCodeInsights` | Tokens-per-LOC and LOC-per-active-hour over the displayed window | `get_code_stats_history`, `get_token_history` |
...
| `useExploreSeries` | Every Explorer series on one shared daily grid (7d/30d/90d) — token activity, sessions/projects, net lines, tok/LOC, LOC/hr, and bucket-averaged runtime; see [[features#Features#Usage Explorer]] | `get_model_usage_overview`, `get_activity_series`, `get_llm_runtime_stats`, `get_token_history`, `get_code_stats_history` |
...
Key type categories: usage/token tracking (`UsageBucket`, `TokenDataPoint`, `TokenStats`, `ProviderCredits`, `LimitReset`), context savings (`ContextSavingsAnalytics`, `ContextSavingsEvent`), indicator state (`IndicatorPrimaryProvider`, `IndicatorMetric`, `StatusIndicatorState`), model analytics (`ModelUsageOverviewResponse`, `ModelAnalyticsError`), learning (`LearnedRule`, `LearningRun`, `LearningSettings`), and session search (`SearchHit`, `SearchResults`, `SessionContext`). `IntegrationProvider` includes `pi`; shared provider utilities assign its label, order, and `--provider-pi` hue lookup while Limits omits it because Pi has no quota API.
...
A second token block in the same `:root` carries the Flat Polish system that DESIGN.md now describes as the whole app's target: the flat surface pair (`--surface` `#14181f`, `--inset`), hairline and hover alphas (`--line`, `--line-soft`, `--hover`), a brightness-first text ladder (`--text-hi`, `--text`, `--faint`), six metric hues (`--metric-runtime`, `--metric-tok-per-loc`, `--metric-loc-per-hr`, `--metric-sessions`, `--metric-projects`, `--metric-net-lines`), and three context-category hues (`--context-preserved`, `--context-retrieved`, `--context-routing`). Each Usage readout passes its hue through the cell as `currentColor`, so swatch and sparkline take it raw while the two text tiers `color-mix` it back toward the neutral ladder — value at 75% hue against `--text`, label at 45% against `--text-hi`. The raw hues sit at 0.30–0.65 relative luminance against `--text-hi`'s 0.84, and colored text at that gap loses its antialiased edge coverage and reads a size smaller; both mixes clear 7:1 on `--surface`, with the paler tier going to the 9px letterspaced label because the 15px value carries enough weight to hold the saturated one. Delta status remains on the separate meter tokens, so identity never reads as state. Context split segments and LIMITS' named window labels reuse the same category tokens without changing their own neutral values. Provider hues are reused unchanged from the block above. The Graphite Stack block survives only for the windows DESIGN.md §6 defers; new surfaces are built on `--surface`.
...
| File | Lines | Scope |
|------|-------|-------|
| `src/styles/index.css` | 3,723 | Design tokens, shared chrome, and every widget section |
...
| `src/styles/explore.css` | 529 | Explore section: overlay chart card, series panel, correlation strip |
...
- **Metric identity** — each Usage readout carries one of six hues across its value, text label, swatch, sparkline stroke, and endpoint, with the two text tiers mixed toward the neutral ladder so colored type keeps its weight; context-category hues remain on their split-bar segments. A category that would otherwise want green/red (added versus removed lines) takes `--metric-net-lines` / `--metric-loc-per-hr` instead, because a category is not a threshold. Delta text still uses status tokens.
...
Bands absorb the width instead of scaling into it. The shell is a flex column at `height: 100vh`, the content column has no width of its own, `.wg-grid` splits into three `1fr` tracks, and rows are flex lines whose text cells carry `min-width: 0` so they ellipsize rather than push.

Absorbing width has a useful ceiling, and only the browser surface reaches it. A desktop window is dragged to a width the user chose, so it is left alone; a browser tab simply inherits the monitor, so `.wg-shell--web` caps at 640px and centres. Beyond that the bands are no longer absorbing width, they are diluting into it — measured against the mock at 360, 560, 640, 720, and 1440px, the limit meters and the readout grid are the first to go slack.

Height follows the same reasoning one step further. Capping width alone left the shell stretched to `100dvh` around content that ended at the footer, so above the cap it sizes to content and treats the viewport as a ceiling instead. That is the one place a Quill surface is a centred panel rather than a filled plane, and it is because a browser tab's dimensions are not a user's stated intent the way a dragged window's are. The one band with an intrinsic width wider than the 320px floor is `.wg-footer`, which wraps its Manage affordance onto a second line under that pressure; at the design width and above it stays a single 40px row. The `.wg-row` hover bleed (`margin: 0 -7px`) reports as horizontal overflow on `.wg-rows` at every width — it is contained by the band's 14px gutter and clipped by `.wg-scroll`, which is the intent.
