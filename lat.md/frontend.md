...
[[src/components/widget/LimitsSection.tsx#LimitsSection]] is the widget's whole subscription readout: direct-provider rows plus source-keyed CPA pool summaries and disclosed account detail.

Each direct row is an identity swatch (the fixed provider hue), provider name, fixed-width rate-limit cells, and a right-aligned nearest-reset countdown. CPA rows use the same identity and meter vocabulary but are grouped independently by `(provider, source)`, carry a visible `CPA` tag plus healthy/total count, and appear immediately after that provider's direct row. Meter columns wrap at their legible minimum on narrow windows instead of shrinking type or overflowing the 320px floor. A numeric cell states rounded percent, compressed label, and a 4px `role="progressbar"`; its full label remains in the title and accessible name.
