...
Sessions rows render only positive transcript-derived agent evidence; null and zero reserve no element and make no numeric claim.

[[src/components/widget/views/UsageView.tsx#UsageView]] orders each row as project → optional agent identity → provider → tokens → recency. Positive evidence renders a 12 px Agent Orchid bot icon, typographic middle dot, then tabular groups such as `2×Opus · 3×Sonnet`; the icon and dot are assistive-hidden while the cluster labels the full grouped total. Each agent uses its transcript-derived type only while its exact retained model is unresolved, so model families replace rather than accompany type fallbacks. Claude tiers sort Opus → Sonnet → Haiku → Fable, Codex tiers sort Sol → Terra → Luna, other exact labels sort stably after known tiers, and only agents with neither model nor type form the final `N×?` group.
...
`SessionBreakdown.observed_subagent_count` is a required `number | null`; `observed_subagent_models` is the matching exact model/type/count partition. A scan always answers for the sessions it covers, so the count is a real number there — zero included — and null means only that the session has no live snapshot at all: it is outside the 15-minute idle window, or its provider or activity tracking is disabled. Positive means that many agents were still unresolved in the transcripts at scan time; it does not prove process liveness. `observed_only` marks an active root that lacks retained token metrics, so the row renders an em dash for tokens and makes no turns claim. `active` keeps meaning recent activity rather than a separate open-versus-working distinction, so an agent that stopped producing bytes ages out through the same idle window.

Count and exact open-agent membership come from one registry snapshot. Retained model resolution runs after unlocking against that snapshot, so a scan landing mid-resolution cannot mix its membership into the count's model groups.
...
Key type categories: usage/token tracking (`UsageBucket`, `TokenDataPoint`, `TokenStats`, `ProviderCredits`), context savings (`ContextSavingsAnalytics`, `ContextSavingsEvent`), indicator state (`IndicatorPrimaryProvider`, `IndicatorMetric`, `StatusIndicatorState`), model analytics (`ModelUsageOverviewResponse`, `ModelAnalyticsError`), learning (`LearnedRule`, `LearningRun`, `LearningSettings`), session search (`SearchHit`, `SearchResults`, `SessionContext`), restart (`RestartInstance`, `RestartStatus`).
