...
Each row has a fixed identity region and fluid meter region. Every cell places percent at left and its window label geometrically centered over the gauge. Summary values step up one type size over account values. Optional CPA timing occupies a dedicated footer centered beneath its matching track, using a 10px tabular countdown in the brighter secondary label tone so it stays readable without competing with an adjacent window's utilization. Missing countdowns render a dash; elapsed ones render neutral `now`. Direct rows end with their nearest reset, and a CPA pool replaces its matching direct row while present.

CPA provider and account rows share the same 70px identity column, metric start, and 88px minimum cell grid. Their window tracks therefore align vertically and consume the same full meter width; dynamic extras reflow rather than clipping at the 320px floor. Direct cells retain a 60px minimum because they do not carry per-window reset metadata inside the cell.
