...
Each readout is value, label, swatch, and sparkline — no per-cell percentage. A cell's own sparkline already draws its direction over the same window, so a redundant chip was dropped along with the previous-window comparison that fed it; the hero chart keeps the view's one delta. Colour carries meaning and nothing else: each readout inherits its fixed metric hue across all four elements, binding them into one scannable unit while label text, the swatch, and chart shape preserve non-color cues. Green and red survive only on that chart delta, where they are assigned by *meaning* rather than arrow direction.

Per-metric sources: runtime and its sparkline from [[src/hooks/useLlmRuntimeStats.ts#useLlmRuntimeStats]]; tokens-per-LOC and LOC-per-hour from [[src/hooks/useCodeInsights.ts#useCodeInsights]]; sessions and projects sparklines from [[src/hooks/useWidgetSeries.ts#useActivitySeries]]; net lines from [[src/hooks/useCodeStats.ts#useCodeStats]], bucketed as `lines_added − lines_removed`. The footer's In/Out/Cache totals come from the same model-usage overview as the selected chart grouping, so no nearby number can contradict its chart. `webSurface` retains those totals but omits the desktop Manage action.
