...
Session rows key identity by provider, hostname, and root session. `observed_subagent_count` is nullable current-boot evidence, not retained history: positive values render exact neutral tabular metadata with one singular/plural accessible label, including on idle rows. Missing coverage and observed zero remain absent. Normal hook delivery is exact, but a missed stop can remain observed open until parent end or restart; the UI never claims verified process liveness.
