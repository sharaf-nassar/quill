...
Session rows key identity by provider, normalized hostname, and root session. `observed_subagent_count` is nullable current-boot evidence, not retained history: positive values render an Agent Orchid branch glyph and exact tabular count with one singular/plural accessible label, including on idle rows. `observed_only` marks active roots whose retained token metrics are not available yet. Missing coverage and observed zero remain absent. Normal hook delivery is exact, but a missed stop can remain observed open until parent end or restart; the UI never claims verified process liveness.
