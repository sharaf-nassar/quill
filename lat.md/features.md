...
The Skills breakdown reads [[src-tauri/src/storage.rs#Storage#get_skill_breakdown]] via [[src/hooks/useBreakdownData.ts#useBreakdownData]] and shows one row per recognized skill. Recognition is intentionally conservative: session indexing stores a skill-use row only for read-like `SKILL.md` loads (Claude `Read`, or Codex read commands such as `sed`/`cat`), and ambiguous mentions or skill-file edits are excluded. Skills follow the active analytics timeframe by default, an "All time" toggle switches the query to all indexed history, and All/Codex/CC badges filter counts by provider without changing the surrounding Now range. All view keeps provider sub-counts visible inline (`C` and `CC`) while total count remains the primary sort key.
