...
The hero chart groups one model-evidence token series by CLI, LLM, or Models, selected by compact controls at its top-right. Models is the fresh-profile default; the valid local choice persists across restarts and storage failures retain the current choice. CLI uses Quill's recorded provider, LLM uses Pi's recorded upstream prefix and otherwise the recorded CLI provider, and Models uses the exact provider-qualified model identity. Model-less activity remains a factual `unattributed` series rather than being dropped. The headline, delta, legend, insight split, and footer all read the same selected evidence total. Pointer movement or focused arrow/Home/End keys select any aligned time bucket and replace the legend totals with its values while brightening its time and drawing a crosshair plus group markers. Pointer leave, blur, or Escape restores the range summary and final endpoints. The delta compares the back half of the buckets against the front half rather than against the previous window, because a headline delta whose evidence the chart does not draw is not evidence.
