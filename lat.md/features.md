...
The view name and the shared 1H/6H/24H/7D range strip live in the region's band header, so switching views keeps the operator's range and there is only ever one control strip. `30d` is deliberately absent — a month is not a widget scope — and the default is 6H, the shortest window that still spans a working session. A view may declare itself unranged, in which case the strip is hidden rather than left standing as a control that would change nothing. The dropdown is a listbox, not a menu, because it has a value. Composition detail lives in [[frontend#Frontend#Components#Widget View Region]].
...
The hero chart stacks one smoothed area per provider for the selected range, with the range's total tokens and an in-range momentum delta overlaid top-left and a hover-only legend chip carrying per-provider totals. The delta compares the back half of the buckets against the front half rather than against the previous window, because a headline delta whose evidence the chart does not draw is not evidence.
...
The breakdown switches five modes over one row grammar — status dot, name, optional metadata, identity chip, primary value, recency: Sessions (default), Projects, Hosts, Skills, and Hooks. Sessions inserts an Agent Orchid bot icon, middle dot, and positive observed-agent model groups between project and provider; null and zero insert nothing. A current-process row shown before its first token snapshot uses an em dash for unavailable tokens and omits turns from its tooltip. Hosts sort by `total_tokens DESC`; projects and sessions by `last_active DESC`; skills and hooks by recognized count descending with a stable name tie-break. The projects sort is enforced both in [[src-tauri/src/storage.rs#Storage#get_project_breakdown]]'s SQL `ORDER BY` and in the post-merge re-sort inside [[src-tauri/src/storage.rs#merge_project_subdirs]], since the subdir-folding step would otherwise override the SQL order. The footer states In/Out/Cache totals and the ⌘M Manage entry.
...
The Hooks breakdown (feature 009) reads [[src-tauri/src/storage.rs#Storage#get_hook_breakdown]] via the same hook and shows one row per canonicalized hook identity. Claude rows are per-script: extracted from JSONL `attachment` records via [[src-tauri/src/sessions.rs#extract_hook_invocation_from_attachment]] and canonicalized via [[src-tauri/src/sessions.rs#canonicalize_hook_identity]] so Quill-managed paths collapse to `quill:<basename>`, `${CLAUDE_PLUGIN_ROOT}/<dir>/<file>` stays verbatim, other paths reduce to basename, and missing-command records fall back to `hookName`. Codex rows are per-event because Codex rollouts do not log hook executions — the deployed `hook-observe.cjs` observer captures every fire of the seven observed events via `POST /api/v1/hooks/observed` but cannot attribute individual sibling scripts since Codex registers multiple scripts per event. Rows backed by Quill-deployed scripts carry a `QUILL` chip, so users see Quill's own telemetry overhead alongside their hooks rather than hidden. The schema preserves `cwd` and the `is_sidechain` / `agent_id` columns for future disclosure layers.

Session rows key identity by provider, normalized hostname, and root session. `observed_subagent_count` is a nullable count from the newest transcript scan, while `observed_subagent_models` partitions each positive total by raw exact model id and retained agent type. Agents show their transcript-derived agent type only until retained child-transcript evidence resolves the exact model by root and agent id, never from the root model. Each agent renders one identity: resolved model families replace type fallbacks, and `?` appears only when neither exists. The UI orders Claude Opus → Sonnet → Haiku → Fable and Codex Sol → Terra → Luna, then other exact labels and `?`. `observed_only` marks active roots whose retained token metrics are not available yet. Sessions outside the scan and scanned zeros both stay absent. An agent that died mid-turn stays open until the idle window closes it; the UI never claims verified process liveness.

### Trends View

The widget's slow instrument: three week-over-week rows — tokens, velocity, cache efficiency — each with this week's value, the delta against last week, and paired mini-bars.

The windows are the last seven days against the seven before them and are not re-scopable, which is why the view registers itself unranged. A delta renders only when both weeks have a figure, since a percentage against an absent week is invented movement. Color stays on the delta and is assigned by meaning: rising velocity and rising cache efficiency read green, while token volume, being neither good nor bad, stays neutral. Only velocity degrades under retention, and a condensed line beneath the rows states the cutoff whenever either compared week is affected.

### Charts View

Tokens, code changes, and cache efficiency stacked on one time axis under one crosshair, so the three can be read against each other.

Every panel is bucketed onto the timestamps `get_provider_token_series` returns for the region's range, because a panel drawn on a different grid would make the shared crosshair assert an alignment that does not exist. Cache rates come from summed numerators and denominators per bucket rather than averaged point rates, so the range figure is the same weighted rate the Usage footer reports. There is no floating tooltip: scrubbing (pointer, or arrow keys once the stack has focus) swaps each panel's head value in place and brightens the matching axis tick, which makes the axis itself the time readout and costs no layout shift. Because that tick is the readout, the axis caption carries whatever the returned grid needs to keep every tick distinct — clock time inside a day, weekday plus hour once the buckets are narrower than the days they span. Gaps stay gaps — a bucket with no cacheable input breaks the cache line instead of drawing 0%.
...
`src-tauri/claude-integration/scripts/context-capture.cjs` is the shared bundled capture hook deployed to both Claude and Codex. It writes compact JSONL events under `~/.config/quill/context/continuity/`, captures prompts and simple decision/task hints, and stores PreCompact or Stop snapshots when available. SessionStart guidance is scoped by provider and project key, where the project key is the nearest git root for the current `cwd` or the normalized `cwd` when no git root is found, so recent work from another project cannot leak into a new session. Continuity JSONL and per-session files are pruned to a 30-day retention window at most once per day; aggregate JSONL pruning uses an advisory per-file lock, a temporary replacement, and atomic rename so concurrent aggregate appends are preserved.

The SessionStart `<quill_continuity>` directive rejects trivial prompts (trimmed text shorter than 12 characters or one whitespace-free token) as `last_prompt`. It first anchors on the newest session with both a non-trivial prompt and hints, keeping the prompt and hints coherent; within that session it prefers snapshots and uses events only to fill empty fields. If no coherent session exists, it selects the newest non-trivial prompt and fills remaining hints by recency; if every prompt is trivial, `last_prompt` is omitted while useful hints may still inject. The directive stays suppressed only when all fields are empty. Its `capture.guidance` telemetry records the selected `source` (`snapshot` or `event`), `trivialSkipped` count, and whether the selected context was `coherent`.
