...
Session rows keep provider identity alongside `session_id`, so every downstream read stays scoped to the correct Claude or Codex session. `SessionBreakdown.subagent_count` is rendered marked rather than exact — see [[frontend#Frontend#Components#Retention Degradation#Mixed-Horizon Sub-Agent Counts]].
