...
Direct resets ride `UsageData.provider_resets`. A provider served from cached buckets, or from a Codex fallback that cannot see credits, keeps the resets it last reported in the process cache. CPA accounts carry theirs in `quota.resets`, persisted with the account snapshot. Claude grants arrive in the existing usage response; a Codex account whose WHAM usage reports banked credits gets one extra `wham/rate-limit-reset-credits` read, and a failed list read keeps the last known rows. Expired resets are hidden.
...
The default view and the product's core surface: hero chart, one computed insight line, a two-tier metric readout grid, the switchable breakdown, and a totals footer.

The hero chart groups one model-evidence token series by CLI, Provider, or Model, selected by compact controls at its top-right. Model is the fresh-profile default; the valid local choice persists across restarts and storage failures retain the current choice. CLI uses Quill's recorded provider, Provider uses Pi's recorded upstream prefix and otherwise the recorded CLI provider, and Model normalizes cross-CLI identity by stripping Pi's `{upstream}/` prefix from the stored id before grouping, so one model used through different CLIs merges into a single series. The Provider choice retains the internal `llm` storage key so existing saved preferences survive the copy change. Model-less activity remains a factual `unattributed` series rather than being dropped. The headline, delta, legend, insight split, and footer all read the same selected evidence total. Pointer movement or focused arrow/Home/End keys select any aligned time bucket and replace the legend totals with its values while brightening its time and drawing a crosshair plus group markers. Pointer leave, blur, or Escape restores the range summary and final endpoints. The delta compares the back half of the buckets against the front half rather than against the previous window, because a headline delta whose evidence the chart does not draw is not evidence.

The insight line states one computed fact per window — context savings, cached-token volume, or the selected chart grouping — chosen by a fixed priority that speaks last about anything the widget already draws. A candidate speaks only when its figure exists and is non-zero, so the line is never padded; with no eligible candidate it is simply not drawn.

The readout grid carries ten metrics in two tiers, each with a metric-hue swatch and value. The primary row adds a sparkline and a secondary chip: **Runtime** (total active time across every provider's sessions and agents — model generation, reasoning, and tool execution counted together, user-idle gaps over 5 minutes excluded; chip: multiple of the selected window), **Tools** (tool calls of every category; chip: failure rate over calls with a recorded outcome, absent when none record one), **Turns** (LLM turns across every provider and agent; chip: active runtime per turn), and **Net lines**. The secondary tier is value and label: **Prompts** (root-session user messages; chip: per session), **Reasoning** (share of output spent on reasoning over the turns that report it, Pi only today; chip: the reasoning token total), **Tokens per LOC**, **LOC per hour** (both over active LLM runtime, not wall clock), **Sessions**, and **Projects**. Every chip is a measurement with a stated denominator or absent; none defaults to zero. Every band reads the region's selected range, so chart, delta, insight, all ten readouts and the footer always describe the same window. The Projects readout reuses the selected Projects breakdown when available instead of mounting the same query twice.
...
Compaction and branch-summary spend carries no model identity, so when the range holds any it appears as a hairline-separated "Summaries (unattributed)" entry beneath the ranked rows — observation count plus token figure, never a pseudo-model row with a swatch — so the per-model figures visibly reconcile against range totals that already include that spend. The entry is absent when the range holds none; summaries whose raw rows retention pruned stay in totals but leave the entry.
...
[[src-tauri/src/storage.rs#Storage#apply_pi_protocol_v2_envelope]] commits receipt dedupe, process/sequence ordering, durable lifecycle, and replacement close in one transaction. [[src-tauri/src/live_tracker.rs#LiveTracker#apply_pi_protocol_v2_event]] receives only committed `applied` events. Restart and tracking re-enable load durable open rows as recovering; recently closed rows still inside the live idle window seed end tombstones so their fresh transcripts cannot resurrect that occurrence. A recovering row is proven live by its own process's hook telemetry: every Pi `/api/v1/hooks/observed` request names its process, and [[src-tauri/src/server.rs#prove_pi_session_from_hook]] flips the durable row back to `open` and clears the tracker's recovering flag on the same-process match, so a session that survives a Quill restart reads live again on its next tool call. A recovering row nothing proves inside the idle window is a crashed process and is closed by [[src-tauri/src/storage.rs#Storage#expire_pi_recovering_sessions]] on the retained reconciliation cadence. A later `session_start` clears the matching tombstone. These durable rows remain for remote-host ordering, idempotency, lineage, and closure that the local fold cannot reconstruct.
...
The bundled Pi adapter in `src-tauri/pi-integration/quill.ts` applies the same fetch policy to Pi's default `fetch_content` tool and retained fetch aliases. It preserves the tool's single `url` or each string in `urls` when building ready-to-paste `quill_fetch_and_index` guidance.
...
Always-on-top defaults to off on fresh installs and after Reset to defaults. Startup reads the same runtime settings as Settings and the titlebar, preserves saved `true` or `false` values, and initializes the main window and tray checkmark from that result. The old `widget_ui_v1` seed no longer writes an enabled preference.
