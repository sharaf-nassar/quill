...
Session rows key identity by provider, normalized hostname, and root session. `observed_subagent_count` is nullable current-boot evidence, while `observed_subagent_models` partitions each positive total into raw exact model ids plus an explicit null-id unknown group. Codex agent-start hooks provide the active model; Claude agents resolve only from retained child-transcript evidence keyed by root and agent id, never from the root model. The UI combines exact ids into provider families, orders Claude Opus → Sonnet → Haiku and Codex Sol → Terra → Luna, then places `?` last. `observed_only` marks active roots whose retained token metrics are not available yet. Missing coverage and observed zero remain absent. Normal hook delivery is exact, but a missed stop can remain observed open until parent end or restart; the UI never claims verified process liveness.
