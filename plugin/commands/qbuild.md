...
| Choice | Commands |
|--------|----------|
| merge | `cd "<ORIGINAL_DIR>" && rm -f ".qbuild-lock.$LOCK_ID" && git merge --ff-only "qbuild/<slug>" && git worktree remove "<WORKTREE_PATH>" && git branch -d "qbuild/<slug>"` |
| squash | `cd "<ORIGINAL_DIR>" && rm -f ".qbuild-lock.$LOCK_ID" && git merge --squash "qbuild/<slug>" && git commit -m "<summary>" && git worktree remove "<WORKTREE_PATH>" && git branch -D "qbuild/<slug>"` |
| skip | `cd "<ORIGINAL_DIR>" && rm -f ".qbuild-lock.$LOCK_ID"` — leave worktree in place, provide path and commands for later |
